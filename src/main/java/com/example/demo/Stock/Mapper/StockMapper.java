...
public interface StockMapper {
	@Select("IF EXISTS (SELECT distinct * FROM data_stock_\r\n" + "	WHERE id=#{id}) \r\n"
			+ "	SELECT 'True' ELSE  SELECT 'False'")
	boolean existsTransaction(@Param("id") Long id);

	@Select("select DISTINCT fmms.* from data_stock_mms fmms, data_stock_ fc where\r\n"
			...
			+ "	SELECT DISTINCT fmms.* FROM data_stock_mms fmms inner JOIN data_stock_ fc ON ((ROUND(fmms.amount, 2) = fc.amount) OR (ROUND(fc.amount, 2) = fmms.amount)) and fmms.account_segment like '%'+#{type}+ '%' and   fc.account like '%'+#{type}+ '%'\r\n"
	...
	@Select(" select DISTINCT fc.*  from data_stock_mms fmms, data_stock_ fc where\r\n"
			...
			+ "	select DISTINCT fc.*  from data_stock_mms fmms, data_stock_ fc where\r\n"
	...
	List<RawStockModel> get_stock__for_recon_auto(@Param("account") String account,
	...
	@Select("INSERT INTO stock_(posting_date,value_date,account,description,transaction_reference,source_branch,branch_name,amount,dr_cr,balance,match_status,status,availability,match_id)OUTPUT Inserted.id "
			...
			+ " FROM data_stock_ where id = #{id};delete from data_stock_ where id = #{id};")
	Long moveStockData(@Param("id") Long long1, @Param("match_id") String match_id);

	@Select("select distinct fc.*, fm.match_date, u.firstname, u.middlename as lastname\r\n"
			+ "    from  stock_ fc,users u,user_stock_matched ufm, stock_matched fm where fc.status = 1 and fc.availability = 1 and fc.status = 1\r\n"
	...
	List<RawStockModel> get_stock__matched_for_view(@Param("date") int recon_date,
			@Param("account") String account);

	@Select("select distinct fc.*,mr.reason, fm.match_date, u.firstname, u.middlename as lastname\r\n"
			+ "    from  stock_ fc,users u,user_stock_matched ufm, stock_matched fm,match_reason mr where fc.status = 1 and fc.availability = 1 and fc.status = 1\r\n"
	...
	List<RawStockModel> get_stock__matched_with_reason(@Param("date") int recon_date,
			@Param("account") String account);
	...
	List<RawStockModel> get_stock__matched_with_reasonAll(@Param("date") int recon_date);

	@Select("select * from reversal_stock_ where match_date = #{date} and availability=1 and status=1 ")
	List<RawStockModel> get_stock_reversal(@Param("date") int recon_date);
	...
	@Select("select * from data_stock_ where status = 1 and availability = 1 \r\n"
			+ "   and cast(value_date as date) <= #{date} and (account like '%'+ #{account}+'%')")
	List<RawStockModel> get_stock__unmatched_for_view(@Param("date") String recon_date,
			@Param("account") String account);
	...
	List<RawStockModel> get_stock__unmatched_for_viewAll(@Param("date") String recon_date);
...
//	added by Demeke
	@Select(" select DISTINCT fc.* from data_stock_ fc where\r\n"
			+ "	fc.status = 1 and fc.availability = 1 and (fc.account like '%'+#{account}+ '%')\r\n"
			+ "	and cast(fc.value_date as date) <= #{date}")
	List<RawStockModel> get_stock__for_recon_manual(@Param("account") String account,
			@Param("date") String recon_date);

	@Select(" select DISTINCT fc.* from data_stock_ fc where\r\n"
			+ "	fc.status = 1 and fc.availability = 1 and cast(fc.value_date as date) <= #{date}")
	List<RawStockModel> get_all_stock__for_recon_manual(@Param("date") String recon_date);
	...
	@Select("INSERT INTO stock__edit_history(posting_date, value_date, account, description, transaction_reference, source_branch, branch_name, amount, dr_cr, balance, match_status, status, availability,"
			...
			+ "availability, #{reason_id}, 'deleted' FROM data_stock_ where id = #{id};")
	Long moveDeletedStockData(@Param("id") Long id, @Param("reason_id") Long reason_id);

	@Update("update data_stock_ set availability = '0' where id = #{id}")
	...
	@Select("INSERT INTO reversal_stock_(posting_date, value_date, account, description, transaction_reference, source_branch, branch_name, amount, dr_cr, balance, status, availability,"
			...
			+ "availability, #{match_id}, #{match_date}, #{match_by} FROM data_stock_ where id = #{id}; delete from data_stock_ where id = #{id};")
	Long moveStockReversalData(@Param("id") Long long1, @Param("match_id") String match_id,
...
//	added By demeke
	...
	List<RawStockModel> get_stock__matched_for_viewAll(@Param("date") int recon_date);
	...
	@Select("INSERT INTO data_stock_(posting_date,value_date,account,description,transaction_reference,source_branch,branch_name,amount,dr_cr,balance,match_status,status,availability ) OUTPUT Inserted.id "
			...
			+ "FROM stock_ where id = #{id}; update stock_ set availability = 0 where id = #{id};")
	Long moveStockMatched(Long id);

	@Select("INSERT INTO data_stock_(posting_date,value_date,account,description,transaction_reference,source_branch,branch_name,amount,dr_cr,balance,match_status,status,availability ) OUTPUT Inserted.id "
			+ "SELECT posting_date,value_date,account,description,transaction_reference,source_branch,branch_name,amount,dr_cr,balance,'0',status,availability "
			+ "FROM reversal_stock_ where id = #{id}; update reversal_stock_ set availability = 0 where id = #{id};")
	Long moveStockMatchedReversal(Long id);
	...
	@Select("select distinct dna.id, dna.transaction_reference, dna.posting_date, dna.value_date,  dna.amount,  dna.source_branch, dna.branch_name, dna.description, dna.dr_cr,  dna.account, er.edit_delete from data_stock_ dna, edit_reason er where dna.id=er.current_id and er.type='data_stock_'\r\n"
			+ " union select distinct rna.id, rna.transaction_reference, rna.posting_date, rna.value_date,  rna.amount,  rna.source_branch, rna.branch_name, rna.description, rna.dr_cr,  rna.account, er.edit_delete from stock_ rna, edit_reason er where rna.id=er.current_id and er.type='stock_' and er.edit_delete='1' ")
	List<StockDeleted> get_edited_stock_();

	@Select("select distinct dna.id, dna.new_old,  dna.transaction_reference, dna.posting_date, dna.value_date,  dna.amount,  dna.source_branch, dna.branch_name, dna.description, dna.dr_cr,  dna.account, er.reason, u.firstname, u.lastname, er.DATE, er.edit_delete , dna.edit_reason_id from stock__edit_history dna, edit_reason er, users u where er.current_id=#{id} and er.id=dna.edit_reason_id and u.id=er.user_id")
	List<stockDetail> get_edited_detail_stock_(Long id);
