...
public interface FixedAutReconMapper {
	...
	List<RawFixedCoreModel> get_fixed_core_for_recon_auto_computer(@Param("account_id") Long account_id,
			@Param("date") String recon_date);
	
	@Select("select  DISTINCT fc.*  from data_fixed_mms fmms, data_fixed_core fc where"
			+ " Patindex('%'+fmms.giv_number+'%', fc.naration) !=0 and (ROUND(fmms.original_cost, 2) = fc.credit or ROUND(fmms.original_cost, 2)=fc.debit)and fc.status = 1 and fc.availability = 1 and  (fc.account_number like '01196%' or fc.account_number like '01A96%') and fmms.status = 1 and fmms.availability = 1 and fc.transaction_date<=#{date} and fmms.created_date<=#{date}")
	List<RawFixedCoreModel> get_fixed_core_for_recon_auto_equipment(@Param("account_id") Long account_id,
			@Param("date") String recon_date);
	
	@Select("select DISTINCT  fc.*  from data_fixed_mms fmms, data_fixed_core fc where"
			+ " Patindex('%'+fmms.giv_number+'%', fc.naration) !=0 and (ROUND(fmms.original_cost, 2) = fc.credit or ROUND(fmms.original_cost, 2)=fc.debit)and fc.status = 1 and fc.availability = 1 and (fc.account_number like '01198%' or fc.account_number like '01A98%') and fmms.status = 1 and fmms.availability = 1 and fc.transaction_date<=#{date} and fmms.created_date<=#{date}")
	List<RawFixedCoreModel> get_fixed_core_for_recon_auto_vehicle(@Param("account_id") Long account_id,
			@Param("date") String recon_date);
	
	@Select("select DISTINCT fc.*  from data_fixed_mms fmms, data_fixed_core fc where"
			+ " Patindex('%'+fmms.giv_number+'%', fc.naration) !=0 and (ROUND(fmms.original_cost, 2) = fc.credit or ROUND(fmms.original_cost, 2)=fc.debit)and fc.status = 1 and fc.availability = 1 and (fc.account_number like '01195%' or fc.account_number like '01A95%') and fmms.status = 1 and fmms.availability = 1 and fc.transaction_date<=#{date} and fmms.created_date<=#{date}")
	List<RawFixedCoreModel> get_fixed_core_for_recon_auto_furniture(@Param("account_id") Long account_id,
			@Param("date") String recon_date);
	...
	////// Move fixed core data when data matched////////////

	@Select("INSERT INTO fixed_core(account_number, transaction_date, posting_date, value_date, credit, debit, "
			...
			+ " FROM data_fixed_core where id = #{id};delete from data_fixed_core where id = #{id};")
	Long moveCoreFixedData(@Param("id") Long long1, @Param("match_id") String match_id);
	...
	@Select("select fc.id, fc.account_number, fc.transaction_date, fc.posting_date, fc.value_date, fc.credit, fc.debit, fc.branch_code, fc.reference,"
			...
			+ " from  fixed_core fc,users u,user_fixed_matched ufm, fixed_matched fm where fc.status = 1 and fc.availability = 1 and fc.status = 1"
...
//        		+ " and fm.match_id= fc.match_id and fm.match_date=#{date, jdbcType=INTEGER} and u.id=ufm.user_id and ufm.fixed_matched_id=fm.id and ufm.availability=1 and ufm.status=1")
	List<UnMatchFixedCoreModel> get_fixed_core_for_view(@Param("date") String date);
	...
	@Select("select fc.id, fc.account_number, fc.transaction_date, fc.posting_date, fc.value_date, fc.credit, fc.debit, fc.branch_code, fc.reference,"
			...
			+ " from  fixed_core fc,users u,user_fixed_matched ufm, fixed_matched fm, match_reason mr where fc.status = 1 and fc.availability = 1 and fc.status = 1 and mr.match_id= fm.match_id"
			+ " and fm.match_id= fc.match_id and fm.match_date=#{date} and u.id=ufm.user_id and ufm.fixed_matched_id=fm.id and ufm.availability=1 and ufm.status=1")
	List<Transactionhistory> get_fixed_core_for_view_reason(int date);

	@Select("INSERT INTO data_fixed_core(account_number, transaction_date, posting_date, value_date, credit, debit, "
			...
			+ "FROM fixed_core where id = #{id}; update fixed_core set availability = 0 where id = #{id};")
	Long moveFixedCoreMatched(Long id);
	...
	@Select("select * from data_fixed_core where status = 1 and availability = 1")
	List<RawFixedCoreModel> get_unmatched_fixed_core_for_view();
	// =================================
	...
	@Select("select * from data_fixed_core dfc where dfc.status = 1 and dfc.availability = 1 "
			+ "and  dfc.account_number like '01197%' and cast(dfc.transaction_date as date) <= #{date}")
	List<RawFixedCoreModel> get_computer_core_unmatched_for_view(@Param("date") String recon_date);
	...
	List<RawFixedCoreModel> get_furniture_core_unmatched_for_view(@Param("date") String recon_date);
	...
	List<RawFixedCoreModel> get_equipment_core_unmatched_for_view(@Param("date") String recon_date);
	...
	List<RawFixedCoreModel> get_vehicle_core_unmatched_for_view(@Param("date") String recon_date);
	...
	List<UnMatchFixedCoreModel> get_computer_core_matched_for_view(@Param("date") int recon_date);
	...
	List<UnMatchFixedCoreModel> get_furniture_core_matched_for_view(@Param("date") int recon_date);
	...
	List<UnMatchFixedCoreModel> get_equipment_core_matched_for_view(@Param("date") int recon_date);
	...
	List<UnMatchFixedCoreModel> get_vehicle_core_matched_for_view(@Param("date") int recon_date);
	...
	@Select("select id, account_number, transaction_date, posting_date, value_date, credit, debit, branch_code, reference, naration, account_description, account_name, match_id, match_date, firstname from reversal_fixed_core  where  match_date=#{date} and  availability=1 and status=1;")
	List<fixedCoreReversal> get_coreReversal_for_view(@Param("date") String recon_date);
	
	@Select("INSERT INTO data_fixed_core(account_number, transaction_date, posting_date, value_date, credit, debit, "
			+ "branch_code, REFERENCE, naration, account_description, account_name, status,availability,match_status ) OUTPUT Inserted.id "
			+ "SELECT account_number, transaction_date, posting_date, value_date, credit, debit, branch_code, REFERENCE,naration,account_description, "
			+ "account_name, '1', '1', '0' "
			+ "FROM reversal_fixed_core where id = #{id}; update reversal_fixed_core set availability = 0 where id = #{id};")
