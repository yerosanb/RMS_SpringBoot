...
public interface ErcaMapper {
	...
	@Insert("insert into erca_nbe_ats (value_date_type, reference, sender, receiver, additional_information, amount, "
	...
	public abstract void uploadData_erca_nbe(File_erca_nbe erca_nbe);
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	@Select("select * from erca_nbe_ats ena, file_account fa,users u,user_erca_matched uem, erca_matched em where ena.status = 1 and ena.availability = 1 and ena.match_status = 1 \r\n"
			+ "			and ena.file_id = fa.file_id and fa.account_id = #{account_id} and em.match_date=#{date} and em.erca_nbe_ats_id=ena.id and em.status=1\r\n"
			+ "			and em.availability =1 and u.id=uem.user_id and em.id=uem.erca_matched_id and uem.availability=1 and uem.status=1;")
	List<File_rtgs_nbe_ats> get_erca_ats_for_view(@Param("date") String recon_date,
	...
	@Select("select distinct eac.id, eac.additional_information, eac.amount, eac.value_date, eac.branch_code,\r\n"
			...
			+ "em.match_id from erca_awb_core eac, file_account fa,users u,user_erca_matched uem, erca_matched em where eac.status = 1 and eac.availability = 1 and eac.match_status = 1\r\n"
	...
	List<File_rtgs_awb_core> get_erca_core_for_view(@Param("date") String recon_date,
			@Param("account_id") Long account_id);

	@Select("select  distinct * from b2b_nbe_ats bna, b2b_matched bm, file_account fa,users u,user_b2b_matched ubm where bna.status = 1 and bna.availability = 1 and bna.match_status = 1 \r\n"
			+ "			and bna.file_id = fa.file_id and fa.account_id = #{account_id} and bm.match_date= #{date} and bm.b2b_nbe_ats_id = bna.id and bm.status=1 and \r\n"
			+ "			bm.availability=1 and u.id=ubm.user_id and ubm.b2b_matched_partial_id=bm.id and  ubm.availability=1 and ubm.status=1")
	List<File_rtgs_nbe_ats> get_btb_ats_for_view(@Param("date") String recon_date,
			@Param("account_id") Long account_id);
...
@Select("select  distinct bac.id ,bac.additional_information, bac.amount, bac.value_date, bac.branch_code,\r\n"
		...
		+ "  from b2b_awb_core bac, b2b_matched bm, file_account fa,users u,user_b2b_matched ubm  where bac.status = 1 and bac.availability = 1 and bac.match_status = 1\r\n"
	...
	List<File_rtgs_awb_core> get_btb_core_for_view(@Param("date") String recon_date,
			@Param("account_id") Long account_id);

	@Select("select * from rtgs_nbe_ats rna, file_account fa where rna.status = 1 and rna.availability = 1 and rna.match_status = 0 "
			+ "and rna.file_id = fa.file_id and fa.account_id = #{account_id} and rna.upload_date = #{date};")
	List<File_erca_nbe> get_btb_ats_unmatched_for_view(@Param("date") String recon_date,
			@Param("account_id") Long account_id);

	@Select("select * from rtgs_nbe_ats rna, file_account fa where rna.status = 1 and rna.availability = 1 and rna.match_status = 0 "
			+ "and rna.file_id = fa.file_id and fa.account_id = #{account_id} and rna.upload_date = #{date};")
	List<File_erca_nbe> get_btb_core_unmatched_for_view(@Param("date") String recon_date,
			@Param("account_id") Long account_id);
	@Select("select * from sos_nbe_ats sna, sos_matched sm, file_account fa,users u,user_sos_matched usm  where sna.status = 1 and sna.availability = 1 and sna.match_status = 1\r\n"
	...
	List<File_rtgs_nbe_ats> get_sos_ats_for_view(@Param("date") String recon_date,
			@Param("account_id") Long account_id);

	@Select("select * from sos_awb_core sac, sos_matched sm, file_account fa,users u,user_sos_matched usm where sac.status = 1 and sac.availability = 1 and sac.match_status = 1\r\n"
			+ "			and sac.file_id = fa.file_id and fa.account_id = #{account_id} and sm.match_date = #{date} and sm.match_id = sac.match_id \r\n"
			+ "			and sm.availability =1 and sm.status =1 and u.id= usm.user_id and sm.id=usm.sos_matched_id and usm.availability=1 and usm.status=1;")
	List<File_rtgs_awb_core> get_sos_core_for_view(@Param("date") String recon_date,
			@Param("account_id") Long account_id);
	...
	@Select("               select distinct rna.id, rna.value_date_type, rna.sender, rna.receiver, u.firstname,u.lastname,rm.match_date,\r\n"
			...
			+ "						         rna.availability, rna.reference,rna.file_id, rm.match_id from users u,user_rgts_matched urm, rtgs_nbe_ats rna, file_account fa, rtgs_matched rm where rna.status = 1 and rna.availability = 1 and rna.match_status=1 \r\n"
			...
			+ "					             sna.availability, sna.reference,sna.file_id, sm.match_id  from  users u,user_sos_matched usm, sos_nbe_ats sna, sos_matched sm, file_account fa where sna.status = 1 and sna.availability = 1 and sna.match_status = 1\r\n"
			+ "						 		and sna.file_id = fa.file_id and fa.account_id = #{account_id} and sm.match_date =#{date} and sm.sos_nbe_ats_id = sna.id \r\n"
			...
			+ "						        bna.availability, bna.reference,bna.file_id, bm.match_id  from users u,user_b2b_matched ubm, b2b_nbe_ats bna, b2b_matched bm, file_account fa where bna.status = 1 and bna.availability = 1 and bna.match_status = 1 \r\n"
			+ "						 		and bna.file_id = fa.file_id and fa.account_id= #{account_id} and bm.match_date= #{date} and bm.b2b_nbe_ats_id = bna.id and bm.status=1 and \r\n"
			+ "				 		      bm.availability=1 and u.id=ubm.user_id and ubm.b2b_matched_partial_id=bm.id and ubm.availability=1 and ubm.status=1\r\n"
			+ "						                                 union\r\n"
			...
			+ "				               ena.availability, ena.reference, ena.file_id, em.match_id  from  users u,user_erca_matched uem, erca_nbe_ats ena, file_account fa, erca_matched em where ena.status = 1 and ena.availability = 1 and ena.match_status = 1\r\n"
			+ "					          and ena.file_id = fa.file_id and fa.account_id = #{account_id} and em.match_date= #{date} and em.erca_nbe_ats_id=ena.id and em.status=1\r\n"
			+ "					        and em.availability =1  and u.id=uem.user_id and uem.erca_matched_id=em.id and uem.availability=1 and uem.status=1")
	List<File_rtgs_nbe_ats> get_all_ats_for_view(@Param("date") String recon_date,
			@Param("account_id") Long account_id);
	...
	@Select("select distinct eac.id, eac.posting_date, eac.transaction_reference, eac.branch_code,\r\n"
			...
			+ "			 			eac.value_date, eac.additional_information, eac.file_id, eac.match_id from erca_awb_core eac, file_account fa,users u,user_erca_matched uem, erca_matched em where eac.status = 1 and eac.availability = 1 and eac.match_status = 1\r\n"
			...
			+ "					bac.value_date, bac.additional_information, bac.file_id, bac.match_id from b2b_awb_core bac, b2b_matched bm, file_account fa,\r\n"
			...
			+ "					sac.value_date, sac.additional_information, sac.file_id, sac.match_id from sos_awb_core sac, sos_matched sm, file_account fa,\r\n"
			...
			+ "					rac.value_date, rac.additional_information, rac.file_id, rac.match_id from rtgs_awb_core rac, file_account fa, \r\n"
	...
	List<File_rtgs_awb_core> get_all_core_for_view(@Param("date") String recon_date,
			@Param("account_id") Long account_id);

	@Select("select * from data_nbe_ats rna, file_account fa where rna.status = 1 and rna.availability = 1 and rna.match_status = 0 "
			+ "and rna.file_id = fa.file_id and fa.account_id = #{account_id} and rna.upload_date = #{date};")
	List<File_rtgs_nbe_ats> get_all_ats_unmatched_for_view(@Param("date") String recon_date,
			@Param("account_id") Long account_id);
	...
	List<File_rtgs_awb_core> get_all_core_unmatched_for_view(@Param("date") String recon_date,
			@Param("account_id") Long account_id);
	//============================================== start get unmatch payable transaction ========================================
	@Select("select * from data_payable dpc where dpc.status = 1 and dpc.availability = 1  "
			+ "and dpc.dr_cr='CR';")
	List<File_rtgs_awb_core> get_payable_credit_unmatched_for_view();
	@Select("select * from data_payable dpd  where dpd.status = 1 and dpd.availability = 1"
			+ "and dpd.dr_cr='DR';")
	List<File_rtgs_awb_core> get_payable_debit_unmatched_for_view();
	//=============================================== ending get payable unmatch transaction ======================================

	@Select("select * from rtgs_nbe_ats rna, file_account fa where rna.status = 1 and rna.availability = 1 and rna.match_status = 0 "
			+ "and rna.file_id = fa.file_id and fa.account_id = #{account_id} and rna.upload_date = #{date};")
	List<File_erca_nbe> get_all_ats_partial_for_view(@Param("date") String recon_date,
			@Param("account_id") Long account_id);

	@Select("select * from rtgs_nbe_ats rna, file_account fa where rna.status = 1 and rna.availability = 1 and rna.match_status = 0 "
			+ "and rna.file_id = fa.file_id and fa.account_id = #{account_id} and rna.upload_date = #{date};")
	List<File_erca_nbe> get_all_core_partial_for_view(@Param("date") String recon_date,
			@Param("account_id") Long account_id);

	@Update("UPDATE rtgs_nbe_ats SET availability = '0' WHERE id=#{id}")
	public void deleteReconItems(Long i);

////	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////


	@Insert("insert into erca_awb_core (posting_date, transaction_reference, branch_code,amount, "
	...
	public void uploadData_erca_core(File_erca_core erca_core);

	@Select("select value_date_type, sum(amount) as amount from erca_nbe_ats\r\n"
	...
	@Update("update erca_nbe_ats set match_status='1' where value_date_type=#{value_date_type}\r\n"
			+ "and additional_information like'%\r\n" + ".\r\n" + "value of erca%'")
	public void matchErcaNbe(String value_date_type);

	@Update("update erca_awb_core set match_status='1' where amount=#{amount}")
	public void matchErcaCore(float amount);

	@Select("select * from erca_nbe_ats where status = 1 and availability = 1 and match_status = 0 and value_date_type=#{value_date_type} and \r\n "
			+ "and additional_information like'%\r\n" + ".\r\n" + "value of erca%'")
	List<File_erca_nbe> get_erca_nbe_for_recon(String value_date_type);

	@Select("select * from erca_awb_core where status = 1 and availability = 1 and match_status = 0 and amount=#{amount} ;")
	List<File_erca_core> get_erca_core_for_recon(float amount);
	...
	@Select("INSERT INTO data_awb_core(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			...
			+ "FROM erca_awb_core where id = #{id}; update erca_awb_core set availability = 0 where id = #{id};")
	Long moveErcaCoreMatched(@Param("id") Long long1);
	...
	@Select("INSERT INTO data_nbe_ats(value_date_type, sender, receiver, additional_information, amount, dr_cr, upload_date, "
			...
			+ "additional_information, amount, dr_cr, upload_date, '0', status, availability, REFERENCE, file_id FROM erca_nbe_ats "
			+ "where id = #{id}; update erca_nbe_ats set availability= 0 where id = #{id};")
	Long moveErcaAtsMatchedToData(@Param("id") Long long1);

	@Update("update erca_matched set availability=0 where erca_nbe_ats_id=#{id}")
	...
	@Select("INSERT INTO data_awb_core(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			...
			+ "SELECT posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, '0', status, "
			+ "availability, value_date, additional_information, file_id "
			+ "FROM b2b_awb_core where id = #{id}; update b2b_awb_core set availability = 0 where id = #{id};")
	Long moveBtbCoreMatched(@Param("id") Long long1);

	@Select("INSERT INTO data_nbe_ats(value_date_type, sender, receiver, additional_information, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, REFERENCE, file_id)OUTPUT Inserted.id SELECT value_date_type, sender, receiver, "
			+ "additional_information, amount, dr_cr, upload_date, '0', status, availability, REFERENCE, file_id FROM b2b_nbe_ats "
			+ "where id = #{id}; update b2b_nbe_ats set availability= 0 where id = #{id};")
	Long moveBtbAtsMatchedToData(@Param("id") Long long1);

	@Update("update b2b_matched set availability = 0 where b2b_nbe_ats_id=#{id}")
	...
	@Select("INSERT INTO data_awb_core(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			...
			+ "SELECT posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, '0', status, "
			+ "availability, value_date, additional_information, file_id "
			+ "FROM sos_awb_core where id = #{id}; update sos_awb_core set availability = 0 where id = #{id};")
	Long moveSosCoreMatched(@Param("id") Long long1);
	...
	List<File_rtgs_nbe_ats> get_ats_for_view_partial(@Param("account_id") Long account_id);

	@Select("select distinct * from core_partially_matched cpm, partially_matched pm ,comment c,partial_comment pc , file_account fa  ,users u,user_parial_match upm where pm.ats_id= cpm.id and fa.account_id=#{account_id} "
			...
			+ "	pm.match_id=pc.match_id and c.availability=1 and c.status=1 and pc.availability=1 and pc.status=1")
	List<File_rtgs_awb_core> get_core_for_view_partial(@Param("account_id") Long account_id);

	@Select("INSERT INTO data_nbe_ats(value_date_type, sender, receiver, additional_information, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, REFERENCE, file_id) OUTPUT Inserted.Id SELECT value_date_type, sender, receiver, "
			+ "additional_information, amount, dr_cr, upload_date, '0', status, availability, REFERENCE, file_id FROM sos_nbe_ats "
			+ "where id = #{id}; update sos_nbe_ats set availability= 0 where id = #{id};")
	Long moveSosAtsMatchedToData(@Param("id") Long long1);

	@Update("update sos_matched set availability = 0 where sos_nbe_ats_id=#{id}")
	...
	@Select("INSERT INTO data_awb_core(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			...
			+ "SELECT posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, '0', status, "
			+ "availability, value_date, additional_information, file_id "
			+ "FROM core_partially_matched where id = #{id}; update core_partially_matched set availability = 0 where id = #{id};")
	void moveAllPartialCoreMatched(@Param("id") Long long1);
