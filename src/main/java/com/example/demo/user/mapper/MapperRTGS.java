...
public interface MapperRTGS {
	...
	@Select("select * from data_nbe_ats dna, file_account fa where dna.status = 1 and dna.availability = 1 and dna.match_status = 0 "
			+ "and dna.file_id = fa.file_id and fa.account_id = #{account_id} and dna.upload_date <= #{date};")
	List<File_rtgs_nbe_ats> get_ats_for_recon(@Param("date") int recon_date, @Param("account_id") Long account_id);

	@Select("select * from rtgs_nbe_ats rna, file_account fa, rtgs_matched rm, rtgs_awb_core rac where rna.status = 1 and rna.availability = 1 and rna.match_status = 1 "
			+ "and rna.file_id = fa.file_id and fa.account_id = #{account_id} and rm.match_date = #{date} and rm.match_id=rac.match_id and rna.id=rm.rtgs_nbe_ats_id")
	List<File_rtgs_nbe_ats> get_ats_matched(@Param("date") String matched_date, @Param("account_id") Long account_id);

	@Select("select * from rtgs_nbe_ats rna, file_account fa, rtgs_partially_matched rpm, rtgs_awb_core rac where rna.status = 1 and rna.availability = 1 and rna.match_status = 1 "
			+ "and rna.file_id = fa.file_id and fa.account_id = #{account_id} and rpm.match_date = #{date} and rpm.match_id=rac.match_id and rna.id=rpm.rtgs_nbe_ats_id")
	List<File_rtgs_nbe_ats> get_ats_partially_matched(@Param("date") String matched_date,
			@Param("account_id") Long account_id);

	@Select("select * from data_awb_core dac, file_account fa where dac.status = 1 and dac.availability = 1 and dac.match_status = 0 "
			+ "and dac.file_id = fa.file_id and fa.account_id = #{account_id};")
	List<File_rtgs_awb_core> get_core_for_recon(@Param("account_id") Long account_id);

	////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	@Select("select * from rtgs_nbe_ats rna, file_account fa, users u,user_rgts_matched urm, rtgs_matched rm where rna.status = 1 and rna.availability = 1 and rna.match_status=1  "
			+ " and rna.file_id = fa.file_id and fa.account_id = #{account_id} and  rm.match_date = #{date} and rm.rtgs_nbe_ats_id = rna.id "
	...
	List<File_rtgs_nbe_ats> get_ats_for_view(@Param("date") String recon_date, @Param("account_id") Long account_id);

	@Select("select * from rtgs_awb_core rac, file_account fa,users u,user_rgts_matched urm, rtgs_matched rm where rac.status = 1 and rac.availability = 1 and rac.match_status = 1 "
	...
	List<File_rtgs_awb_core> get_rtgs_core_for_view(@Param("date") String recon_date,
			@Param("account_id") Long account_id);
	@Select("select * from pas_awb_core_reversal  where  match_date=#{date} and  availability=1 and status=1;")
	List<File_rtgs_awb_core> get_coreReversal_for_view(@Param("date") String recon_date);
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// List<File_rtgs_awb_core> get_payable_credit_for_view(
	// @Param("account_id") Long account_id);
	//
	...
	// upm.rtgs_matched_partial_id=pm.id and upm.availability=1 and upm.status=1;")
	// List<File_rtgs_awb_core> get_payable_debit_for_view(
	// @Param("account_id") Long account_id);
...
//	List<File_rtgs_awb_core> get_payable_credit_for_view(@Param("account_id") Long account_id,
...
//	List<File_rtgs_awb_core> get_payable_debit_for_view(@Param("account_id") Long account_id,
//			@Param("match_date") String match_date);
//
...
//	List<File_rtgs_awb_core> get_payable_debit_for_view(@Param("match_date") String match_date);
	...
	@Select("select * from rtgs_awb_core rac, file_account fa, rtgs_matched rm where rac.status = 1 and rac.availability = 1 and rac.match_status = 1"
			+ "and rac.file_id = fa.file_id and fa.account_id = #{account_id} and rm.match_date = #{date} and rm.match_id=rac.match_id;")
	List<File_rtgs_awb_core> get_core_matched(@Param("date") String matched_date, @Param("account_id") Long account_id);

	@Select("select * from rtgs_awb_core rac, file_account fa where rac.status = 1 and rac.availability = 1 and rac.match_status = 1"
			+ "and rac.file_id = fa.file_id and fa.account_id = #{account_id};")
	List<File_rtgs_awb_core> get_core_partially_matched(@Param("account_id") Long account_id);

	@Update("update rtgs_nbe_ats set match_status = 1 where id = #{id}")
	void updateMatchStatus_ats(@Param("id") Long id);

	@Update("update rtgs_nbe_ats set match_status = 0 where id = #{id}")
	void updateUnMatchStatus_ats(@Param("id") Long id);

	@Update("update rtgs_awb_core set match_status = 1, match_id = #{match_id} where id = #{id}")
	void updateMatchStatus_core(@Param("id") Long id, @Param("match_id") String match_id);

	@Update("update rtgs_awb_core set match_status = 0, match_id = null where id = #{id}")
	void updateUnMatchStatus_core(@Param("id") Long id);
	...
	@Select("select match_id from rtgs_awb_core where id=#{id}")
	...
	//////////////////////////////////// insert into payable matched
	//////////////////////////////////// ending//////////////////////////
	@Select("insert into rtgs_matched (rtgs_nbe_ats_id, match_id, match_date, reconciliation_type, many_to_many, status, availability) OUTPUT Inserted.Id "
			+ "values(#{id}, #{match_id}, #{date}, #{recon_type}, #{many_to_many}, #{status}, #{availability})")
	Long addRtgsMatched(@Param("id") Long id, @Param("match_id") String match_id, @Param("date") Long date_now,
			@Param("recon_type") String recon_type, @Param("many_to_many") String many_to_many,
			@Param("status") String status, @Param("availability") String availability);

	@Select("insert into b2b_matched (b2b_nbe_ats_id, match_id, match_date, reconciliation_type, many_to_many, status, availability) OUTPUT Inserted.Id "
			+ "values(#{id}, #{match_id}, #{date}, #{recon_type}, #{many_to_many}, #{status}, #{availability})")
	Long addB2bMatched(@Param("id") Long id, @Param("match_id") String match_id, @Param("date") Long date_now,
			@Param("recon_type") String recon_type, @Param("many_to_many") String many_to_many,
			@Param("status") String status, @Param("availability") String availability);

	@Select("insert into erca_matched (erca_nbe_ats_id, match_id, match_date, reconciliation_type, many_to_many, status, availability) OUTPUT Inserted.Id "
			+ "values(#{id}, #{match_id}, #{date}, #{recon_type}, #{many_to_many}, #{status}, #{availability})")
	Long addErcaMatched(@Param("id") Long id, @Param("match_id") String match_id, @Param("date") Long date_now,
			@Param("recon_type") String recon_type, @Param("many_to_many") String many_to_many,
			@Param("status") String status, @Param("availability") String availability);

	@Select("insert into sos_matched (sos_nbe_ats_id, match_id, match_date, reconciliation_type, many_to_many, status, availability) OUTPUT Inserted.Id "
			+ "values(#{id}, #{match_id}, #{date}, #{recon_type}, #{many_to_many}, #{status}, #{availability})")
	...
	@Select("select id from rtgs_matched where rtgs_nbe_ats_id = #{id}")
	Long getrtgsMatchedId(@Param("id") long id);

	@Select("select id from b2b_matched where b2b_nbe_ats_id = #{id}")
	Long getb2bMatchedId(@Param("id") long id);

	@Select("select id from sos_matched where sos_nbe_ats_id = #{id}")
	Long getsosMatchedId(@Param("id") long id);

	@Select("select id from erca_matched where erca_nbe_ats_id = #{id}")
	...
	@Select("select distinct dna.id, dac.id as core_id, dna.value_date_type, dna.sender, dna.receiver,"
			+ "	dna.additional_information, dna.amount, dna.dr_cr, dna.upload_date, dna.reference from data_nbe_ats dna, data_awb_core dac, file_account fa where dac.additional_information like CONCAT('%', dna.reference, '%') "
			...
			+ "union "
			+ "select distinct dna.id, dac.id as core_id, dna.value_date_type, dna.sender, dna.receiver, "
			+ "dna.additional_information, dna.amount, dna.dr_cr, dna.upload_date, dna.reference from data_nbe_ats dna, data_awb_core dac, file_account fa where REPLACE(dna.additional_information, ' ', '')  like CONCAT('%', REPLACE(LEFT(dac.additional_information, CASE WHEN LEN(dac.additional_information) >= 4 THEN LEN(dac.additional_information) - 4 ELSE 0 END), ' ', ''), '%') "
			+ "and dac.amount =dna.amount and dac.dr_cr != dna.dr_cr and dna.status = 1 and dna.availability = 1 and dna.match_status = 0 and dna.file_id = fa.file_id and fa.account_id = #{account_id} "
			+ "and dna.upload_date <= #{date} and dac.status = 1 and dac.availability = 1 and dac.match_status = 0 and dac.upload_date <= #{date} and LEN(CAST(dna.reference AS VARCHAR))>3")
	List<File_rtgs_nbe_ats> get_ats_for_recon_auto(@Param("date") int recon_date, @Param("account_id") Long account_id);

	@Select("select dna.id, dac.id as core_id, dna.value_date_type, dna.sender, dna.receiver,"
			+ "	dna.additional_information, dna.amount, dna.dr_cr, dna.upload_date, dna.reference from data_nbe_ats dna, data_awb_core dac, file_account fa where dac.additional_information like CONCAT('%', dna.reference, '%') "
	...
	List<File_rtgs_nbe_ats> get_ats_for_recon_partial_auto(@Param("date") int recon_date,
			@Param("account_id") Long account_id);
	...
	@Select("select distinct dac.id, dac.posting_date, dac.transaction_reference, dac.branch_code,"
			...
			+ " from data_awb_core dac, data_nbe_ats dna, file_account fa where dac.additional_information like CONCAT('%', dna.reference, '%') "
			...
			+ " dac.amount, dac.dr_cr, dac.upload_date, dac.match_status, dac.status, dac.availability, dac.value_date, dac.additional_information, dac.file_id, dac.match_id "
			+ "	from data_awb_core dac, data_nbe_ats dna, file_account fa where REPLACE(dna.additional_information, ' ', '') LIKE CONCAT('%', REPLACE(CASE WHEN LEN(dac.additional_information) >= 4 THEN LEFT(dac.additional_information, LEN(dac.additional_information) - 4) ELSE '' END, ' ', ''), '%') "
			+ " AND dac.amount = dna.amount "
			+ "	and dac.amount = dna.amount and dac.dr_cr != dna.dr_cr and dac.status = 1 and dac.availability = 1 and dac.match_status = 0 and dac.file_id = fa.file_id and fa.account_id =#{account_id}  "
			+ "	and dna.status = 1 and dna.availability = 1 and dna.match_status = 0 and dna.upload_date <= #{date} and dac.upload_date <= #{date} and LEN(CAST(dna.reference AS VARCHAR))>3")
	List<File_rtgs_awb_core> get_core_for_recon_auto(@Param("account_id") Long account_id,
...
//	List<File_rtgs_awb_core> get_payable_credit_for_recon_auto(@Param("account_id") Long account_id);
	...
	List<File_rtgs_awb_core> get_payable_credit_for_recon_auto();
...
//	List<File_rtgs_awb_core> get_payable_debit_for_recon_auto(@Param("account_id") Long account_id);
	...
	@Select(" 			 select distinct dpd.id,  "
			...
			+ "			  and dpd.availability=1 and dpd.status=1 "
			+ "			  and dpc.availability=1 and dpc.status=1 order by dpd.amount asc")
	List<File_rtgs_awb_core> get_payable_debit_for_recon_auto();
	...
	List<File_rtgs_awb_core> get_core_for_recon_partial_auto(@Param("account_id") Long account_id,
			@Param("date") int recon_date);
	...
	@Select("select distinct cpm.id, cpm.posting_date, cpm.transaction_reference, cpm.branch_code, "
			...
			+ "			       from core_partially_matched cpm  where cpm.availability =1 and cpm.status=1 union  "
			+ "select distinct dac.id, dac.posting_date, dac.transaction_reference, dac.branch_code, "
			...
			+ "				from data_awb_core dac, file_account fa, "
			+ "				ats_partially_matched apm, partially_matched pm, core_partially_matched cpm where  "
	...
	List<File_rtgs_awb_core> get_core_for_recon_partial(@Param("account_id") Long account_id);

	@Select("select distinct rna.id from data_nbe_ats rna, data_awb_core rac, file_account fa where rna.amount = "
			+ "(select amount from data_nbe_ats rna where rna.id = #{ats_id}) and rna.file_id = fa.file_id"
			+ " and rna.status = 1 and rna.availability = 1 and rna.match_status = 0 and rna.reference = "
			+ "(select reference from data_nbe_ats rna where rna.id = #{ats_id}) and rna.upload_date = #{date}")
	...
	@Select("select distinct rac.id from data_nbe_ats rna, data_awb_core rac, file_account fa where rac.amount = "
			+ " (select amount from data_nbe_ats rna where rna.id = #{ats_id}) and"
			+ " rac.additional_information like CONCAT('%', (select reference from data_nbe_ats rna where rna.id = #{ats_id}), '%') and rac.status = 1 "
	...
	Long[] getMatchedIdsFromCoreByAtsId(@Param("ats_id") Long ats_id, @Param("date") String recon_date);

	// @Update("update rtgs_nbe_ats set match_status = 2 where id = #{id}")
	...
	@Select("insert into rtgs_partially_matched (rtgs_nbe_ats_id, match_id, match_date, reconciliation_type, many_to_many, status, availability) OUTPUT Inserted.Id "
			+ "values(#{id}, #{match_id}, #{date}, #{recon_type}, #{many_to_many}, #{status}, #{availability})")
	Long addRtgsMatched_partial(@Param("id") Long id, @Param("match_id") String match_id, @Param("date") Long date_now,
			@Param("recon_type") String recon_type, @Param("many_to_many") String many_to_many,
			@Param("status") String status, @Param("availability") String availability);

	// @Update("update rtgs_awb_core set match_status = 2, match_id = #{match_id}
	// where id = #{id}")
	// void updateMatchStatus_core_partial(@Param("id") Long id, @Param("match_id")
	// String match_id);

	// ================================================= not main
	@Select("SELECT CASE WHEN EXISTS ("
			+ " SELECT * FROM data_nbe_ats where id = #{id} and RIGHT( value_date_type, 3 ) = '103' )"
			+ " THEN 1 ELSE 0 END")
	int isRtgs(@Param("id") Long long1);

	@Select("SELECT CASE WHEN EXISTS ("
			+ " SELECT * FROM data_nbe_ats where id = #{id} and RIGHT( value_date_type, 3 ) = '204' )"
			+ " THEN 1 ELSE 0 END")
	int isErca(@Param("id") Long long1);

	@Select("SELECT CASE WHEN EXISTS ("
			+ " SELECT * FROM data_nbe_ats where id = #{id} and RIGHT( value_date_type, 3 ) = '202' )"
			+ " THEN 1 ELSE 0 END")
	int isB2b(@Param("id") Long long1);

	@Select("SELECT CASE WHEN EXISTS ("
			+ " SELECT * FROM data_nbe_ats where id = #{id} and ((value_date_type like '%298%' and (additional_information like '%sid%' or additional_information like '%TTC%')) or RIGHT( value_date_type, 3 ) = '205'))"
			+ " THEN 1 ELSE 0 END")
	int isSos(@Param("id") Long long1);

	@Select("INSERT INTO rtgs_nbe_ats(value_date_type, sender, receiver, additional_information, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, REFERENCE, file_id) OUTPUT Inserted.id SELECT value_date_type, sender, receiver, "
			+ "additional_information, amount, dr_cr, upload_date, '1', status, availability, REFERENCE, file_id FROM data_nbe_ats "
			+ "where id = #{id}; delete from data_nbe_ats where id = #{id};")
	...
	@Select("INSERT INTO erca_nbe_ats(value_date_type, sender, receiver, additional_information, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, REFERENCE, file_id) OUTPUT Inserted.id SELECT value_date_type, sender, receiver, "
			+ "additional_information, amount, dr_cr, upload_date, '1', status, availability, REFERENCE, file_id FROM data_nbe_ats "
			+ "where id = #{id}; delete from data_nbe_ats where id = #{id};")
	Long moveErcaAtsData(@Param("id") Long long1);

	@Select("INSERT INTO sos_nbe_ats(value_date_type, sender, receiver, additional_information, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, REFERENCE, file_id) OUTPUT Inserted.id SELECT value_date_type, sender, receiver, "
			+ "additional_information, amount, dr_cr, upload_date, '1', status, availability, REFERENCE, file_id FROM data_nbe_ats "
			+ "where id = #{id}; delete from data_nbe_ats where id = #{id};")
	Long moveSosAtsData(@Param("id") Long long1);

	@Select("INSERT INTO rtgs_awb_core(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			...
			+ "FROM data_awb_core where id = #{id}; delete from data_awb_core where id = #{id};")
	Long moveRtgsCoreData(@Param("id") Long long1, @Param("match_id") String match_id);
	...
	// ===========================
	@Select("INSERT INTO sos_awb_core(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, value_date, additional_information, file_id, match_id )OUTPUT Inserted.id "
			+ "SELECT posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, '1', status, "
			+ "availability, value_date, additional_information, file_id, #{match_id} "
			+ "FROM data_awb_core where id = #{id}; delete from data_awb_core where id = #{id};")
	Long moveSosCoreData(@Param("id") Long long1, @Param("match_id") String match_id);

	@Select("INSERT INTO erca_awb_core(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, value_date, additional_information, file_id, match_id )OUTPUT Inserted.id "
			+ "SELECT posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, '1', status, "
			+ "availability, value_date, additional_information, file_id, #{match_id} "
			+ "FROM data_awb_core where id = #{id}; delete from data_awb_core where id = #{id};")
	Long moveErcaCoreData(@Param("id") Long long1, @Param("match_id") String match_id);

	@Select("INSERT INTO b2b_nbe_ats(value_date_type, sender, receiver, additional_information, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, REFERENCE, file_id) OUTPUT Inserted.id SELECT value_date_type, sender, receiver, "
			+ "additional_information, amount, dr_cr, upload_date, '1', status, availability, REFERENCE, file_id FROM data_nbe_ats "
			+ "where id = #{id}; delete from data_nbe_ats where id = #{id};")
	Long moveB2bAtsData(@Param("id") Long long1);

	@Select("INSERT INTO b2b_awb_core(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, value_date, additional_information, file_id, match_id )OUTPUT Inserted.id "
			+ "SELECT posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, '1', status, "
			+ "availability, value_date, additional_information, file_id, #{match_id} "
			+ "FROM data_awb_core where id = #{id}; delete from data_awb_core where id = #{id};")
	Long moveB2bCoreData(@Param("id") Long long1, @Param("match_id") String match_id);
	@Select("INSERT INTO pas_awb_core_reversal(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			...
			+ "FROM data_awb_core where id = #{id}; delete from data_awb_core where id = #{id};")
	Long moveCoreReversalData(@Param("id") Long long1, @Param("match_id") String match_id, @Param("matched_date") Long matched_date, @Param("matched_by") String matched_by);
	// =================================================main
	...
	@Select("select ending_balance_con from files where availability=1 and type='CORE'and upload_date=#{lastUploadDate}")
	String check_openig_with_closing_balance_core_con(@Param("lastUploadDate") String lastUploadDate);

	@Select("select ending_balance_ifb from files where availability=1 and type='CORE' and upload_date=#{lastUploadDate}")
	String check_openig_with_closing_balance_core_ifb(@Param("lastUploadDate") String lastUploadDate);

	@Select("select ending_balance_ifb from files where availability=1 and type='ISSUE_CORE' and upload_date=#{lastUploadDate}")
	String check_openig_with_closing_balance_issue_core(@Param("lastUploadDate") String lastUploadDate);
	...
	@Select("select count(*) from files  where type='CORE' and availability=1   ")
	int check_core_for_firstly();

	@Select("select count(*) from files  where type='Receivable' and availability=1   ")
	int check_recievable_for_firstly();

	@Select("select count(*) from files where type='ISSUE_CORE' and availability = 1")
	int check_issue_core_for_firstly();
	...
	@Update("update data_nbe_ats set reference = #{reference}, amount =#{amount}, value_date_type = #{value_date_type}, "
			+ "dr_cr = #{dr_cr}, upload_date = #{upload_date}, additional_information = #{additional_information} where id = #{id}")
	Boolean updateTransaction(File_rtgs_nbe_ats edit_data);

	@Update("update data_awb_core set amount =#{amount}, value_date = #{value_date_type}, "
			+ "dr_cr = #{dr_cr}, upload_date = #{upload_date}, additional_information = #{additional_information} where id = #{id}")
	Boolean updateTransactionCore(File_rtgs_nbe_ats edit_data);
	...
	@Select("insert into edit_reason (user_id, current_id, type, reason, date, status, availability, edit_delete)OUTPUT Inserted.Id values "
			+ "(#{user_id}, #{id},#{type}, #{reason}, #{date}, #{status}, #{availability}, #{edit_delete})")
	Long addReasonForEditCore(@Param("id") Long id, @Param("user_id") Long user_id, @Param("type") String type,
			@Param("reason") String reason, @Param("date") String string, @Param("status") String status,
			@Param("availability") String availability, @Param("edit_delete") String edit_delete);

	@Update("update data_nbe_ats set availability = '0' where id = #{id}")
	void deleteTransaction(@Param("id") Long id);

	@Update("update data_awb_core set availability = '0' where id = #{id}")
	void deleteTransactionCore(@Param("id") Long long1);
	...
	@Select("INSERT INTO core_partially_matched(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, value_date, additional_information, file_id, match_id )OUTPUT Inserted.id "
			+ "SELECT posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, '1', status, "
			+ "availability, value_date, additional_information, file_id, #{match_id} "
			+ "FROM data_awb_core where id = #{id}; delete from data_awb_core where id = #{id};")
	Long moveCorePartiallyMatched(@Param("id") Long long1, @Param("match_id") String match_id);
	...
	@Select("select distinct apm.id, apm.value_date_type, apm.sender, apm.receiver, "
			...
			+ "                dna.availability, dna.reference,dna.file_id from data_nbe_ats dna, "
			...
			+ "select distinct dna.id, dna.value_date_type, dna.sender, dna.receiver, "
			+ "               dna.additional_information, dna.amount, dna.dr_cr, dna.upload_date, dna.match_status, dna.status, "
			+ "               dna.availability, dna.reference,dna.file_id from  data_nbe_ats dna,partially_matched pm,file_account fa,ats_partially_matched atspm "
	...
	List<File_rtgs_nbe_ats> get_ats_for_recon_partial(@Param("account_id") Long account_id);
	...
	@Select("INSERT INTO data_nbe_ats(value_date_type, sender, receiver, additional_information, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, REFERENCE, file_id)OUTPUT Inserted.id SELECT value_date_type, sender, receiver, "
			+ "additional_information, amount, dr_cr, upload_date, '0', status, availability, REFERENCE, file_id FROM rtgs_nbe_ats "
			+ "where id = #{id}; update rtgs_nbe_ats set availability= 0 where id = #{id};")
	public Long moveRtgsAtsMatchedToData(@Param("id") Long long1);

	@Update("update rtgs_matched set availability=0 where rtgs_nbe_ats_id=#{id}")
	public void deleteRtgsMatched(@Param("id") Long long1);

	@Select("INSERT INTO data_awb_core(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			...
			+ "FROM rtgs_awb_core where id = #{id}; update rtgs_awb_core set availability = 0 where id = #{id};")
	Long moveRtgsCoreMatched(@Param("id") Long long1);

	@Select("INSERT INTO data_awb_core(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, value_date, additional_information, file_id )OUTPUT Inserted.id "
			+ "SELECT posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, '0', status, "
			+ "availability, value_date, additional_information, file_id "
			+ "FROM pas_awb_core_reversal where id = #{id}; update pas_awb_core_reversal set availability = 0 where id = #{id};")
	Long moveCoreReversalMatched(@Param("id") Long long1);
	...
	@Select("INSERT INTO b2b_awb_core(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			...
			+ "SELECT posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, '1', status, "
			+ "availability, value_date, additional_information, file_id, #{match_id} "
			+ "FROM core_partially_matched where id = #{id}; update core_partially_matched set status = '0' where id = #{id};")
	Long moveB2bCoreDataFromPartial(@Param("id") Long long1, @Param("match_id") String match_id);
	...
	// rna.value_date_type as CITY from rtgs_nbe_ats rna")
	// List<Ee> getDataListForReportFake();
	...
	@Select("select \r\n" + //
				"\tdistinct dac.id, dac.amount, dac.branch_code, dac.additional_information, dac.value_date from data_awb_core dac where upload_date <= #{date} and dr_cr = 'DR' and dac.status = 1 and dac.availability = 1 \r\n" + //
				"union \r\n" + //
				"select \r\n" + //
				"\tdistinct dac.id, dac.amount, dac.branch_code, dac.additional_information, dac.value_date from core_partially_matched dac where upload_date <= #{date} and dr_cr = 'DR' and dac.status = 1 and dac.availability = 1 \r\n" + //
				"union \r\n" + //
				"select \r\n" + //
				"\tdistinct rac.id, rac.amount, rac.branch_code, rac.additional_information, rac.value_date from rtgs_awb_core rac, rtgs_matched rm, rtgs_nbe_ats rna \r\n" + //
				"\t\twhere rac.dr_cr = 'DR' and rac.upload_date <= #{date} and (rna.id = rm.rtgs_nbe_ats_id and rac.match_id = rm.match_id and rna.upload_date > #{date}) and rac.status = 1 and rac.availability = 1  \r\n" + //
				"\t\tand rm.status = 1 and rm.availability = 1  and rna.status = 1 and rna.availability = 1  \r\n" + //
				"union \r\n" + //
				"select \r\n" + //
				"\tdistinct eac.id, eac.amount, eac.branch_code, eac.additional_information, eac.value_date from erca_awb_core eac, erca_matched em, erca_nbe_ats ena \r\n" + //
				"\t\twhere eac.dr_cr = 'DR' and eac.upload_date <= #{date} and ((ena.id = em.erca_nbe_ats_id and eac.match_id = em.match_id and ena.upload_date > #{date})) and eac.status = 1 and eac.availability = 1  \r\n" + //
				...
				"select \r\n" + //
				"\tdistinct sac.id, sac.amount, sac.branch_code, sac.additional_information, sac.value_date from sos_awb_core sac, sos_matched sm, sos_nbe_ats sna \r\n" + //
				"\t\twhere sac.dr_cr = 'DR' and sac.upload_date <= #{date} and ((sna.id = sm.sos_nbe_ats_id and sac.match_id = sm.match_id and sna.upload_date >= #{date})) and sac.additional_information = 'ECX Interbank' and sac.upload_date < sna.upload_date and sac.status = 1 and sac.availability = 1  \r\n" + //
				"\t\tand sm.status = 1 and sm.availability = 1  and sna.status = 1 and sna.availability = 1 union \r\n" + //
				"select \r\n" + //
				"\tdistinct bac.id, bac.amount, bac.branch_code, bac.additional_information, bac.value_date from b2b_awb_core bac, b2b_matched bm, b2b_nbe_ats bna \r\n" + //
				"\t\twhere bac.dr_cr = 'DR' and bac.upload_date <= #{date} and ((bna.id = bm.b2b_nbe_ats_id and bac.match_id = bm.match_id and bna.upload_date > #{date})) and bac.status = 1 and bac.availability = 1  \r\n" + //
				"\t\tand bm.status = 1 and bm.availability = 1  and bna.status = 1 and bna.availability = 1\r\n" + //
				"union\r\n" + //
				"select \r\n" + //
				"\tdistinct pacr1.id, pacr1.amount, pacr1.branch_code, pacr1.additional_information, pacr1.value_date from pas_awb_core_reversal pacr1, pas_awb_core_reversal pacr2 \r\n" + //
	...
	List<File_rtgs_awb_core> reportCoreDebit(@Param("date") String date);

	// 	@Select("select distinct dac.id, dac.amount, dac.branch_code, dac.additional_information, dac.value_date from data_awb_core dac where upload_date <= #{date} and dr_cr = 'DR' and dac.status = 1 and dac.availability = 1 union "
	// 		+ "select distinct dac.id, dac.amount, dac.branch_code, dac.additional_information, dac.value_date from core_partially_matched dac where upload_date <= #{date} and dr_cr = 'DR' and dac.status = 1 and dac.availability = 1 union "
	// 		+ "select distinct rac.id, rac.amount, rac.branch_code, rac.additional_information, rac.value_date from rtgs_awb_core rac, rtgs_matched rm, rtgs_nbe_ats rna "
	// 		+ "where rac.dr_cr = 'DR' and rac.upload_date <= #{date} and (rna.id = rm.rtgs_nbe_ats_id and rac.match_id = rm.match_id and rna.upload_date > #{date}) and rac.status = 1 and rac.availability = 1  "
	// 		+ " and rm.status = 1 and rm.availability = 1  and rna.status = 1 and rna.availability = 1  union "
	// 		+ "select distinct eac.id, eac.amount, eac.branch_code, eac.additional_information, eac.value_date from erca_awb_core eac, erca_matched em, erca_nbe_ats ena "
	// 		+ "where eac.dr_cr = 'DR' and eac.upload_date <= #{date} and ((ena.id = em.erca_nbe_ats_id and eac.match_id = em.match_id and ena.upload_date > #{date})) and eac.status = 1 and eac.availability = 1  "
	// 		+ " and em.status = 1 and em.availability = 1  and ena.status = 1 and ena.availability = 1 union "
	// 		+ "select distinct sac.id, sac.amount, sac.branch_code, sac.additional_information, sac.value_date from sos_awb_core sac, sos_matched sm, sos_nbe_ats sna "
	// 		+ "where sac.dr_cr = 'DR' and sac.upload_date <= #{date} and ((sna.id = sm.sos_nbe_ats_id and sac.match_id = sm.match_id and sna.upload_date > #{date}))and sac.additional_information != 'ECX Interbank' and sac.status = 1 and sac.availability = 1  "
	// 		+ " and sm.status = 1 and sm.availability = 1  and sna.status = 1 and sna.availability = 1 union "
	// 		+ "select distinct sac.id, sac.amount, sac.branch_code, sac.additional_information, sac.value_date from sos_awb_core sac, sos_matched sm, sos_nbe_ats sna "
	// 		+ "where sac.dr_cr = 'DR' and sac.upload_date <= #{date} and ((sna.id = sm.sos_nbe_ats_id and sac.match_id = sm.match_id and sna.upload_date > #{date})) and sac.additional_information = 'ECX Interbank' and sac.upload_date < sna.upload_date and sac.status = 1 and sac.availability = 1  "
	// 		+ " and sm.status = 1 and sm.availability = 1  and sna.status = 1 and sna.availability = 1 union "
	// 		+ "select distinct bac.id, bac.amount, bac.branch_code, bac.additional_information, bac.value_date from b2b_awb_core bac, b2b_matched bm, b2b_nbe_ats bna "
	// 		+ "where bac.dr_cr = 'DR' and bac.upload_date <= #{date} and ((bna.id = bm.b2b_nbe_ats_id and bac.match_id = bm.match_id and bna.upload_date > #{date})) and bac.status = 1 and bac.availability = 1  "
	// 		+ " and bm.status = 1 and bm.availability = 1  and bna.status = 1 and bna.availability = 1")
	// List<File_rtgs_awb_core> reportCoreDebit(@Param("date") String date);
	...
	List<File_rtgs_awb_core> reportPayableDebit(@Param("date") String date);
	...
	List<File_rtgs_awb_core> reportPayableCredit(@Param("date") String date);
...
//			+ "")
//	List<File_rtgs_awb_core> PayableRawData(@Param("date") String date);
	...
	// @Select("SELECT (\r\n" + //
	// 			"    (SELECT ISNULL(SUM(dac.amount), 0) FROM data_awb_core dac WHERE upload_date <= #{date} AND dr_cr = 'DR' AND dac.status = 1 AND dac.availability = 1) +\r\n" + //
	// 			"    (SELECT ISNULL(SUM(dac.amount), 0) FROM core_partially_matched dac WHERE upload_date <= #{date} AND dr_cr = 'DR' AND dac.status = 1 AND dac.availability = 1) +\r\n" + //
	// 			"    (\r\n" + //
	// 			"        SELECT ISNULL(SUM(rac.amount), 0) FROM rtgs_awb_core rac, rtgs_matched rm, rtgs_nbe_ats rna\r\n" + //
	// 			"        WHERE rac.dr_cr = 'DR' AND rac.upload_date <= #{date} AND ((rna.id = rm.rtgs_nbe_ats_id AND rac.match_id = rm.match_id AND rna.upload_date > #{date}))\r\n" + //
	// 			"        AND rac.status = 1 AND rac.availability = 1 AND rm.status = 1 AND rm.availability = 1 AND rna.status = 1 AND rna.availability = 1\r\n" + //
	// 			"    ) +\r\n" + //
	// 			"    (\r\n" + //
	// 			"        SELECT ISNULL(SUM(eac.amount), 0) FROM erca_awb_core eac, erca_matched em, erca_nbe_ats ena\r\n" + //
	// 			"        WHERE eac.dr_cr = 'DR' AND eac.upload_date <= #{date} AND ((ena.id = em.erca_nbe_ats_id AND eac.match_id = em.match_id AND ena.upload_date > #{date}))\r\n" + //
	// 			"        AND eac.status = 1 AND eac.availability = 1 AND em.status = 1 AND em.availability = 1 AND ena.status = 1 AND ena.availability = 1\r\n" + //
	// 			"    ) +\r\n" + //
	// 			"    (\r\n" + //
	// 			"        SELECT ISNULL(SUM(sac.amount), 0) FROM sos_awb_core sac, sos_matched sm, sos_nbe_ats sna\r\n" + //
	// 			"        WHERE sac.dr_cr = 'DR' AND sac.upload_date <= #{date} AND ((sna.id = sm.sos_nbe_ats_id AND sac.match_id = sm.match_id AND sna.upload_date >= #{date}))\r\n" + //
	// 			"        AND sac.additional_information != 'ECX Interbank' AND sac.status = 1 AND sac.availability = 1 AND sm.status = 1 AND sm.availability = 1 AND sna.status = 1 AND sna.availability = 1\r\n" + //
	// 			"    ) +\r\n" + //
	// 			"    (\r\n" + //
	// 			"        SELECT ISNULL(SUM(sac.amount), 0) FROM sos_awb_core sac, sos_matched sm, sos_nbe_ats sna\r\n" + //
	// 			"        WHERE sac.dr_cr = 'DR' AND sac.upload_date <= #{date} AND ((sna.id = sm.sos_nbe_ats_id AND sac.match_id = sm.match_id AND sna.upload_date >= #{date}))\r\n" + //
	// 			"        AND sac.additional_information = 'ECX Interbank' AND sna.upload_date > sac.upload_date AND sac.status = 1 AND sac.availability = 1 AND sm.status = 1 AND sm.availability = 1 AND sna.status = 1 AND sna.availability = 1\r\n" + //
	// 			"    ) +\r\n" + //
	// 			"    (\r\n" + //
	// 			"        SELECT ISNULL(SUM(bac.amount), 0) FROM b2b_awb_core bac, b2b_matched bm, b2b_nbe_ats bna\r\n" + //
	// 			"        WHERE bac.dr_cr = 'DR' AND bac.upload_date <= #{date} AND ((bna.id = bm.b2b_nbe_ats_id AND bac.match_id = bm.match_id AND bna.upload_date > #{date})) \r\n" + //
	// 			"\t\tAND bac.status = 1 AND bac.availability = 1 AND bm.status = 1 AND bm.availability = 1 AND bna.status = 1 AND bna.availability = 1\r\n" + //
	// 			"    ) +\r\n" + //
	// 			"    (\r\n" + //
	// 			"       select ISNULL(SUM(pacr1.amount), 0) from pas_awb_core_reversal pacr1, pas_awb_core_reversal pacr2 \r\n" + //
	...
	// 			"    )\r\n" + //
	// 			") AS total_amount;")
	// Double getCoreTotalDebit(@Param("date") String string);
	@Select("WITH CombinedResult AS (\r\n" + //
				"\tSELECT sac.* FROM sos_awb_core sac, sos_matched sm, sos_nbe_ats sna\r\n" + //
				"\tWHERE sac.dr_cr = 'DR' AND sac.upload_date <= #{date} AND ((sna.id = sm.sos_nbe_ats_id AND sac.match_id = sm.match_id AND sna.upload_date > #{date}))\r\n" + //
				"\tAND sac.additional_information != 'ECX Interbank' AND sac.status = 1 AND sac.availability = 1 AND sm.status = 1 AND sm.availability = 1 AND sna.status = 1 AND sna.availability = 1\r\n" + //
				"\tunion\r\n" + //
				"\tselect sac.* from sos_awb_core sac, sos_awb_core sac2 where sac.dr_cr = 'DR'  AND sac.upload_date <= #{date} and sac2.upload_date > #{date} and sac.match_id = sac2.match_id\r\n" + //
				"\tand sac.status = 1 and sac.availability = 1 and sac2.status = 1 and sac2.availability = 1 and sac.additional_information != 'ECX Interbank' and sac2.additional_information != 'ECX Interbank'\r\n" + //
				")\r\n" + //
				"SELECT (\r\n" + //
				"\t\t(SELECT ISNULL(SUM(dac.amount), 0) FROM data_awb_core dac WHERE upload_date <= #{date} AND dr_cr = 'DR' AND dac.status = 1 AND dac.availability = 1) +\r\n" + //
				"\t\t(SELECT ISNULL(SUM(dac.amount), 0) FROM core_partially_matched dac WHERE upload_date <= #{date} AND dr_cr = 'DR' AND dac.status = 1 AND dac.availability = 1) +\r\n" + //
				...
				+ "FROM pas_awb_core_reversal pacr1\r\n"
				+ "INNER JOIN (\r\n"
				+ "SELECT DISTINCT match_id, status ,availability\r\n"
				+ "FROM pas_awb_core_reversal\r\n"
	...
	// @Select("select ((select ISNULL(sum(dac.amount), 0) from data_awb_core dac where upload_date <= #{date} and dr_cr = 'DR' and dac.status = 1 and dac.availability = 1 ) "
	// 		+ " + (select ISNULL(sum(dac.amount), 0) from core_partially_matched dac where upload_date <= #{date} and dr_cr = 'DR' and dac.status = 1 and dac.availability = 1 ) "
	// 		+ " + ( "
	// 		+ " select ISNULL(sum(rac.amount), 0) from rtgs_awb_core rac, rtgs_matched rm, rtgs_nbe_ats rna where rac.dr_cr = 'DR' and rac.upload_date <= #{date}  "
	// 		+ " and ((rna.id = rm.rtgs_nbe_ats_id and rac.match_id = rm.match_id and rna.upload_date > #{date})) and rac.status = 1 and rac.availability = 1  "
	// 		+ "  and rm.status = 1 and rm.availability = 1  and rna.status = 1 and rna.availability = 1) " + " + ( "
	// 		+ " select ISNULL(sum(eac.amount), 0) from erca_awb_core eac, erca_matched em, erca_nbe_ats ena where eac.dr_cr = 'DR' and eac.upload_date <= #{date} "
	// 		+ "  and ((ena.id = em.erca_nbe_ats_id and eac.match_id = em.match_id and ena.upload_date > #{date})) and eac.status = 1 and eac.availability = 1  "
	// 		+ "  and em.status = 1 and em.availability = 1  and ena.status = 1 and ena.availability = 1 ) " + " + ( "
	// 		+ " select ISNULL(sum(sac.amount), 0) from sos_awb_core sac, sos_matched sm, sos_nbe_ats sna where sac.dr_cr = 'DR' and sac.upload_date <= #{date} "
	// 		+ "  and ((sna.id = sm.sos_nbe_ats_id and sac.match_id = sm.match_id and sna.upload_date > #{date})) and sac.additional_information != 'ECX Interbank' and sac.status = 1 and sac.availability = 1  "
	// 		+ "  and sm.status = 1 and sm.availability = 1  and sna.status = 1 and sna.availability = 1) " + " + ( "
	// 		+ " select ISNULL(sum(sac.amount), 0) from sos_awb_core sac, sos_matched sm, sos_nbe_ats sna where sac.dr_cr = 'DR' and sac.upload_date <= #{date} "
	// 		+ "  and ((sna.id = sm.sos_nbe_ats_id and sac.match_id = sm.match_id and sna.upload_date > #{date})) and sac.additional_information = 'ECX Interbank' and sna.upload_date > sac.upload_date and sac.status = 1 and sac.availability = 1  "
	// 		+ "  and sm.status = 1 and sm.availability = 1  and sna.status = 1 and sna.availability = 1) " + " + ( "
	// 		+ " select ISNULL(sum(bac.amount), 0) from b2b_awb_core bac, b2b_matched bm, b2b_nbe_ats bna where bac.dr_cr = 'DR' and bac.upload_date <= #{date}  "
	// 		+ " and ((bna.id = bm.b2b_nbe_ats_id and bac.match_id = bm.match_id and bna.upload_date > #{date})) and bac.status = 1 and bac.availability = 1  "
	// 		+ "  and bm.status = 1 and bm.availability = 1  and bna.status = 1 and bna.availability = 1) " + " )")
	// Double getCoreTotalDebit(@Param("date") String string);

	@Select("select \r\n" + //
				"\tdistinct dac.id, dac.amount, dac.branch_code, dac.additional_information, dac.value_date from data_awb_core dac where upload_date <= #{date} and dr_cr = 'CR' and dac.status = 1 and dac.availability = 1 \r\n" + //
				"union \r\n" + //
				"select \r\n" + //
				"\tdistinct dac.id, dac.amount, dac.branch_code, dac.additional_information, dac.value_date from core_partially_matched dac where upload_date <= #{date} and dr_cr = 'CR' and dac.status = 1 and dac.availability = 1 \r\n" + //
				"union \r\n" + //
				"select \r\n" + //
				"\tdistinct rac.id, rac.amount, rac.branch_code, rac.additional_information, rac.value_date from rtgs_awb_core rac, rtgs_matched rm, rtgs_nbe_ats rna \r\n" + //
				"\t\twhere rac.dr_cr = 'CR' and rac.upload_date <= #{date} and (rna.id = rm.rtgs_nbe_ats_id and rac.match_id = rm.match_id and rna.upload_date > #{date}) and rac.status = 1 and rac.availability = 1  \r\n" + //
				"\t\tand rm.status = 1 and rm.availability = 1  and rna.status = 1 and rna.availability = 1  \r\n" + //
				"union \r\n" + //
				"select \r\n" + //
				"\tdistinct eac.id, eac.amount, eac.branch_code, eac.additional_information, eac.value_date from erca_awb_core eac, erca_matched em, erca_nbe_ats ena \r\n" + //
				"\t\twhere eac.dr_cr = 'CR' and eac.upload_date <= #{date} and ((ena.id = em.erca_nbe_ats_id and eac.match_id = em.match_id and ena.upload_date > #{date})) and eac.status = 1 and eac.availability = 1  \r\n" + //
				"\t\tand em.status = 1 and em.availability = 1  and ena.status = 1 and ena.availability = 1\r\n" + //
				"union \r\n" + //
				"select \r\n" + //
				"\tdistinct sac.id, sac.amount, sac.branch_code, sac.additional_information, sac.value_date from sos_awb_core sac, sos_matched sm, sos_nbe_ats sna \r\n" + //
				...
				"select \r\n" + //
				"\tdistinct sac.id, sac.amount, sac.branch_code, sac.additional_information, sac.value_date from sos_awb_core sac, sos_matched sm, sos_nbe_ats sna \r\n" + //
				"\t\twhere sac.dr_cr = 'CR' and sac.upload_date <= #{date} and ((sna.id = sm.sos_nbe_ats_id and sac.match_id = sm.match_id and sna.upload_date >= #{date})) and sac.additional_information = 'ECX Interbank' and sac.upload_date < sna.upload_date and sac.status = 1 and sac.availability = 1  \r\n" + //
				"\t\tand sm.status = 1 and sm.availability = 1  and sna.status = 1 and sna.availability = 1 union \r\n" + //
				"select \r\n" + //
				"\tdistinct bac.id, bac.amount, bac.branch_code, bac.additional_information, bac.value_date from b2b_awb_core bac, b2b_matched bm, b2b_nbe_ats bna \r\n" + //
				"\t\twhere bac.dr_cr = 'CR' and bac.upload_date <= #{date} and ((bna.id = bm.b2b_nbe_ats_id and bac.match_id = bm.match_id and bna.upload_date > #{date})) and bac.status = 1 and bac.availability = 1  \r\n" + //
				"\t\tand bm.status = 1 and bm.availability = 1  and bna.status = 1 and bna.availability = 1\r\n" + //
				"union\r\n" + //
				"select \r\n" + //
				"\tdistinct pacr1.id, pacr1.amount, pacr1.branch_code, pacr1.additional_information, pacr1.value_date from pas_awb_core_reversal pacr1, pas_awb_core_reversal pacr2 \r\n" + //
				...
				"\t\tAND pacr1.status = 1 AND pacr1.availability = 1 AND pacr2.status = 1 AND pacr2.availability = 1;")
	List<File_rtgs_awb_core> reportCoreCredit(@Param("date") String string);

	// 	@Select("select distinct dac.id, dac.amount, dac.branch_code, dac.additional_information, dac.value_date from data_awb_core dac where upload_date <= #{date} and dr_cr = 'CR' and dac.status = 1 and dac.availability = 1 union "
	// 		+ "select distinct dac.id, dac.amount, dac.branch_code, dac.additional_information, dac.value_date from core_partially_matched dac where upload_date <= #{date} and dr_cr = 'CR' and dac.status = 1 and dac.availability = 1 union "
	// 		+ "select distinct rac.id, rac.amount, rac.branch_code, rac.additional_information, rac.value_date from rtgs_awb_core rac, rtgs_matched rm, rtgs_nbe_ats rna "
	// 		+ "where rac.dr_cr = 'CR' and rac.upload_date <= #{date} and (rna.id = rm.rtgs_nbe_ats_id and rac.match_id = rm.match_id and rna.upload_date > #{date}) and rac.status = 1 and rac.availability = 1  "
	// 		+ " and rm.status = 1 and rm.availability = 1  and rna.status = 1 and rna.availability = 1  union "
	// 		+ "select distinct eac.id, eac.amount, eac.branch_code, eac.additional_information, eac.value_date from erca_awb_core eac, erca_matched em, erca_nbe_ats ena "
	...
	// @Select("\r\n" + //
	// 			"SELECT (\r\n" + //
	// 			"    (SELECT ISNULL(SUM(dac.amount), 0) FROM data_awb_core dac WHERE upload_date <= #{date} AND dr_cr = 'CR' AND dac.status = 1 AND dac.availability = 1) +\r\n" + //
	// 			"    (SELECT ISNULL(SUM(dac.amount), 0) FROM core_partially_matched dac WHERE upload_date <= #{date} AND dr_cr = 'CR' AND dac.status = 1 AND dac.availability = 1) +\r\n" + //
	// 			"    (\r\n" + //
	// 			"        SELECT ISNULL(SUM(rac.amount), 0) FROM rtgs_awb_core rac, rtgs_matched rm, rtgs_nbe_ats rna\r\n" + //
	// 			"        WHERE rac.dr_cr = 'CR' AND rac.upload_date <= #{date} AND ((rna.id = rm.rtgs_nbe_ats_id AND rac.match_id = rm.match_id AND rna.upload_date > #{date}))\r\n" + //
	// 			"        AND rac.status = 1 AND rac.availability = 1 AND rm.status = 1 AND rm.availability = 1 AND rna.status = 1 AND rna.availability = 1\r\n" + //
	// 			"    ) +\r\n" + //
	// 			"    (\r\n" + //
	// 			"        SELECT ISNULL(SUM(eac.amount), 0) FROM erca_awb_core eac, erca_matched em, erca_nbe_ats ena\r\n" + //
	// 			"        WHERE eac.dr_cr = 'CR' AND eac.upload_date <= #{date} AND ((ena.id = em.erca_nbe_ats_id AND eac.match_id = em.match_id AND ena.upload_date > #{date}))\r\n" + //
	// 			"        AND eac.status = 1 AND eac.availability = 1 AND em.status = 1 AND em.availability = 1 AND ena.status = 1 AND ena.availability = 1\r\n" + //
	// 			"    ) +\r\n" + //
	// 			"    (\r\n" + //
	// 			"        SELECT ISNULL(SUM(sac.amount), 0) FROM sos_awb_core sac, sos_matched sm, sos_nbe_ats sna\r\n" + //
	// 			"        WHERE sac.dr_cr = 'CR' AND sac.upload_date <= #{date} AND ((sna.id = sm.sos_nbe_ats_id AND sac.match_id = sm.match_id AND sna.upload_date >= #{date}))\r\n" + //
	// 			"        AND sac.additional_information != 'ECX Interbank' AND sac.status = 1 AND sac.availability = 1 AND sm.status = 1 AND sm.availability = 1 AND sna.status = 1 AND sna.availability = 1\r\n" + //
	// 			"    ) +\r\n" + //
	// 			"    (\r\n" + //
	// 			"        SELECT ISNULL(SUM(sac.amount), 0) FROM sos_awb_core sac, sos_matched sm, sos_nbe_ats sna\r\n" + //
	...
	// 			"    (\r\n" + //
	// 			"        SELECT ISNULL(SUM(bac.amount), 0) FROM b2b_awb_core bac, b2b_matched bm, b2b_nbe_ats bna\r\n" + //
	// 			"        WHERE bac.dr_cr = 'CR' AND bac.upload_date <= #{date} AND ((bna.id = bm.b2b_nbe_ats_id AND bac.match_id = bm.match_id AND bna.upload_date > #{date})) \r\n" + //
	// 			"\t\tAND bac.status = 1 AND bac.availability = 1 AND bm.status = 1 AND bm.availability = 1 AND bna.status = 1 AND bna.availability = 1\r\n" + //
	// 			"    ) +\r\n" + //
	// 			"    (\r\n" + //
	// 			"       select ISNULL(SUM(pacr1.amount), 0) from pas_awb_core_reversal pacr1, pas_awb_core_reversal pacr2 \r\n" + //
	...
	@Select("WITH CombinedResult AS (\r\n" + //
				...
				"\tAND sac.additional_information != 'ECX Interbank' AND sac.status = 1 AND sac.availability = 1 AND sm.status = 1 AND sm.availability = 1 AND sna.status = 1 AND sna.availability = 1\r\n" + //
				"\tunion\r\n" + //
				"\tselect sac.* from sos_awb_core sac, sos_awb_core sac2 where sac.dr_cr = 'CR'  AND sac.upload_date <= #{date} and sac2.upload_date > #{date} and sac.match_id = sac2.match_id\r\n" + //
				"\tand sac.status = 1 and sac.availability = 1 and sac2.status = 1 and sac2.availability = 1 and sac.additional_information != 'ECX Interbank' and sac2.additional_information != 'ECX Interbank'\r\n" + //
				")\r\n" + //
				"SELECT (\r\n" + //
				"\t\t(SELECT ISNULL(SUM(dac.amount), 0) FROM data_awb_core dac WHERE upload_date <= #{date} AND dr_cr = 'CR' AND dac.status = 1 AND dac.availability = 1) +\r\n" + //
				...
				"\t\t)\r\n" + //
				") AS total_amount;")
	Double getCoreTotalCredit(@Param("date") String string);

	// @Select("select ((select ISNULL(sum(dac.amount), 0) from data_awb_core dac where upload_date <= #{date} and dr_cr = 'CR' and dac.status = 1 and dac.availability = 1 ) "
	// 		+ " + (select ISNULL(sum(dac.amount), 0) from core_partially_matched dac where upload_date <= #{date} and dr_cr = 'CR' and dac.status = 1 and dac.availability = 1 ) "
	// 		+ " + ( "
	// 		+ " select ISNULL(sum(rac.amount), 0) from rtgs_awb_core rac, rtgs_matched rm, rtgs_nbe_ats rna where rac.dr_cr = 'CR' and rac.upload_date <= #{date}  "
	// 		+ " and ((rna.id = rm.rtgs_nbe_ats_id and rac.match_id = rm.match_id and rna.upload_date > #{date})) and rac.status = 1 and rac.availability = 1  "
	// 		+ "  and rm.status = 1 and rm.availability = 1  and rna.status = 1 and rna.availability = 1) " + " + ( "
	// 		+ " select ISNULL(sum(eac.amount), 0) from erca_awb_core eac, erca_matched em, erca_nbe_ats ena where eac.dr_cr = 'CR' and eac.upload_date <= #{date} "
	// 		+ "  and ((ena.id = em.erca_nbe_ats_id and eac.match_id = em.match_id and ena.upload_date > #{date})) and eac.status = 1 and eac.availability = 1  "
	// 		+ "  and em.status = 1 and em.availability = 1  and ena.status = 1 and ena.availability = 1 ) " + " + ( "
	// 		+ " select ISNULL(sum(sac.amount), 0) from sos_awb_core sac, sos_matched sm, sos_nbe_ats sna where sac.dr_cr = 'CR' and sac.upload_date <= #{date} "
	// 		+ "  and ((sna.id = sm.sos_nbe_ats_id and sac.match_id = sm.match_id and sna.upload_date > #{date})) and sac.additional_information != 'ECX Interbank' and sac.status = 1 and sac.availability = 1  "
	// 		+ "  and sm.status = 1 and sm.availability = 1  and sna.status = 1 and sna.availability = 1) " + " + ( "
	// 		+ " select ISNULL(sum(sac.amount), 0) from sos_awb_core sac, sos_matched sm, sos_nbe_ats sna where sac.dr_cr = 'CR' and sac.upload_date <= #{date} "
	// 		+ "  and ((sna.id = sm.sos_nbe_ats_id and sac.match_id = sm.match_id and sna.upload_date > #{date})) and sac.additional_information = 'ECX Interbank' and sna.upload_date > sac.upload_date and sac.status = 1 and sac.availability = 1  "
	// 		+ "  and sm.status = 1 and sm.availability = 1  and sna.status = 1 and sna.availability = 1) " + " + ( "
	// 		+ " select ISNULL(sum(bac.amount), 0) from b2b_awb_core bac, b2b_matched bm, b2b_nbe_ats bna where bac.dr_cr = 'CR' and bac.upload_date <= #{date}  "
	// 		+ " and ((bna.id = bm.b2b_nbe_ats_id and bac.match_id = bm.match_id and bna.upload_date > #{date})) and bac.status = 1 and bac.availability = 1  "
	// 		+ "  and bm.status = 1 and bm.availability = 1  and bna.status = 1 and bna.availability = 1) " + " )")
	// Double getCoreTotalCredit(@Param("date") String string);

	@Select("select ISNULL(sum(ending_balance_con), 0) from files where upload_date = #{date} and type = 'CORE'  and status = 1 and availability = 1")
	Double getConventionalEndingBalance(@Param("date") String string);

	@Select("select ISNULL(sum(ending_balance_ifb), 0) from files where upload_date = #{date} and type = 'CORE' and status = 1 and availability = 1")
	Double getIfbEndingBalance(@Param("date") String string);

	@Select("select distinct dna.id, dna.value_date_type, dna.reference, dna.sender, dna.receiver, dna.amount from data_nbe_ats dna where upload_date <= #{date} and dr_cr = 'CR' and status = 1 and availability = 1 union "
			...
			+ "select distinct rna.id, rna.value_date_type, rna.reference, rna.sender, rna.receiver, rna.amount from rtgs_nbe_ats rna, rtgs_matched rm, rtgs_awb_core rac "
			+ "where rna.dr_cr = 'CR' and rna.upload_date <= #{date} and rna.id = rm.rtgs_nbe_ats_id and rm.match_id = rac.match_id and rac.upload_date > #{date}  "
			...
			+ "select distinct ena.id, ena.value_date_type, ena.reference, ena.sender, ena.receiver, ena.amount from erca_nbe_ats ena, erca_matched em, erca_awb_core eac "
			+ "where ena.dr_cr = 'CR' and ena.upload_date <= #{date} and ena.id = em.erca_nbe_ats_id and em.match_id = eac.match_id and eac.upload_date > #{date} "
			...
			+ "select distinct sna.id, sna.value_date_type, sna.reference, sna.sender, sna.receiver, sna.amount from sos_nbe_ats sna, sos_matched sm, sos_awb_core sac "
			+ "where sna.dr_cr = 'CR' and sna.upload_date <= #{date} and sna.id = sm.sos_nbe_ats_id and sm.match_id = sac.match_id and sac.upload_date > #{date} "
			...
			+ "select distinct bna.id, bna.value_date_type, bna.reference, bna.sender, bna.receiver, bna.amount from b2b_nbe_ats bna, b2b_matched bm, b2b_awb_core bac "
			+ "where bna.dr_cr = 'CR' and bna.upload_date <= #{date} and bna.id = bm.b2b_nbe_ats_id and bm.match_id = bac.match_id and bac.upload_date > #{date}  "
			+ "and bna.status = 1 and bna.availability = 1 and bm.status = 1 and bm.availability = 1  and bac.status = 1 and bac.availability = 1")
	List<File_rtgs_nbe_ats> reportAtsCredit(@Param("date") String string);

	@Select("select distinct dna.id, dna.value_date_type, dna.reference, dna.sender, dna.receiver, dna.amount from data_nbe_ats dna where upload_date <= #{date} and dr_cr = 'DR' and status = 1 and availability = 1 union "
			...
			+ "select distinct rna.id, rna.value_date_type, rna.reference, rna.sender, rna.receiver, rna.amount from rtgs_nbe_ats rna, rtgs_matched rm, rtgs_awb_core rac "
			+ "where rna.dr_cr = 'DR' and rna.upload_date <= #{date} and rna.id = rm.rtgs_nbe_ats_id and rm.match_id = rac.match_id and rac.upload_date > #{date}  "
			+ "and rna.status = 1 and rna.availability = 1 and rm.status = 1 and rm.availability = 1  and rac.status = 1 and rac.availability = 1 union "
			+ "select distinct ena.id, ena.value_date_type, ena.reference, ena.sender, ena.receiver, ena.amount from erca_nbe_ats ena, erca_matched em, erca_awb_core eac "
			+ "where ena.dr_cr = 'DR' and ena.upload_date <= #{date} and ena.id = em.erca_nbe_ats_id and em.match_id = eac.match_id and eac.upload_date > #{date} "
			+ "and ena.status = 1 and ena.availability = 1 and em.status = 1 and em.availability = 1  and eac.status = 1 and eac.availability = 1 union "
			+ "select distinct sna.id, sna.value_date_type, sna.reference, sna.sender, sna.receiver, sna.amount from sos_nbe_ats sna, sos_matched sm, sos_awb_core sac "
			+ "where sna.dr_cr = 'DR' and sna.upload_date <= #{date} and sna.id = sm.sos_nbe_ats_id and sm.match_id = sac.match_id and sac.upload_date > #{date} "
			+ "and sna.status = 1 and sna.availability = 1 and sm.status = 1 and sm.availability = 1  and sac.status = 1 and sac.availability = 1 union "
			+ "select distinct bna.id, bna.value_date_type, bna.reference, bna.sender, bna.receiver, bna.amount from b2b_nbe_ats bna, b2b_matched bm, b2b_awb_core bac "
			+ "where bna.dr_cr = 'DR' and bna.upload_date <= #{date} and bna.id = bm.b2b_nbe_ats_id and bm.match_id = bac.match_id and bac.upload_date > #{date}  "
			+ "and bna.status = 1 and bna.availability = 1 and bm.status = 1 and bm.availability = 1  and bac.status = 1 and bac.availability = 1")
	List<File_rtgs_nbe_ats> reportAtsDebit(@Param("date") String string);

	// @Select("select ((select ISNULL(sum(dna.amount), 0) from data_nbe_ats dna where upload_date <= #{date} and dr_cr = 'CR' and status = 1 and availability = 1)  +  "
	...
	// 			"\t\tand sna.status = 1 and sna.availability = 1 and sm.status = 1 and sm.availability = 1  and sac.status = 1 and sac.availability = 1) \r\n" + //
	// 			"\t\t\t \r\n" + //
	// 			"\t\t+ (select ISNULL(sum(distinct bna.amount), 0) from b2b_nbe_ats bna, b2b_nbe_ats bna2, b2b_matched bm, b2b_matched bm2\r\n" + //
	// 			"\t\twhere bna.dr_cr = 'CR' and bna.upload_date <= #{date} and \r\n" + //
	// 			"\t\t(bna.id = bm.b2b_nbe_ats_id and bna2.id = bm2.b2b_nbe_ats_id and bm.match_id = bm2.match_id and bna2.value_date_type like '%202' and bna2.upload_date > #{date} and bna2.dr_cr = 'dr' and bna2.amount = (bna.amount - 50))\r\n" + //
	// 			"\t\tand bna.status = 1 and bna.availability = 1 and bm.status = 1 and bm.availability = 1 ) \r\n" + //
	// 			"\t)")
	@Select("WITH CombinedResult AS (\r\n" + //
				"    SELECT bna.* \r\n" + //
				"    FROM b2b_nbe_ats bna, b2b_matched bm, b2b_awb_core bac \r\n" + //
				"    WHERE bna.dr_cr = 'CR' \r\n" + //
				"        AND bna.upload_date <= #{date} \r\n" + //
				"        AND (bna.id = bm.b2b_nbe_ats_id AND bm.match_id = bac.match_id AND bac.upload_date > #{date})\r\n" + //
				"        AND bna.status = 1 \r\n" + //
				...
				"\r\n" + //
				"    SELECT bna.* \r\n" + //
				"    FROM b2b_nbe_ats bna, b2b_nbe_ats bna2, b2b_matched bm, b2b_matched bm2\r\n" + //
				"    WHERE bna.dr_cr = 'CR' \r\n" + //
				"        AND bna.upload_date <= #{date} \r\n" + //
				"        AND (bna.id = bm.b2b_nbe_ats_id \r\n" + //
				"            AND bna2.id = bm2.b2b_nbe_ats_id \r\n" + //
				...
				")\r\n" + //
				"\r\n" + //
				"select ((select ISNULL(sum(dna.amount), 0) from data_nbe_ats dna where upload_date <= #{date} and dr_cr = 'CR' and status = 1 and availability = 1)  +   \r\n" + //
				"\t\t\t \r\n" + //
				"\t\t(select ISNULL(sum(dna.amount), 0) from ats_partially_matched dna where upload_date <= #{date} and dr_cr = 'CR' and status = 1 and availability = 1)  \r\n" + //
				"\t\t\t \r\n" + //
				"\t\t+ (   select ISNULL(sum(rna.amount), 0) from rtgs_nbe_ats rna, rtgs_matched rm, rtgs_awb_core rac \r\n" + //
	...
	// 			"FROM (\r\n" +
	// 			"  SELECT bna.amount\r\n" +
	// 			"  FROM b2b_nbe_ats bna\r\n" +
	// 			"  JOIN b2b_matched bm ON bna.id = bm.b2b_nbe_ats_id\r\n" +
	// 			"  JOIN b2b_awb_core bac ON bm.match_id = bac.match_id\r\n" +
	// 			"  WHERE bna.dr_cr = 'CR'\r\n" +
	// 			"    AND bna.upload_date <= #{date}\r\n" +
	// 			"    AND bna.status = 1\r\n" +
	// 			"    AND bna.availability = 1\r\n" +
	...
	// 			"  JOIN b2b_nbe_ats bna2 ON bm2.b2b_nbe_ats_id = bna2.id\r\n" +
	// 			"  WHERE bna.dr_cr = 'CR'\r\n" +
	// 			"    AND bna.upload_date <= #{date}\r\n" +
	// 			"    AND bna.status = 1\r\n" +
	// 			"    AND bna.availability = 1\r\n" +
	...
	@Select("select ((select ISNULL(sum(dna.amount), 0) from data_nbe_ats dna where upload_date <= #{date} and dr_cr = 'DR' and status = 1 and availability = 1)  + "
	...
	@Select("IF EXISTS (SELECT * FROM files WHERE upload_date = #{date} and type = 'CORE' ) " + "    SELECT 'True' "
			+ "ELSE " + "    SELECT 'False'")
	...
	@Select("IF EXISTS (SELECT * FROM files WHERE upload_date = #{date} and type = 'ISSUE_CORE' ) "
			+ "    SELECT 'True' " + "ELSE " + "    SELECT 'False'")
	// @Select("SELECT EXISTS(SELECT * FROM files WHERE upload_date = #{date} and
	// type = 'CORE')")
	...
	@Select("	 SELECT CASE \r\n"
			...
			+ "	 WHEN EXISTS(SELECT 1 FROM  data_fixed_core dfc WHERE dfc.transaction_date <= cast(#{date} as date)) THEN 'True'\r\n"
			+ "	 WHEN EXISTS(SELECT 1 FROM  fixed_mms fm WHERE cast(fm.created_date as date)<= cast(#{date} as date)) THEN 'True'\r\n"
			+ "	 WHEN EXISTS(SELECT 1 FROM  fixed_core fc WHERE fc.transaction_date<= cast(#{date} as date)) THEN 'True'\r\n"
			+ "	 ELSE 'False'\r\n"
	 ...
	 *
	 * private Long id; private Long core_id; private String value_date_type;
	...
	// @Select("insert into b2b_matched (b2b_nbe_ats_id, match_id, match_date,
	...
	@Insert("INSERT INTO nbe_ats_edit_history (value_date_type, sender, receiver, additional_information, amount, dr_cr, upload_date,"
			+ "	match_status, status, availability, REFERENCE, file_id, edit_reason_id, new_old)(SELECT value_date_type, sender, receiver,"
			+ " additional_information, amount, dr_cr, upload_date,'1', status, availability, REFERENCE, file_id, #{edit_reason_id}, 'old' FROM data_nbe_ats where id = #{id})")
	void moveOldAtsData(@Param("id") Long id, @Param("edit_reason_id") Long edit_reason_id);

	@Insert("INSERT INTO nbe_ats_edit_history (value_date_type, sender, receiver, additional_information, amount, dr_cr, upload_date,"
			+ "	match_status, status, availability, REFERENCE, file_id, edit_reason_id, new_old)(SELECT value_date_type, sender, receiver,"
			+ " additional_information, amount, dr_cr, upload_date,'1', status, availability, REFERENCE, file_id, #{reason_id}, 'deleted' FROM data_nbe_ats where id = #{id})")
	void moveDeletedAtsData(@Param("id") Long id, @Param("reason_id") Long reason_id);

	@Insert("INSERT INTO nbe_ats_edit_history (value_date_type, sender, receiver, additional_information, amount, dr_cr, upload_date,"
			+ "	match_status, status, availability, REFERENCE, file_id, edit_reason_id, new_old)(SELECT value_date_type, sender, receiver,"
			+ " additional_information, amount, dr_cr, upload_date,'1', status, availability, REFERENCE, file_id, #{edit_reason_id}, 'new' FROM data_nbe_ats where id = #{id})")
	boolean moveEditedAtsData(@Param("id") Long id, @Param("edit_reason_id") Long edit_reason_id);

	@Select("INSERT INTO awb_core_edit_history(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, value_date, additional_information, file_id,edit_reason_id,new_old) "
			+ "SELECT posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, '1', status, "
			+ "availability, value_date, additional_information, file_id, #{edit_reason_id},'old' FROM data_awb_core where id = #{id}")
	void moveOldCoreData(@Param("id") Long id, @Param("edit_reason_id") Long edit_reason_id);
	...
	@Update("update data_payable set amount =#{amount}, value_date = #{value_date_type}, "
			+ "dr_cr = #{dr_cr}, upload_date = #{upload_date}, additional_information = #{additional_information} where id = #{id}")
	Boolean updateTransactionpayable(File_rtgs_nbe_ats edit_data);
	...
	@Select("INSERT INTO awb_core_edit_history(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, value_date, additional_information, file_id,edit_reason_id,new_old) "
			+ "SELECT posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, '1', status, "
			+ "availability, value_date, additional_information, file_id, #{reason_id},'delete' FROM data_awb_core where id = #{id}")
	void moveDeletedCoreData(@Param("id") Long id, @Param("reason_id") Long reason_id);

	@Select("INSERT INTO awb_core_edit_history(posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, value_date, additional_information, file_id,edit_reason_id,new_old) "
			+ "SELECT posting_date, transaction_reference, branch_code, amount, dr_cr, upload_date, '1', status, "
			+ "availability, value_date, additional_information, file_id, #{edit_reason_id},'new' FROM data_awb_core where id = #{id}")
	void moveEditedCoreData(@Param("id") Long id, @Param("edit_reason_id") Long edit_reason_id);

	@Select("select distinct dna.id,dna.additional_information,dna.amount,dna.dr_cr,dna.match_status,dna.REFERENCE,dna.sender,dna.receiver,dna.upload_date,dna.value_date_type,u.firstname,u.lastname  from data_nbe_ats dna,users u,file_user_date fud where  u.id =fud.user_id and dna.file_id=fud.file_id and dna.amount  between #{min_amount} and   #{max_amount}  and dna.REFERENCE like '%'+ #{reference} + '%'"
			+ " and dna.value_date_type like '%'+ #{value_date} + '%' and dna.status = 1 and dna.availability = 1 union "
			+ " select distinct rna.id,rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname from rtgs_nbe_ats rna,users u,file_user_date fud, rtgs_matched rm where  u.id =fud.user_id and rna.file_id=fud.file_id  and rna.amount between   #{min_amount}  and  #{max_amount}"
			+ "  and  rna.REFERENCE like '%'+ #{reference} + '%' and rna.value_date_type like '%'+ #{value_date}+ '%' and rm.rtgs_nbe_ats_id = rna.id and rna.availability = 1 and rna.status=1 union "
			+ " select distinct bna.id, bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna,users u,file_user_date fud, b2b_matched bm where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.amount  between  #{min_amount}   and  #{max_amount}  and bna.REFERENCE like '%'+ #{reference} + '%'"
			+ " and bna.value_date_type like '%'+ #{value_date}+ '%' and bm.b2b_nbe_ats_id = bna.id and bna.availability = 1 and bna.status=1  union "
			+ " select distinct ena.id, ena.additional_information,ena.amount,ena.dr_cr,ena.match_status,ena.REFERENCE,ena.sender,ena.receiver,ena.upload_date,ena.value_date_type,u.firstname,u.lastname  from erca_nbe_ats ena,users u,file_user_date fud, erca_matched em  where  u.id =fud.user_id and ena.file_id=fud.file_id and ena.amount  between   #{min_amount}  and  #{max_amount}  "
			+ " and ena.REFERENCE like '%'+ #{reference} + '%' and ena.value_date_type like '%'+ #{value_date}+ '%' and em.erca_nbe_ats_id = ena.id and ena.availability = 1 and ena.status=1 union "
			+ " select distinct sna.id, sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna,users u,file_user_date fud, sos_matched sm where  u.id =fud.user_id and sna.file_id=fud.file_id and sna.amount  between   #{min_amount}  and  #{max_amount} "
			+ " and sna.REFERENCE like '%'+ #{reference} + '%' and sna.value_date_type like '%'+ #{value_date}+ '%' and sm.sos_nbe_ats_id = sna.id and sna.availability = 1 and sna.status=1  union "
	...
	@Select("select distinct dna.id,dna.additional_information,dna.amount,dna.dr_cr,dna.match_status,dna.REFERENCE,dna.sender,dna.receiver,dna.upload_date,dna.value_date_type,u.firstname,u.lastname  from data_nbe_ats dna,users u,file_user_date fud where  u.id =fud.user_id and dna.file_id=fud.file_id and dna.amount  >= #{min_amount}   and dna.REFERENCE like '%'+ #{reference} + '%'"
			+ " and dna.value_date_type like '%'+ #{value_date}+ '%'  and  dna.status = 1 and dna.availability = 1 union "
			+ " select distinct rna.id,rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname from rtgs_nbe_ats rna,users u,file_user_date fud, rtgs_matched rm where  u.id =fud.user_id and rna.file_id=fud.file_id  and rna.amount >=   #{min_amount}  "
			+ "  and  rna.REFERENCE like '%'+ #{reference} + '%' and rna.value_date_type like '%'+ #{value_date}+ '%' and rm.rtgs_nbe_ats_id = rna.id and rna.availability = 1 and rna.status=1 union "
			+ " select distinct bna.id, bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna,users u,file_user_date fud, b2b_matched bm where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.amount  >=  #{min_amount}   and bna.REFERENCE like '%'+ #{reference} + '%'"
			+ " and bna.value_date_type like '%'+ #{value_date}+ '%' and bm.b2b_nbe_ats_id = bna.id and bna.availability = 1 and bna.status=1  union "
			+ " select distinct ena.id, ena.additional_information,ena.amount,ena.dr_cr,ena.match_status,ena.REFERENCE,ena.sender,ena.receiver,ena.upload_date,ena.value_date_type,u.firstname,u.lastname  from erca_nbe_ats ena,users u,file_user_date fud, erca_matched em  where  u.id =fud.user_id and ena.file_id=fud.file_id and ena.amount  >=   #{min_amount}   "
			+ " and ena.REFERENCE like '%'+ #{reference} + '%' and ena.value_date_type like '%'+ #{value_date}+ '%' and em.erca_nbe_ats_id = ena.id and ena.availability = 1 and ena.status=1 union "
			+ " select distinct sna.id, sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna,users u,file_user_date fud, sos_matched sm where  u.id =fud.user_id and sna.file_id=fud.file_id and sna.amount  >=   #{min_amount}  "
			+ " and sna.REFERENCE like '%'+ #{reference} + '%' and sna.value_date_type like '%'+ #{value_date}+ '%' and sm.sos_nbe_ats_id = sna.id and sna.availability = 1 and sna.status=1  union "
			+ " select distinct apm.id,apm.additional_information,apm.amount,apm.dr_cr,apm.match_status,apm.REFERENCE,apm.sender,apm.receiver,apm.upload_date,apm.value_date_type,u.firstname,u.lastname  from ats_partially_matched apm,users u,file_user_date fud, partially_matched pm  where  u.id =fud.user_id and apm.file_id=fud.file_id and apm.amount  >=  #{min_amount}  "
			+ " and apm.REFERENCE like '%'+ #{reference} + '%' and apm.value_date_type like '%'+ #{value_date}+ '%' and pm.ats_id = apm.id and apm.availability = 1 and apm.status=1")
	...
	@Select("select distinct dna.id,dna.additional_information,dna.amount,dna.dr_cr,dna.match_status,dna.REFERENCE,dna.sender,dna.receiver,dna.upload_date,dna.value_date_type,u.firstname,u.lastname  from data_nbe_ats dna,users u,file_user_date fud where  u.id =fud.user_id and dna.file_id=fud.file_id and dna.amount  <= #{max_amount}   and dna.REFERENCE like '%'+ #{reference} + '%'"
			+ " and dna.value_date_type like '%'+ #{value_date}+ '%' and  dna.status = 1 and dna.availability = 1  union "
			+ " select distinct rna.id,rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname from rtgs_nbe_ats rna,users u,file_user_date fud, rtgs_matched rm where  u.id =fud.user_id and rna.file_id=fud.file_id  and rna.amount <=   #{max_amount}  "
			+ "  and  rna.REFERENCE like '%'+ #{reference} + '%' and rna.value_date_type like '%'+ #{value_date}+ '%' and rm.rtgs_nbe_ats_id = rna.id and rna.availability = 1 and rna.status=1 union "
			+ " select distinct bna.id, bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna,users u,file_user_date fud, b2b_matched bm where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.amount  <=  #{max_amount}   and bna.REFERENCE like '%'+ #{reference} + '%'"
			+ " and bna.value_date_type like '%'+ #{value_date}+ '%' and bm.b2b_nbe_ats_id = bna.id and bna.availability = 1 and bna.status=1  union "
			+ " select distinct ena.id, ena.additional_information,ena.amount,ena.dr_cr,ena.match_status,ena.REFERENCE,ena.sender,ena.receiver,ena.upload_date,ena.value_date_type,u.firstname,u.lastname  from erca_nbe_ats ena,users u,file_user_date fud, erca_matched em  where  u.id =fud.user_id and ena.file_id=fud.file_id and ena.amount  <=   #{max_amount}   "
			+ " and ena.REFERENCE like '%'+ #{reference} + '%' and ena.value_date_type like '%'+ #{value_date}+ '%' and em.erca_nbe_ats_id = ena.id and ena.availability = 1 and ena.status=1 union "
			+ " select distinct sna.id, sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna,users u,file_user_date fud, sos_matched sm where  u.id =fud.user_id and sna.file_id=fud.file_id and sna.amount  <=   #{max_amount}  "
			+ " and sna.REFERENCE like '%'+ #{reference} + '%' and sna.value_date_type like '%'+ #{value_date}+ '%' and sm.sos_nbe_ats_id = sna.id and sna.availability = 1 and sna.status=1  union "
			+ " select distinct apm.id,apm.additional_information,apm.amount,apm.dr_cr,apm.match_status,apm.REFERENCE,apm.sender,apm.receiver,apm.upload_date,apm.value_date_type,u.firstname,u.lastname  from ats_partially_matched apm,users u,file_user_date fud, partially_matched pm  where  u.id =fud.user_id and apm.file_id=fud.file_id and apm.amount  <=  #{max_amount}  "
			+ " and apm.REFERENCE like '%'+ #{reference} + '%' and apm.value_date_type like '%'+ #{value_date}+ '%' and pm.ats_id = apm.id and apm.availability = 1 and apm.status=1")
	...
	@Select("select dna.id,dna.additional_information,dna.amount,dna.dr_cr,dna.match_status,dna.REFERENCE,dna.sender,dna.receiver,dna.upload_date,dna.value_date_type,u.firstname,u.lastname  from data_nbe_ats dna,users u,file_user_date fud  where  u.id =fud.user_id and dna.file_id=fud.file_id  and dna.upload_date between #{min_upload_date} and #{max_upload_date} and dna.REFERENCE like '%'+ #{reference}+ '%' and dna.value_date_type like '%'+ #{value_date}+'%'  and  dna.status = 1 and dna.availability = 1  union"
			+ " select rna.id,rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname  from rtgs_nbe_ats rna,users u,file_user_date fud, rtgs_matched rm  where  u.id =fud.user_id and rna.file_id=fud.file_id and rna.upload_date between #{min_upload_date} and #{max_upload_date} and rna.REFERENCE like '%'+ #{reference}+ '%' and rna.value_date_type like '%'+ #{value_date}+'%' and rm.rtgs_nbe_ats_id = rna.id and rna.availability = 1 and rna.status=1 union"
			+ " select bna.id,bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna,users u,file_user_date fud, b2b_matched bm  where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.upload_date between #{min_upload_date} and #{max_upload_date} and bna.REFERENCE like '%'+ #{reference}+ '%' and bna.value_date_type like '%'+ #{value_date}+'%' and bm.b2b_nbe_ats_id = bna.id and bna.availability = 1 and bna.status=1 union"
			+ " select ena.id, ena.additional_information,ena.amount,ena.dr_cr,ena.match_status,ena.REFERENCE,ena.sender,ena.receiver,ena.upload_date,ena.value_date_type,u.firstname,u.lastname  from erca_nbe_ats ena,users u,file_user_date fud, erca_matched em  where  u.id =fud.user_id and ena.file_id=fud.file_id and ena.upload_date between #{min_upload_date} and #{max_upload_date} and ena.REFERENCE like '%'+ #{reference}+ '%' and ena.value_date_type like '%'+ #{value_date}+'%' and em.erca_nbe_ats_id = ena.id and ena.availability = 1 and ena.status=1union"
			+ " select sna.id, sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna,users u,file_user_date fud, sos_matched sm  where  u.id =fud.user_id and sna.file_id=fud.file_id and sna.upload_date between #{min_upload_date} and #{max_upload_date} and sna.REFERENCE like '%'+ #{reference}+ '%' and sna.value_date_type like '%'+ #{value_date}+'%' and sm.sos_nbe_ats_id = sna.id and sna.availability = 1 and sna.status=1 union"
	...
	@Select("select rna.id, rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname  from rtgs_nbe_ats rna,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rna.file_id=fud.file_id and  rm.match_date between #{min_match_date} and #{max_match_date} and rna.REFERENCE like '%'+ #{reference}+ '%' and rna.value_date_type like '%'+ #{value_date}+'%'  and rm.rtgs_nbe_ats_id = rna.id and rna.availability = 1 and rna.status=1 union"
			+ " select bna.id,bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna,b2b_matched bm, users u,file_user_date fud  where  u.id =fud.user_id and bna.file_id=fud.file_id and  bm.match_date between #{min_match_date} and #{max_match_date}  and bna.REFERENCE like '%'+ #{reference}+ '%' and bna.value_date_type like '%'+ #{value_date}+'%'  and bm.b2b_nbe_ats_id = bna.id and bna.availability = 1 and bna.status=1 union"
			+ " select ena.id,ena.additional_information,ena.amount,ena.dr_cr,ena.match_status,ena.REFERENCE,ena.sender,ena.receiver,ena.upload_date,ena.value_date_type,u.firstname,u.lastname  from erca_nbe_ats ena,erca_matched em, users u,file_user_date fud  where  u.id =fud.user_id and ena.file_id=fud.file_id and  em.match_date between #{min_match_date} and #{max_match_date} and ena.REFERENCE like '%'+ #{reference}+ '%' and ena.value_date_type like '%'+ #{value_date}+'%'  and em.erca_nbe_ats_id = ena.id and ena.availability = 1 and ena.status=1union"
			+ " select sna.id,sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sna.file_id=fud.file_id and  sm.match_date between #{min_match_date} and #{max_match_date} and sna.REFERENCE like '%'+ #{reference}+ '%' and sna.value_date_type like '%'+ #{value_date}+'%'  and sm.sos_nbe_ats_id = sna.id and sna.availability = 1 and sna.status=1 union"
	...
	@Select("select distinct dna.id, dna.additional_information,dna.amount,dna.dr_cr,dna.match_status,dna.REFERENCE,dna.sender,dna.receiver,dna.upload_date,dna.value_date_type,u.firstname,u.lastname  from data_nbe_ats dna, users u,file_user_date fud  where  u.id =fud.user_id and dna.file_id=fud.file_id and dna.amount  between #{min_amount} and #{max_amount}  and dna.upload_date between #{min_upload_date} and #{max_upload_date} and dna.REFERENCE like '%'+ #{reference}+ '%' and dna.value_date_type like '%'+ #{value_date}+'%' and  dna.status = 1 and dna.availability = 1  union"
			+ " select distinct rna.id, rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname  from rtgs_nbe_ats rna, users u,file_user_date fud, rtgs_matched rm  where  u.id =fud.user_id and rna.file_id=fud.file_id and rna.amount  between #{min_amount} and #{max_amount}  and rna.upload_date between #{min_upload_date} and #{max_upload_date} and rna.REFERENCE like '%'+ #{reference}+ '%' and rna.value_date_type like '%'+ #{value_date}+'%'  and rm.rtgs_nbe_ats_id = rna.id and rna.availability = 1 and rna.status=1 union"
			+ " select distinct bna.id,bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna, users u,file_user_date fud, b2b_matched bm  where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.amount  between #{min_amount} and #{max_amount}  and bna.upload_date between #{min_upload_date} and #{max_upload_date} and bna.REFERENCE like '%'+ #{reference}+ '%' and bna.value_date_type like '%'+ #{value_date}+'%'  and bm.b2b_nbe_ats_id = bna.id and bna.availability = 1 and bna.status=1 union"
			+ " select distinct ena.id,ena.additional_information,ena.amount,ena.dr_cr,ena.match_status,ena.REFERENCE,ena.sender,ena.receiver,ena.upload_date,ena.value_date_type,u.firstname,u.lastname  from erca_nbe_ats ena, users u,file_user_date fud, erca_matched em  where  u.id =fud.user_id and ena.file_id=fud.file_id and ena.amount  between #{min_amount} and #{max_amount}  and ena.upload_date between #{min_upload_date} and #{max_upload_date} and ena.REFERENCE like '%'+ #{reference}+ '%' and ena.value_date_type like '%'+ #{value_date}+'%'  and em.erca_nbe_ats_id = ena.id and ena.availability = 1 and ena.status=1 union"
			+ " select distinct sna.id,sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna, users u,file_user_date fud, sos_matched sm  where  u.id =fud.user_id and sna.file_id=fud.file_id and sna.amount  between #{min_amount} and #{max_amount}  and sna.upload_date between #{min_upload_date} and #{max_upload_date} and sna.REFERENCE like '%'+ #{reference}+ '%' and sna.value_date_type like '%'+ #{value_date}+'%'  and sm.sos_nbe_ats_id = sna.id and sna.availability = 1 and sna.status=1 union"
	...
	@Select("select distinct dna.id, dna.additional_information,dna.amount,dna.dr_cr,dna.match_status,dna.REFERENCE,dna.sender,dna.receiver,dna.upload_date,dna.value_date_type,u.firstname,u.lastname  from data_nbe_ats dna, users u,file_user_date fud  where  u.id =fud.user_id and dna.file_id=fud.file_id and dna.amount  >= #{min_amount} and dna.upload_date between #{min_upload_date} and #{max_upload_date} and dna.REFERENCE like '%'+ #{reference}+ '%' and dna.value_date_type like '%'+ #{value_date}+'%' and  dna.status = 1 and dna.availability = 1  union"
			+ " select distinct rna.id, rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname  from rtgs_nbe_ats rna, users u,file_user_date fud, rtgs_matched rm  where  u.id =fud.user_id and rna.file_id=fud.file_id and rna.amount  >= #{min_amount}  and rna.upload_date between #{min_upload_date} and #{max_upload_date} and rna.REFERENCE like '%'+ #{reference}+ '%' and rna.value_date_type like '%'+ #{value_date}+'%'  and rm.rtgs_nbe_ats_id = rna.id and rna.availability = 1 and rna.status=1 union"
			+ " select distinct bna.id,bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna, users u,file_user_date fud, b2b_matched bm  where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.amount  >= #{min_amount} and bna.upload_date between #{min_upload_date} and #{max_upload_date} and bna.REFERENCE like '%'+ #{reference}+ '%' and bna.value_date_type like '%'+ #{value_date}+'%'  and bm.b2b_nbe_ats_id = bna.id and bna.availability = 1 and bna.status=1 union"
			+ " select distinct ena.id,ena.additional_information,ena.amount,ena.dr_cr,ena.match_status,ena.REFERENCE,ena.sender,ena.receiver,ena.upload_date,ena.value_date_type,u.firstname,u.lastname  from erca_nbe_ats ena, users u,file_user_date fud, erca_matched em  where  u.id =fud.user_id and ena.file_id=fud.file_id and ena.amount  >= #{min_amount}  and ena.upload_date between #{min_upload_date} and #{max_upload_date} and ena.REFERENCE like '%'+ #{reference}+ '%' and ena.value_date_type like '%'+ #{value_date}+'%'  and em.erca_nbe_ats_id = ena.id and ena.availability = 1 and ena.status=1 union"
			+ " select distinct sna.id,sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna, users u,file_user_date fud, sos_matched sm  where  u.id =fud.user_id and sna.file_id=fud.file_id and sna.amount  >= #{min_amount}  and sna.upload_date between #{min_upload_date} and #{max_upload_date} and sna.REFERENCE like '%'+ #{reference}+ '%' and sna.value_date_type like '%'+ #{value_date}+'%'  and sm.sos_nbe_ats_id = sna.id and sna.availability = 1 and sna.status=1 union"
	...
	@Select("select distinct dna.id, dna.additional_information,dna.amount,dna.dr_cr,dna.match_status,dna.REFERENCE,dna.sender,dna.receiver,dna.upload_date,dna.value_date_type,u.firstname,u.lastname  from data_nbe_ats dna, users u,file_user_date fud  where  u.id =fud.user_id and dna.file_id=fud.file_id and dna.amount  <= #{max_amount} and dna.upload_date between #{min_upload_date} and #{max_upload_date} and dna.REFERENCE like '%'+ #{reference}+ '%' and dna.value_date_type like '%'+ #{value_date}+'%' and  dna.status = 1 and dna.availability = 1  union"
			+ " select distinct rna.id, rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname  from rtgs_nbe_ats rna, users u,file_user_date fud, rtgs_matched rm  where  u.id =fud.user_id and rna.file_id=fud.file_id and rna.amount  <= #{max_amount}  and rna.upload_date between #{min_upload_date} and #{max_upload_date} and rna.REFERENCE like '%'+ #{reference}+ '%' and rna.value_date_type like '%'+ #{value_date}+'%'  and rm.rtgs_nbe_ats_id = rna.id and rna.availability = 1 and rna.status=1 union"
			+ " select distinct bna.id,bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna, users u,file_user_date fud, b2b_matched bm  where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.amount  <= #{max_amount} and bna.upload_date between #{min_upload_date} and #{max_upload_date} and bna.REFERENCE like '%'+ #{reference}+ '%' and bna.value_date_type like '%'+ #{value_date}+'%'  and bm.b2b_nbe_ats_id = bna.id and bna.availability = 1 and bna.status=1 union"
			+ " select distinct ena.id,ena.additional_information,ena.amount,ena.dr_cr,ena.match_status,ena.REFERENCE,ena.sender,ena.receiver,ena.upload_date,ena.value_date_type,u.firstname,u.lastname  from erca_nbe_ats ena, users u,file_user_date fud, erca_matched em  where  u.id =fud.user_id and ena.file_id=fud.file_id and ena.amount  <= #{max_amount}  and ena.upload_date between #{min_upload_date} and #{max_upload_date} and ena.REFERENCE like '%'+ #{reference}+ '%' and ena.value_date_type like '%'+ #{value_date}+'%'  and em.erca_nbe_ats_id = ena.id and ena.availability = 1 and ena.status=1 union"
			+ " select distinct sna.id,sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna, users u,file_user_date fud, sos_matched sm  where  u.id =fud.user_id and sna.file_id=fud.file_id and sna.amount <= #{max_amount} and sna.upload_date between #{min_upload_date} and #{max_upload_date} and sna.REFERENCE like '%'+ #{reference}+ '%' and sna.value_date_type like '%'+ #{value_date}+'%'  and sm.sos_nbe_ats_id = sna.id and sna.availability = 1 and sna.status=1 union"
	...
	@Select("select distinct rna.id,rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname  from rtgs_nbe_ats rna,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rna.file_id=fud.file_id and rna.amount between  #{min_amount} and  #{max_amount}  and rna.upload_date between #{min_upload_date} and #{min_upload_date} and rm.match_date between #{min_match_date} and #{max_match_date} and rna.REFERENCE like '%'+ #{reference}+ '%' and rna.value_date_type like '%'+ #{value_date}+'%' and rm.rtgs_nbe_ats_id = rna.id and rna.availability = 1 and rna.status=1 union"
			+ " select distinct bna.id,bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna,b2b_matched bm, users u,file_user_date fud  where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.amount between  #{min_amount} and #{max_amount}  and bna.upload_date between #{min_upload_date} and #{max_upload_date} and bm.match_date between #{min_match_date} and #{max_match_date} and bna.REFERENCE like '%'+ #{reference}+ '%' and bna.value_date_type like '%'+ #{value_date}+'%'  and bm.b2b_nbe_ats_id = bna.id and bna.availability = 1 and bna.status=1 union"
			+ " select distinct ena.id ,ena.additional_information,ena.amount,ena.dr_cr,ena.match_status,ena.REFERENCE,ena.sender,ena.receiver,ena.upload_date,ena.value_date_type,u.firstname,u.lastname  from erca_nbe_ats ena,erca_matched em, users u,file_user_date fud  where  u.id =fud.user_id and ena.file_id=fud.file_id and ena.amount between  #{min_amount} and #{max_amount}  and ena.upload_date between #{min_upload_date} and #{max_upload_date} and em.match_date between #{min_match_date} and #{max_match_date} and ena.REFERENCE like '%'+ #{reference}+ '%' and ena.value_date_type like '%'+ #{value_date}+'%'  and em.erca_nbe_ats_id = ena.id and ena.availability = 1 and ena.status=1 union"
			+ " select distinct sna.id ,sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sna.file_id=fud.file_id and sna.amount between  #{min_amount} and #{max_amount}  and sna.upload_date between #{min_upload_date} and #{max_upload_date} and sm.match_date between #{min_match_date} and #{max_match_date} and sna.REFERENCE like '%'+ #{reference}+ '%' and sna.value_date_type like '%'+ #{value_date}+'%'  and sm.sos_nbe_ats_id = sna.id and sna.availability = 1 and sna.status=1 union"
	...
	@Select("select distinct rna.id,rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname  from rtgs_nbe_ats rna,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rna.file_id=fud.file_id and rna.amount >=  #{min_amount}  and rna.upload_date between #{min_upload_date} and #{min_upload_date} and rm.match_date between #{min_match_date} and #{max_match_date} and rna.REFERENCE like '%'+ #{reference}+ '%' and rna.value_date_type like '%'+ #{value_date}+'%' and rm.rtgs_nbe_ats_id = rna.id and rna.availability = 1 and rna.status=1 union"
			+ " select distinct bna.id,bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna,b2b_matched bm, users u,file_user_date fud  where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.amount >=  #{min_amount}  and bna.upload_date between #{min_upload_date} and #{max_upload_date} and bm.match_date between #{min_match_date} and #{max_match_date} and bna.REFERENCE like '%'+ #{reference}+ '%' and bna.value_date_type like '%'+ #{value_date}+'%'  and bm.b2b_nbe_ats_id = bna.id and bna.availability = 1 and bna.status=1 union"
			...
			+ " select distinct sna.id ,sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sna.file_id=fud.file_id and sna.amount >=  #{min_amount} and sna.upload_date between #{min_upload_date} and #{max_upload_date} and sm.match_date between #{min_match_date} and #{max_match_date} and sna.REFERENCE like '%'+ #{reference}+ '%' and sna.value_date_type like '%'+ #{value_date}+'%'  and sm.sos_nbe_ats_id = sna.id and sna.availability = 1 and sna.status=1 union"
	...
	@Select("select distinct rna.id,rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname  from rtgs_nbe_ats rna,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rna.file_id=fud.file_id and rna.amount <=  #{max_amount}  and rna.upload_date between #{min_upload_date} and #{min_upload_date} and rm.match_date between #{min_match_date} and #{max_match_date} and rna.REFERENCE like '%'+ #{reference}+ '%' and rna.value_date_type like '%'+ #{value_date}+'%' and rm.rtgs_nbe_ats_id = rna.id and rna.availability = 1 and rna.status=1 union"
			+ " select distinct bna.id,bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna,b2b_matched bm, users u,file_user_date fud  where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.amount <=  #{max_amount}  and bna.upload_date between #{min_upload_date} and #{max_upload_date} and bm.match_date between #{min_match_date} and #{max_match_date} and bna.REFERENCE like '%'+ #{reference}+ '%' and bna.value_date_type like '%'+ #{value_date}+'%'  and bm.b2b_nbe_ats_id = bna.id and bna.availability = 1 and bna.status=1 union"
			...
			+ " select distinct sna.id ,sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sna.file_id=fud.file_id and sna.amount <=  #{max_amount} and sna.upload_date between #{min_upload_date} and #{max_upload_date} and sm.match_date between #{min_match_date} and #{max_match_date} and sna.REFERENCE like '%'+ #{reference}+ '%' and sna.value_date_type like '%'+ #{value_date}+'%'  and sm.sos_nbe_ats_id = sna.id and sna.availability = 1 and sna.status=1 union"
	...
	@Select("select distinct rna.id ,rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname  from rtgs_nbe_ats rna,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rna.file_id=fud.file_id and  rna.upload_date between #{min_upload_date} and #{max_upload_date}  and rm.match_date between #{min_match_date} and #{max_match_date} and rna.REFERENCE like '%'+ #{reference}+ '%' and rna.value_date_type like '%'+ #{value_date}+'%'  and rm.rtgs_nbe_ats_id = rna.id and rna.availability = 1 and rna.status=1 union"
			+ " select distinct bna.id ,bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna,b2b_matched bm, users u,file_user_date fud  where  u.id =fud.user_id and bna.file_id=fud.file_id and  bna.upload_date between #{min_upload_date}  and #{max_upload_date}  and bm.match_date between #{min_match_date} and #{max_match_date} and bna.REFERENCE like '%'+ #{reference}+ '%' and bna.value_date_type like '%'+ #{value_date}+'%'  and bm.b2b_nbe_ats_id = bna.id and bna.availability = 1 and bna.status=1 union"
			...
			+ " select distinct sna.id ,sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sna.file_id=fud.file_id and  sna.upload_date between #{min_upload_date} and #{max_upload_date} and sm.match_date between #{min_match_date} and #{max_match_date} and sna.REFERENCE like '%'+ #{reference}+ '%' and sna.value_date_type like '%'+ #{value_date}+'%'  and sm.sos_nbe_ats_id = sna.id and sna.availability = 1 and sna.status=1 union"
	...
	@Select("select distinct rna.id ,rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname  from rtgs_nbe_ats rna,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rna.file_id=fud.file_id and rna.amount between #{min_amount} and #{max_amount} and rm.match_date between #{min_match_date} and #{max_match_date} and rna.REFERENCE like '%'+ #{reference}+ '%' and rna.value_date_type like '%'+ #{value_date}+'%'  and rm.rtgs_nbe_ats_id = rna.id and rna.availability = 1 and rna.status=1 union"
			+ " select distinct bna.id ,bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna,b2b_matched bm, users u,file_user_date fud  where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.amount between #{min_amount} and #{max_amount} and bm.match_date between #{min_match_date} and #{max_match_date} and bna.REFERENCE like '%'+ #{reference}+ '%' and bna.value_date_type like '%'+ #{value_date}+'%'  and bm.b2b_nbe_ats_id = bna.id and bna.availability = 1 and bna.status=1union"
			...
			+ " select distinct sna.id,sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sna.file_id=fud.file_id and sna.amount between #{min_amount} and #{max_amount} and sm.match_date between #{min_match_date} and #{max_match_date} and sna.REFERENCE like '%'+ #{reference}+ '%' and sna.value_date_type like '%'+ #{value_date}+'%'  and sm.sos_nbe_ats_id = sna.id and sna.availability = 1 and sna.status=1 union"
	...
	@Select("select distinct rna.id ,rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname  from rtgs_nbe_ats rna,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rna.file_id=fud.file_id and rna.amount >= #{min_amount} and rm.match_date between #{min_match_date} and #{max_match_date} and rna.REFERENCE like '%'+ #{reference}+ '%' and rna.value_date_type like '%'+ #{value_date}+'%'  and rm.rtgs_nbe_ats_id = rna.id and rna.availability = 1 and rna.status=1 union"
			+ " select distinct bna.id ,bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna,b2b_matched bm, users u,file_user_date fud  where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.amount >= #{min_amount} and bm.match_date between #{min_match_date} and #{max_match_date} and bna.REFERENCE like '%'+ #{reference}+ '%' and bna.value_date_type like '%'+ #{value_date}+'%'  and bm.b2b_nbe_ats_id = bna.id and bna.availability = 1 and bna.status=1union"
			+ " select distinct ena.id,ena.additional_information,ena.amount,ena.dr_cr,ena.match_status,ena.REFERENCE,ena.sender,ena.receiver,ena.upload_date,ena.value_date_type,u.firstname,u.lastname  from erca_nbe_ats ena,erca_matched em, users u,file_user_date fud  where  u.id =fud.user_id and ena.file_id=fud.file_id and ena.amount >= #{min_amount} and em.match_date between #{min_match_date} and #{max_match_date} and ena.REFERENCE like '%'+ #{reference}+ '%' and ena.value_date_type like '%'+ #{value_date}+'%'  and em.erca_nbe_ats_id = ena.id and ena.availability = 1 and ena.status=1 union"
			+ " select distinct sna.id,sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sna.file_id=fud.file_id and sna.amount >= #{min_amount} and sm.match_date between #{min_match_date} and #{max_match_date} and sna.REFERENCE like '%'+ #{reference}+ '%' and sna.value_date_type like '%'+ #{value_date}+'%'  and sm.sos_nbe_ats_id = sna.id and sna.availability = 1 and sna.status=1 union"
	...
	@Select("select distinct rna.id ,rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname  from rtgs_nbe_ats rna,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rna.file_id=fud.file_id and rna.amount <= #{max_amount} and rm.match_date between #{min_match_date} and #{max_match_date} and rna.REFERENCE like '%'+ #{reference}+ '%' and rna.value_date_type like '%'+ #{value_date}+'%'  and rm.rtgs_nbe_ats_id = rna.id and rna.availability = 1 and rna.status=1 union"
			+ " select distinct bna.id ,bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna,b2b_matched bm, users u,file_user_date fud  where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.amount <= #{max_amount} and bm.match_date between #{min_match_date} and #{max_match_date} and bna.REFERENCE like '%'+ #{reference}+ '%' and bna.value_date_type like '%'+ #{value_date}+'%'  and bm.b2b_nbe_ats_id = bna.id and bna.availability = 1 and bna.status=1union"
			+ " select distinct ena.id,ena.additional_information,ena.amount,ena.dr_cr,ena.match_status,ena.REFERENCE,ena.sender,ena.receiver,ena.upload_date,ena.value_date_type,u.firstname,u.lastname  from erca_nbe_ats ena,erca_matched em, users u,file_user_date fud  where  u.id =fud.user_id and ena.file_id=fud.file_id and ena.amount <= #{max_amount} and em.match_date between #{min_match_date} and #{max_match_date} and ena.REFERENCE like '%'+ #{reference}+ '%' and ena.value_date_type like '%'+ #{value_date}+'%'  and em.erca_nbe_ats_id = ena.id and ena.availability = 1 and ena.status=1 union"
			+ " select distinct sna.id,sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sna.file_id=fud.file_id and sna.amount <= #{max_amount} and sm.match_date between #{min_match_date} and #{max_match_date} and sna.REFERENCE like '%'+ #{reference}+ '%' and sna.value_date_type like '%'+ #{value_date}+'%'  and sm.sos_nbe_ats_id = sna.id and sna.availability = 1 and sna.status=1 union"
	...
	@Select("select distinct dac.id,dac.additional_information,dac.amount,dac.dr_cr,dac.match_status,dac.transaction_reference,dac.branch_code,dac.value_date,dac.upload_date,dac.posting_date,u.firstname,u.lastname  from data_awb_core dac,users u,file_user_date fud where  u.id =fud.user_id and dac.file_id=fud.file_id and dac.amount  between #{min_amount} and #{max_amount}  and dac.additional_information like '%'+ #{reference} + '%' and dac.value_date like '%'+#{value_date}+ '%' and dac.branch_code like '%'+#{branch_code}+'%'  and dac.status = 1 and dac.availability = 1 union"
			+ " select distinct rac.id,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname from rtgs_awb_core rac,users u,file_user_date fud, rtgs_matched rm where  u.id =fud.user_id and rac.file_id=fud.file_id  and rac.amount between  #{min_amount} and #{max_amount} and rac.additional_information like '%'+ #{reference} + '%' and rac.value_date like '%'+#{value_date}+ '%' and rac.branch_code like '%'+#{branch_code}+'%' and rm.match_id = rac.match_id and rac.availability = 1 and rac.status=1 union"
			+ " select distinct bac.id, bac.additional_information,bac.amount,bac.dr_cr,bac.match_status,bac.transaction_reference,bac.branch_code,bac.value_date,bac.upload_date,bac.posting_date,u.firstname,u.lastname  from b2b_awb_core bac,users u,file_user_date fud, b2b_matched bm  where  u.id =fud.user_id and bac.file_id=fud.file_id and bac.amount  between #{min_amount} and #{max_amount} and bac.additional_information like '%'+ #{reference} + '%' and bac.value_date like '%'+#{value_date}+ '%' and bac.branch_code like '%'+#{branch_code}+'%'  and bm.match_id = bac.match_id and bac.availability = 1 and bac.status=1 union"
			+ " select distinct eac.id, eac.additional_information,eac.amount,eac.dr_cr,eac.match_status,eac.transaction_reference,eac.branch_code,eac.value_date,eac.upload_date,eac.posting_date,u.firstname,u.lastname  from erca_awb_core eac,users u,file_user_date fud, erca_matched em  where  u.id =fud.user_id and eac.file_id=fud.file_id and eac.amount  between #{min_amount} and #{max_amount} and eac.additional_information like '%'+ #{reference} + '%' and eac.value_date like '%'+#{value_date}+ '%' and eac.branch_code like '%'+#{branch_code}+'%' and em.match_id = eac.match_id and eac.availability = 1 and eac.status=1 union"
			+ " select distinct sac.id, sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,users u,file_user_date fud, sos_matched sm  where  u.id =fud.user_id and sac.file_id=fud.file_id and sac.amount  between #{min_amount} and #{max_amount} and sac.additional_information like '%'+ #{reference} + '%' and sac.value_date like '%'+#{value_date}+ '%' and sac.branch_code like '%'+#{branch_code}+'%'  and sm.match_id = sac.match_id and sac.availability = 1 and sac.status=1 union"
			+ " select distinct cr.id, cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.upload_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr,users u,file_user_date fud  where  u.id =fud.user_id and cr.file_id=fud.file_id and cr.amount  between #{min_amount} and #{max_amount} and cr.additional_information like '%'+ #{reference} + '%' and cr.posting_date like '%'+#{value_date}+ '%' and cr.branch_code like '%'+#{branch_code}+'%' and cr.availability = 1 and cr.status=1 union"
			+ " select distinct cpm.id,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,users u,file_user_date fud, partially_matched pm  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.amount  between #{min_amount} and #{max_amount} and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+#{value_date}+ '%' and cpm.branch_code like '%'+#{branch_code}+'%'  and pm.match_id = cpm.match_id and cpm.availability = 1 and cpm.status=1")
	public List<Transactionhistory> searchWithAmountCore(@Param("min_amount") float min_amount,
			@Param("max_amount") float max_amount, @Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	// min core
	@Select("select distinct dac.id,dac.additional_information,dac.amount,dac.dr_cr,dac.match_status,dac.transaction_reference,dac.branch_code,dac.value_date,dac.upload_date,dac.posting_date,u.firstname,u.lastname  from data_awb_core dac,users u,file_user_date fud where  u.id =fud.user_id and dac.file_id=fud.file_id and dac.amount  >= #{min_amount} and dac.additional_information like '%'+ #{reference} + '%' and dac.value_date like '%'+#{value_date}+ '%' and dac.branch_code like '%'+#{branch_code}+'%'  and dac.status = 1 and dac.availability = 1  union"
			+ " select distinct rac.id,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname from rtgs_awb_core rac,users u,file_user_date fud, rtgs_matched rm where  u.id =fud.user_id and rac.file_id=fud.file_id  and rac.amount >=  #{min_amount}  and rac.additional_information like '%'+ #{reference} + '%' and rac.value_date like '%'+#{value_date}+ '%' and rac.branch_code like '%'+#{branch_code}+'%' and rm.match_id = rac.match_id and rac.availability = 1 and rac.status=1 union"
			+ " select distinct bac.id, bac.additional_information,bac.amount,bac.dr_cr,bac.match_status,bac.transaction_reference,bac.branch_code,bac.value_date,bac.upload_date,bac.posting_date,u.firstname,u.lastname  from b2b_awb_core bac,users u,file_user_date fud, b2b_matched bm  where  u.id =fud.user_id and bac.file_id=fud.file_id and bac.amount  >= #{min_amount} and bac.additional_information like '%'+ #{reference} + '%' and bac.value_date like '%'+#{value_date}+ '%' and bac.branch_code like '%'+#{branch_code}+'%'  and bm.match_id = bac.match_id and bac.availability = 1 and bac.status=1 union"
			...
			+ " select distinct sac.id, sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,users u,file_user_date fud, sos_matched sm  where  u.id =fud.user_id and sac.file_id=fud.file_id and sac.amount  >= #{min_amount} and sac.additional_information like '%'+ #{reference} + '%' and sac.value_date like '%'+#{value_date}+ '%' and sac.branch_code like '%'+#{branch_code}+'%'  and sm.match_id = sac.match_id and sac.availability = 1 and sac.status=1 union"
			+ " select distinct cr.id, cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.upload_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr,users u,file_user_date fud where  u.id =fud.user_id and cr.file_id=fud.file_id and cr.amount  >= #{min_amount} and cr.additional_information like '%'+ #{reference} + '%' and cr.value_date like '%'+#{value_date}+ '%' and cr.branch_code like '%'+#{branch_code}+'%'  and cr.availability = 1 and cr.status=1 union"
			+ " select distinct cpm.id,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,users u,file_user_date fud, partially_matched pm  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.amount  >= #{min_amount} and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+#{value_date}+ '%' and cpm.branch_code like '%'+#{branch_code}+'%'  and pm.match_id = cpm.match_id and cpm.availability = 1 and cpm.status=1")
	public List<Transactionhistory> searchWithMinAmountCore(@Param("min_amount") float min_amount,
			@Param("reference") String reference, @Param("value_date") String value_date,
			@Param("branch_code") String branch_code);
	// min core

	// max core
	@Select("select distinct dac.id,dac.additional_information,dac.amount,dac.dr_cr,dac.match_status,dac.transaction_reference,dac.branch_code,dac.value_date,dac.upload_date,dac.posting_date,u.firstname,u.lastname  from data_awb_core dac,users u,file_user_date fud where  u.id =fud.user_id and dac.file_id=fud.file_id and dac.amount  <= #{max_amount} and dac.additional_information like '%'+ #{reference} + '%' and dac.value_date like '%'+#{value_date}+ '%' and dac.branch_code like '%'+#{branch_code}+'%' and  dac.status = 1 and dac.availability = 1  union"
			+ " select distinct rac.id,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname from rtgs_awb_core rac,users u,file_user_date fud, rtgs_matched rm where  u.id =fud.user_id and rac.file_id=fud.file_id  and rac.amount <=  #{max_amount}  and rac.additional_information like '%'+ #{reference} + '%' and rac.value_date like '%'+#{value_date}+ '%' and rac.branch_code like '%'+#{branch_code}+'%' and rm.match_id = rac.match_id and rac.availability = 1 and rac.status=1 union"
			+ " select distinct bac.id, bac.additional_information,bac.amount,bac.dr_cr,bac.match_status,bac.transaction_reference,bac.branch_code,bac.value_date,bac.upload_date,bac.posting_date,u.firstname,u.lastname  from b2b_awb_core bac,users u,file_user_date fud, b2b_matched bm  where  u.id =fud.user_id and bac.file_id=fud.file_id and bac.amount  <= #{max_amount} and bac.additional_information like '%'+ #{reference} + '%' and bac.value_date like '%'+#{value_date}+ '%' and bac.branch_code like '%'+#{branch_code}+'%'  and bm.match_id = bac.match_id and bac.availability = 1 and bac.status=1 union"
			...
			+ " select distinct sac.id, sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,users u,file_user_date fud, sos_matched sm  where  u.id =fud.user_id and sac.file_id=fud.file_id and sac.amount  <= #{max_amount} and sac.additional_information like '%'+ #{reference} + '%' and sac.value_date like '%'+#{value_date}+ '%' and sac.branch_code like '%'+#{branch_code}+'%'  and sm.match_id = sac.match_id and sac.availability = 1 and sac.status=1 union"
			+ " select distinct cr.id, cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.upload_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr,users u,file_user_date fud where  u.id =fud.user_id and cr.file_id=fud.file_id and cr.amount  <= #{max_amount} and cr.additional_information like '%'+ #{reference} + '%' and cr.value_date like '%'+#{value_date}+ '%' and cr.branch_code like '%'+#{branch_code}+'%'  and cr.availability = 1 and cr.status=1 union"
			+ " select distinct cpm.id,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,users u,file_user_date fud, partially_matched pm  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.amount  <= #{max_amount} and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+#{value_date}+ '%' and cpm.branch_code like '%'+#{branch_code}+'%'  and pm.match_id = cpm.match_id and cpm.availability = 1 and cpm.status=1")
	public List<Transactionhistory> searchWithMaxAmountCore(@Param("max_amount") float max_amount,
			@Param("reference") String reference, @Param("value_date") String value_date,
			@Param("branch_code") String branch_code);

	// max core
	@Select("select dac.id,dac.additional_information,dac.amount,dac.dr_cr,dac.match_status,dac.transaction_reference,dac.branch_code,dac.value_date,dac.upload_date,dac.posting_date,u.firstname,u.lastname  from data_awb_core dac,users u,file_user_date fud  where  u.id =fud.user_id and dac.file_id=fud.file_id  and dac.upload_date between #{min_upload_date} and #{max_upload_date} and dac.additional_information like '%'+ #{reference} + '%' and dac.value_date like '%'+#{value_date}+ '%' and dac.branch_code like '%'+#{branch_code}+'%' and  dac.status = 1 and dac.availability = 1  union"
			+ " select rac.id,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname  from rtgs_awb_core rac,users u,file_user_date fud, rtgs_matched rm  where  u.id =fud.user_id and rac.file_id=fud.file_id and rac.upload_date between #{min_upload_date} and #{max_upload_date} and rac.additional_information like '%'+ #{reference} + '%' and rac.value_date like '%'+#{value_date}+ '%' and rac.branch_code like '%'+#{branch_code}+'%' and rm.match_id = rac.match_id and rac.availability = 1 and rac.status=1 union"
			+ " select bac.id,bac.additional_information,bac.amount,bac.dr_cr,bac.match_status,bac.transaction_reference,bac.branch_code,bac.value_date,bac.upload_date,bac.posting_date,u.firstname,u.lastname  from b2b_awb_core bac,users u,file_user_date fud, b2b_matched bm  where  u.id =fud.user_id and bac.file_id=fud.file_id and bac.upload_date between #{min_upload_date} and #{max_upload_date} and bac.additional_information like '%'+ #{reference} + '%' and bac.value_date like '%'+#{value_date}+ '%' and bac.branch_code like '%'+#{branch_code}+'%'  and bm.match_id = bac.match_id and bac.availability = 1 and bac.status=1 union"
			...
			+ " select sac.id, sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,users u,file_user_date fud, sos_matched sm where  u.id =fud.user_id and sac.file_id=fud.file_id and sac.upload_date between #{min_upload_date} and #{max_upload_date} and sac.additional_information like '%'+ #{reference} + '%' and sac.value_date like '%'+#{value_date}+ '%' and sac.branch_code like '%'+#{branch_code}+'%'  and sm.match_id = sac.match_id and sac.availability = 1 and sac.status=1 union"
			+ " select cr.id, cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.upload_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr,users u,file_user_date fud where  u.id =fud.user_id and cr.file_id=fud.file_id and cr.upload_date between #{min_upload_date} and #{max_upload_date} and cr.additional_information like '%'+ #{reference} + '%' and cr.posting_date like '%'+#{value_date}+ '%' and cr.branch_code like '%'+#{branch_code}+'%' and cr.availability = 1 and cr.status=1 union"
			+ " select cpm.id, cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,users u,file_user_date fud, partially_matched pm  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.upload_date between #{min_upload_date} and #{max_upload_date} and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+#{value_date}+ '%' and cpm.branch_code like '%'+#{branch_code}+'%'  and pm.match_id = cpm.match_id and cpm.availability = 1 and cpm.status=1")
	public List<Transactionhistory> searchWithUploadDateCore(@Param("min_upload_date") int min_upload_date,
			@Param("max_upload_date") int max_upload_date, @Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	@Select("select rac.id, rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname  from rtgs_awb_core rac,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rac.file_id=fud.file_id and  rm.match_date between #{min_match_date} and #{max_match_date} and rac.additional_information like '%'+ #{reference} + '%' and rac.value_date like '%'+#{value_date}+ '%' and rac.branch_code like '%'+#{branch_code}+'%' and rm.match_id = rac.match_id and rac.availability = 1 and rac.status=1 union"
			+ " select bac.id,bac.additional_information,bac.amount,bac.dr_cr,bac.match_status,bac.transaction_reference,bac.branch_code,bac.value_date,bac.upload_date,bac.posting_date,u.firstname,u.lastname  from b2b_awb_core bac,b2b_matched bm, users u,file_user_date fud  where  u.id =fud.user_id and bac.file_id=fud.file_id and  bm.match_date between #{min_match_date} and #{max_match_date} and bac.additional_information like '%'+ #{reference} + '%' and bac.value_date like '%'+#{value_date}+ '%' and bac.branch_code like '%'+#{branch_code}+'%'  and bm.match_id = bac.match_id and bac.availability = 1 and bac.status=1 union"
			+ " select eac.id,eac.additional_information,eac.amount,eac.dr_cr,eac.match_status,eac.transaction_reference,eac.branch_code,eac.value_date,eac.upload_date,eac.posting_date,u.firstname,u.lastname  from erca_awb_core eac,erca_matched em, users u,file_user_date fud  where  u.id =fud.user_id and eac.file_id=fud.file_id and  em.match_date between #{min_match_date} and #{max_match_date} and eac.additional_information like '%'+ #{reference} + '%' and eac.value_date like '%'+#{value_date}+ '%' and eac.branch_code like '%'+#{branch_code}+'%'  and em.match_id = eac.match_id and eac.availability = 1 and eac.status=1 union"
			+ " select sac.id,sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sac.file_id=fud.file_id and  sm.match_date between #{min_match_date} and #{max_match_date} and sac.additional_information like '%'+ #{reference} + '%' and sac.value_date like '%'+#{value_date}+ '%' and sac.branch_code like '%'+#{branch_code}+'%'  and sm.match_id = sac.match_id and sac.availability = 1 and sac.status=1 union"
			+ " select cr.id,cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.upload_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr, users u,file_user_date fud  where  u.id =fud.user_id and cr.file_id=fud.file_id and  cr.match_date between #{min_match_date} and #{max_match_date} and cr.additional_information like '%'+ #{reference} + '%' and cr.value_date like '%'+#{value_date}+ '%' and cr.branch_code like '%'+#{branch_code}+'%'  and cr.availability = 1 and cr.status=1 union"
			+ " select cpm.id,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,partially_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and cpm.file_id=fud.file_id and  pm.match_date between #{min_match_date} and #{max_match_date} and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+#{value_date}+ '%' and cpm.branch_code like '%'+#{branch_code}+'%'  and pm.match_id = cpm.match_id and cpm.availability = 1 and cpm.status=1")
	public List<Transactionhistory> searchWithMatchDateCore(@Param("min_match_date") int min_match_date,
			@Param("max_match_date") int max_match_date, @Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	@Select("select distinct dac.id, dac.additional_information,dac.amount,dac.dr_cr,dac.match_status,dac.transaction_reference,dac.branch_code,dac.value_date,dac.upload_date,dac.posting_date,u.firstname,u.lastname  from data_awb_core dac, users u,file_user_date fud  where  u.id =fud.user_id and dac.file_id=fud.file_id and dac.amount  between #{min_amount} and  #{max_amount}  and dac.upload_date between #{min_upload_date} and #{max_upload_date} and dac.additional_information like '%'+ #{reference} + '%' and dac.value_date like '%'+#{value_date}+ '%' and dac.branch_code like '%'+#{branch_code}+'%'  and  dac.status = 1 and dac.availability = 1  union"
			+ " select distinct rac.id, rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname  from rtgs_awb_core rac, users u,file_user_date fud, rtgs_matched rm  where  u.id =fud.user_id and rac.file_id=fud.file_id and rac.amount  between #{min_amount} and #{max_amount}  and rac.upload_date between #{min_upload_date} and #{max_upload_date} and rac.additional_information like '%'+ #{reference} + '%' and rac.value_date like '%'+#{value_date}+ '%' and rac.branch_code like '%'+#{branch_code}+'%' and rm.match_id = rac.match_id and rac.availability = 1 and rac.status=1union"
			+ " select distinct bac.id,bac.additional_information,bac.amount,bac.dr_cr,bac.match_status,bac.transaction_reference,bac.branch_code,bac.value_date,bac.upload_date,bac.posting_date,u.firstname,u.lastname  from b2b_awb_core bac, users u,file_user_date fud, b2b_matched bm  where  u.id =fud.user_id and bac.file_id=fud.file_id and bac.amount  between #{min_amount} and #{max_amount}  and bac.upload_date between #{min_upload_date} and #{max_upload_date} and bac.additional_information like '%'+ #{reference} + '%' and bac.value_date like '%'+#{value_date}+ '%' and bac.branch_code like '%'+#{branch_code}+'%'  and bm.match_id = bac.match_id and bac.availability = 1 and bac.status=1 union"
			...
			+ " select distinct sac.id,sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.value_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac, users u,file_user_date fud, sos_matched sm  where  u.id =fud.user_id and sac.file_id=fud.file_id and sac.amount  between #{min_amount} and #{max_amount}  and sac.upload_date between #{min_upload_date} and #{max_upload_date} and sac.additional_information like '%'+ #{reference} + '%' and sac.value_date like '%'+#{value_date}+ '%' and sac.branch_code like '%'+#{branch_code}+'%'  and sm.match_id = sac.match_id and sac.availability = 1 and sac.status=1 union"
			+ " select distinct cr.id,cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.value_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr, users u,file_user_date fud  where  u.id =fud.user_id and cr.file_id=fud.file_id and cr.amount  between #{min_amount} and #{max_amount}  and cr.upload_date between #{min_upload_date} and #{max_upload_date} and cr.additional_information like '%'+ #{reference} + '%' and cr.value_date like '%'+#{value_date}+ '%' and cr.branch_code like '%'+#{branch_code}+'%'  and cr.availability = 1 and cr.status=1 union"
			+ " select distinct cpm.id ,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm, users u,file_user_date fud, partially_matched pm  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.amount  between #{min_amount} and #{max_amount}  and cpm.upload_date between #{min_upload_date} and #{max_upload_date} and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+#{value_date}+ '%' and cpm.branch_code like '%'+#{branch_code}+'%'  and pm.match_id = cpm.match_id and cpm.availability = 1 and cpm.status=1")
	public List<Transactionhistory> searchWithAmountAndUploadDateCore(@Param("min_amount") float min_amount,
			@Param("max_amount") float max_amount, @Param("min_upload_date") int min_upload_date,
			@Param("max_upload_date") int max_upload_date, @Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	@Select("select distinct dac.id, dac.additional_information,dac.amount,dac.dr_cr,dac.match_status,dac.transaction_reference,dac.branch_code,dac.value_date,dac.upload_date,dac.posting_date,u.firstname,u.lastname  from data_awb_core dac, users u,file_user_date fud  where  u.id =fud.user_id and dac.file_id=fud.file_id and dac.amount  >= #{min_amount} and dac.upload_date between #{min_upload_date} and #{max_upload_date} and dac.additional_information like '%'+ #{reference} + '%' and dac.value_date like '%'+#{value_date}+ '%' and dac.branch_code like '%'+#{branch_code}+'%'  and  dac.status = 1 and dac.availability = 1  union"
			+ " select distinct rac.id, rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname  from rtgs_awb_core rac, users u,file_user_date fud, rtgs_matched rm  where  u.id =fud.user_id and rac.file_id=fud.file_id and rac.amount   >= #{min_amount}  and rac.upload_date between #{min_upload_date} and #{max_upload_date} and rac.additional_information like '%'+ #{reference} + '%' and rac.value_date like '%'+#{value_date}+ '%' and rac.branch_code like '%'+#{branch_code}+'%' and rm.match_id = rac.match_id and rac.availability = 1 and rac.status=1union"
			+ " select distinct bac.id,bac.additional_information,bac.amount,bac.dr_cr,bac.match_status,bac.transaction_reference,bac.branch_code,bac.value_date,bac.upload_date,bac.posting_date,u.firstname,u.lastname  from b2b_awb_core bac, users u,file_user_date fud, b2b_matched bm  where  u.id =fud.user_id and bac.file_id=fud.file_id and bac.amount  >= #{min_amount}  and bac.upload_date between #{min_upload_date} and #{max_upload_date} and bac.additional_information like '%'+ #{reference} + '%' and bac.value_date like '%'+#{value_date}+ '%' and bac.branch_code like '%'+#{branch_code}+'%'  and bm.match_id = bac.match_id and bac.availability = 1 and bac.status=1 union"
			...
			+ " select distinct sac.id,sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.value_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac, users u,file_user_date fud, sos_matched sm  where  u.id =fud.user_id and sac.file_id=fud.file_id and sac.amount  >= #{min_amount}  and sac.upload_date between #{min_upload_date} and #{max_upload_date} and sac.additional_information like '%'+ #{reference} + '%' and sac.value_date like '%'+#{value_date}+ '%' and sac.branch_code like '%'+#{branch_code}+'%'  and sm.match_id = sac.match_id and sac.availability = 1 and sac.status=1 union"
			+ " select distinct cr.id,cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.value_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr, users u,file_user_date fud, sos_matched sm  where  u.id =fud.user_id and cr.file_id=fud.file_id and cr.amount  >= #{min_amount}  and cr.upload_date between #{min_upload_date} and #{max_upload_date} and cr.additional_information like '%'+ #{reference} + '%' and cr.value_date like '%'+#{value_date}+ '%' and cr.branch_code like '%'+#{branch_code}+'%'  and cr.availability = 1 and cr.status=1 union"
			+ " select distinct cpm.id ,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm, users u,file_user_date fud, partially_matched pm  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.amount   >= #{min_amount}  and cpm.upload_date between #{min_upload_date} and #{max_upload_date} and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+#{value_date}+ '%' and cpm.branch_code like '%'+#{branch_code}+'%'  and pm.match_id = cpm.match_id and cpm.availability = 1 and cpm.status=1")
	public List<Transactionhistory> searchWithMinAmountAndUploadDateCore(@Param("min_amount") float min_amount,
			@Param("min_upload_date") int min_upload_date, @Param("max_upload_date") int max_upload_date,
			@Param("reference") String reference, @Param("value_date") String value_date,
			@Param("branch_code") String branch_code);

	@Select("select distinct dac.id, dac.additional_information,dac.amount,dac.dr_cr,dac.match_status,dac.transaction_reference,dac.branch_code,dac.value_date,dac.upload_date,dac.posting_date,u.firstname,u.lastname  from data_awb_core dac, users u,file_user_date fud  where  u.id =fud.user_id and dac.file_id=fud.file_id and dac.amount  <= #{max_amount} and dac.upload_date between #{min_upload_date} and #{max_upload_date} and dac.additional_information like '%'+ #{reference} + '%' and dac.value_date like '%'+#{value_date}+ '%' and dac.branch_code like '%'+#{branch_code}+'%'  and  dac.status = 1 and dac.availability = 1  union"
			+ " select distinct rac.id, rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname  from rtgs_awb_core rac, users u,file_user_date fud, rtgs_matched rm  where  u.id =fud.user_id and rac.file_id=fud.file_id and rac.amount   <= #{max_amount}  and rac.upload_date between #{min_upload_date} and #{max_upload_date} and rac.additional_information like '%'+ #{reference} + '%' and rac.value_date like '%'+#{value_date}+ '%' and rac.branch_code like '%'+#{branch_code}+'%' and rm.match_id = rac.match_id and rac.availability = 1 and rac.status=1union"
			+ " select distinct bac.id,bac.additional_information,bac.amount,bac.dr_cr,bac.match_status,bac.transaction_reference,bac.branch_code,bac.value_date,bac.upload_date,bac.posting_date,u.firstname,u.lastname  from b2b_awb_core bac, users u,file_user_date fud, b2b_matched bm  where  u.id =fud.user_id and bac.file_id=fud.file_id and bac.amount  <= #{max_amount}  and bac.upload_date between #{min_upload_date} and #{max_upload_date} and bac.additional_information like '%'+ #{reference} + '%' and bac.value_date like '%'+#{value_date}+ '%' and bac.branch_code like '%'+#{branch_code}+'%'  and bm.match_id = bac.match_id and bac.availability = 1 and bac.status=1 union"
			...
			+ " select distinct sac.id,sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.value_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac, users u,file_user_date fud, sos_matched sm  where  u.id =fud.user_id and sac.file_id=fud.file_id and sac.amount  <= #{max_amount}  and sac.upload_date between #{min_upload_date} and #{max_upload_date} and sac.additional_information like '%'+ #{reference} + '%' and sac.value_date like '%'+#{value_date}+ '%' and sac.branch_code like '%'+#{branch_code}+'%'  and sm.match_id = sac.match_id and sac.availability = 1 and sac.status=1 union"
			+ " select distinct cr.id,cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.value_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr, users u,file_user_date fud, sos_matched sm  where  u.id =fud.user_id and cr.file_id=fud.file_id and cr.amount  <= #{max_amount}  and cr.upload_date between #{min_upload_date} and #{max_upload_date} and cr.additional_information like '%'+ #{reference} + '%' and cr.value_date like '%'+#{value_date}+ '%' and cr.branch_code like '%'+#{branch_code}+'%'  and cr.availability = 1 and cr.status=1 union"
			+ " select distinct cpm.id ,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm, users u,file_user_date fud, partially_matched pm  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.amount   <= #{max_amount}  and cpm.upload_date between #{min_upload_date} and #{max_upload_date} and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+#{value_date}+ '%' and cpm.branch_code like '%'+#{branch_code}+'%'  and pm.match_id = cpm.match_id and cpm.availability = 1 and cpm.status=1")
	public List<Transactionhistory> searchWithMaxAmountAndUploadDateCore(@Param("max_amount") float max_amount,
			@Param("min_upload_date") int min_upload_date, @Param("max_upload_date") int max_upload_date,
			@Param("reference") String reference, @Param("value_date") String value_date,
			@Param("branch_code") String branch_code);

	@Select("select distinct rac.id,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname  from rtgs_awb_core rac,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rac.file_id=fud.file_id and rac.amount between  #{min_amount} and #{max_amount}  and rac.upload_date between #{min_upload_date} and #{max_upload_date} and rm.match_date between #{min_match_date} and #{max_match_date} and rac.additional_information like '%'+ #{reference} + '%' and rac.value_date like '%'+#{value_date}+ '%' and rac.branch_code like '%'+#{branch_code}+'%'  and rm.match_id = rac.match_id and rac.availability = 1 and rac.status=1 union"
			...
			+ " select distinct sac.id ,sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sac.file_id=fud.file_id and sac.amount between  #{min_amount} and #{max_amount}  and sac.upload_date between #{min_upload_date} and #{max_upload_date} and sm.match_date between #{min_match_date} and #{max_match_date} and sac.additional_information like '%'+ #{reference} + '%' and sac.value_date like '%'+#{value_date}+ '%' and sac.branch_code like '%'+#{branch_code}+'%'  and sm.match_id = sac.match_id and sac.availability = 1 and sac.status=1 union"
			+ " select distinct cr.id ,cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.upload_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr, users u,file_user_date fud  where  u.id =fud.user_id and cr.file_id=fud.file_id and cr.amount between  #{min_amount} and #{max_amount}  and cr.upload_date between #{min_upload_date} and #{max_upload_date} and cr.match_date between #{min_match_date} and #{max_match_date} and cr.additional_information like '%'+ #{reference} + '%' and cr.value_date like '%'+#{value_date}+ '%' and cr.branch_code like '%'+#{branch_code}+'%'  and cr.availability = 1 and cr.status=1 union"
			+ " select distinct cpm.id ,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,partially_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.amount between  #{min_amount} and #{max_amount}  and cpm.upload_date between #{min_upload_date} and #{max_upload_date} and pm.match_date between #{min_match_date} and #{max_match_date} and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+#{value_date}+ '%' and cpm.branch_code like '%'+#{branch_code}+'%'  and pm.match_id = cpm.match_id and cpm.availability = 1 and cpm.status=1")
	public List<Transactionhistory> searchWithAmountUploadDateAndMatchDateCore(@Param("min_amount") float min_amount,
			@Param("max_amount") float max_amount, @Param("min_upload_date") int min_upload_date,
			@Param("max_upload_date") int max_upload_date, @Param("min_match_date") int min_match_date,
			@Param("max_match_date") int max_match_date, @Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	@Select("select distinct rac.id,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname  from rtgs_awb_core rac,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rac.file_id=fud.file_id and rac.amount >=  #{min_amount} and rac.upload_date between #{min_upload_date} and #{max_upload_date} and rm.match_date between #{min_match_date} and #{max_match_date} and rac.additional_information like '%'+ #{reference} + '%' and rac.value_date like '%'+#{value_date}+ '%' and rac.branch_code like '%'+#{branch_code}+'%'  and rm.match_id = rac.match_id and rac.availability = 1 and rac.status=1 union"
			...
			+ " select distinct sac.id ,sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sac.file_id=fud.file_id and sac.amount >=  #{min_amount}  and sac.upload_date between #{min_upload_date} and #{max_upload_date} and sm.match_date between #{min_match_date} and #{max_match_date} and sac.additional_information like '%'+ #{reference} + '%' and sac.value_date like '%'+#{value_date}+ '%' and sac.branch_code like '%'+#{branch_code}+'%'  and sm.match_id = sac.match_id and sac.availability = 1 and sac.status=1 union"
			+ " select distinct cr.id ,cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.upload_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr, users u,file_user_date fud  where  u.id =fud.user_id and cr.file_id=fud.file_id and cr.amount >=  #{min_amount}  and cr.upload_date between #{min_upload_date} and #{max_upload_date} and cr.match_date between #{min_match_date} and #{max_match_date} and cr.additional_information like '%'+ #{reference} + '%' and cr.value_date like '%'+#{value_date}+ '%' and cr.branch_code like '%'+#{branch_code}+'%' and cr.availability = 1 and cr.status=1 union"
			+ " select distinct cpm.id ,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,partially_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.amount >=  #{min_amount}  and cpm.upload_date between #{min_upload_date} and #{max_upload_date} and pm.match_date between #{min_match_date} and #{max_match_date} and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+#{value_date}+ '%' and cpm.branch_code like '%'+#{branch_code}+'%'  and pm.match_id = cpm.match_id and cpm.availability = 1 and cpm.status=1")
	public List<Transactionhistory> searchWithMinAmountUploadDateAndMatchDateCore(@Param("min_amount") float min_amount,
			@Param("min_upload_date") int min_upload_date, @Param("max_upload_date") int max_upload_date,
			@Param("min_match_date") int min_match_date, @Param("max_match_date") int max_match_date,
			@Param("reference") String reference, @Param("value_date") String value_date,
			@Param("branch_code") String branch_code);

	@Select("select distinct rac.id,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname  from rtgs_awb_core rac,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rac.file_id=fud.file_id and rac.amount <=  #{max_amount} and rac.upload_date between #{min_upload_date} and #{max_upload_date} and rm.match_date between #{min_match_date} and #{max_match_date} and rac.additional_information like '%'+ #{reference} + '%' and rac.value_date like '%'+#{value_date}+ '%' and rac.branch_code like '%'+#{branch_code}+'%'  and rm.match_id = rac.match_id and rac.availability = 1 and rac.status=1 union"
			...
			+ " select distinct sac.id ,sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sac.file_id=fud.file_id and sac.amount <=  #{max_amount}  and sac.upload_date between #{min_upload_date} and #{max_upload_date} and sm.match_date between #{min_match_date} and #{max_match_date} and sac.additional_information like '%'+ #{reference} + '%' and sac.value_date like '%'+#{value_date}+ '%' and sac.branch_code like '%'+#{branch_code}+'%'  and sm.match_id = sac.match_id and sac.availability = 1 and sac.status=1 union"
			+ " select distinct cr.id ,cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.upload_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr, users u,file_user_date fud  where  u.id =fud.user_id and cr.file_id=fud.file_id and cr.amount <=  #{max_amount}  and cr.upload_date between #{min_upload_date} and #{max_upload_date} and cr.match_date between #{min_match_date} and #{max_match_date} and cr.additional_information like '%'+ #{reference} + '%' and cr.value_date like '%'+#{value_date}+ '%' and cr.branch_code like '%'+#{branch_code}+'%' and cr.availability = 1 and cr.status=1 union"
			+ " select distinct cpm.id ,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,partially_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.amount <=  #{max_amount}  and cpm.upload_date between #{min_upload_date} and #{max_upload_date} and pm.match_date between #{min_match_date} and #{max_match_date} and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+#{value_date}+ '%' and cpm.branch_code like '%'+#{branch_code}+'%'  and pm.match_id = cpm.match_id and cpm.availability = 1 and cpm.status=1")
	public List<Transactionhistory> searchWithMaxAmountUploadDateAndMatchDateCore(@Param("max_amount") float max_amount,
			@Param("min_upload_date") int min_upload_date, @Param("max_upload_date") int max_upload_date,
			@Param("min_match_date") int min_match_date, @Param("max_match_date") int max_match_date,
			@Param("reference") String reference, @Param("value_date") String value_date,
			@Param("branch_code") String branch_code);

	@Select("select distinct rac.id ,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname  from rtgs_awb_core rac,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rac.file_id=fud.file_id and  rac.upload_date between #{min_upload_date} and #{max_upload_date} and rm.match_date between #{min_match_date} and #{max_match_date} and rac.additional_information like '%'+ #{reference} + '%' and rac.value_date like '%'+#{value_date}+ '%' and rac.branch_code like '%'+#{branch_code}+'%'  and rm.match_id = rac.match_id and rac.availability = 1 and rac.status=1 union"
			...
			+ " select distinct sac.id ,sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sac.file_id=fud.file_id and  sac.upload_date between #{min_upload_date} and #{max_upload_date} and sm.match_date between #{min_match_date} and #{max_match_date} and sac.additional_information like '%'+ #{reference} + '%' and sac.value_date like '%'+#{value_date}+ '%' and sac.branch_code like '%'+#{branch_code}+'%'  and sm.match_id = sac.match_id and sac.availability = 1 and sac.status=1 union"
			+ " select distinct cr.id ,cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.upload_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr, users u,file_user_date fud  where  u.id =fud.user_id and cr.file_id=fud.file_id and  cr.upload_date between #{min_upload_date} and #{max_upload_date} and cr.match_date between #{min_match_date} and #{max_match_date} and cr.additional_information like '%'+ #{reference} + '%' and cr.value_date like '%'+#{value_date}+ '%' and cr.branch_code like '%'+#{branch_code}+'%' and cr.availability = 1 and cr.status=1 union"
			+ " select distinct cpm.id ,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,partially_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and cpm.file_id=fud.file_id and  cpm.upload_date between #{min_upload_date} and #{max_upload_date} and pm.match_date between #{min_match_date} and #{max_match_date} and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+#{value_date}+ '%' and cpm.branch_code like '%'+#{branch_code}+'%'  and pm.match_id = cpm.match_id and cpm.availability = 1 and cpm.status=1")
	public List<Transactionhistory> searchWithUploadDateAndMatchDateCore(@Param("min_upload_date") int min_upload_date,
			@Param("max_upload_date") int max_upload_date, @Param("min_match_date") int min_match_date,
			@Param("max_match_date") int max_match_date, @Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	@Select("select distinct rac.id ,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname  from rtgs_awb_core rac,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rac.file_id=fud.file_id and rac.amount between #{min_amount} and #{max_amount} and rm.match_date between #{min_match_date} and #{max_match_date} and rac.additional_information like '%'+ #{reference} + '%' and rac.value_date like '%'+#{value_date}+ '%' and rac.branch_code like '%'+#{branch_code}+'%'  and rm.match_id = rac.match_id and rac.availability = 1 and rac.status=1 union"
			...
			+ " select distinct sac.id,sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sac.file_id=fud.file_id and sac.amount between #{min_amount} and #{max_amount} and sm.match_date between #{min_match_date} and #{max_match_date} and sac.additional_information like '%'+ #{reference} + '%' and sac.value_date like '%'+#{value_date}+ '%' and sac.branch_code like '%'+#{branch_code}+'%'  and sm.match_id = sac.match_id and sac.availability = 1 and sac.status=1 union"
			+ " select distinct cr.id,cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.upload_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr, users u,file_user_date fud  where  u.id =fud.user_id and cr.file_id=fud.file_id and cr.amount between #{min_amount} and #{max_amount} and cr.match_date between #{min_match_date} and #{max_match_date} and cr.additional_information like '%'+ #{reference} + '%' and cr.value_date like '%'+#{value_date}+ '%' and cr.branch_code like '%'+#{branch_code}+'%' and cr.availability = 1 and cr.status=1 union"
			+ " select distinct cpm.id,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,partially_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.amount between #{min_amount} and #{max_amount} and pm.match_date between #{min_match_date} and #{max_match_date} and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+#{value_date}+ '%' and cpm.branch_code like '%'+#{branch_code}+'%'  and pm.match_id = cpm.match_id and cpm.availability = 1 and cpm.status=1")
	public List<Transactionhistory> searchWithAmountAndMatchDateCore(@Param("min_amount") float min_amount,
			@Param("max_amount") float max_amount, @Param("min_match_date") int min_match_date,
			@Param("max_match_date") int max_match_date, @Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	@Select("select distinct rac.id ,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname  from rtgs_awb_core rac,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rac.file_id=fud.file_id and rac.amount >= #{min_amount} and rm.match_date between #{min_match_date} and #{max_match_date} and rac.additional_information like '%'+ #{reference} + '%' and rac.value_date like '%'+#{value_date}+ '%' and rac.branch_code like '%'+#{branch_code}+'%'  and rm.match_id = rac.match_id and rac.availability = 1 and rac.status=1 union"
			...
			+ " select distinct sac.id,sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sac.file_id=fud.file_id and sac.amount >= #{min_amount} and sm.match_date between #{min_match_date} and #{max_match_date} and sac.additional_information like '%'+ #{reference} + '%' and sac.value_date like '%'+#{value_date}+ '%' and sac.branch_code like '%'+#{branch_code}+'%'  and sm.match_id = sac.match_id and sac.availability = 1 and sac.status=1 union"
			+ " select distinct cr.id,cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.upload_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr, users u,file_user_date fud  where  u.id =fud.user_id and cr.file_id=fud.file_id and cr.amount >= #{min_amount} and cr.match_date between #{min_match_date} and #{max_match_date} and cr.additional_information like '%'+ #{reference} + '%' and cr.value_date like '%'+#{value_date}+ '%' and cr.branch_code like '%'+#{branch_code}+'%' and cr.availability = 1 and cr.status=1 union"
			+ " select distinct cpm.id,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,partially_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.amount >= #{min_amount} and pm.match_date between #{min_match_date} and #{max_match_date} and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+#{value_date}+ '%' and cpm.branch_code like '%'+#{branch_code}+'%'  and pm.match_id = cpm.match_id and cpm.availability = 1 and cpm.status=1")
	public List<Transactionhistory> searchWithMinAmountAndMatchDateCore(@Param("min_amount") float min_amount,
			@Param("min_match_date") int min_match_date, @Param("max_match_date") int max_match_date,
			@Param("reference") String reference, @Param("value_date") String value_date,
			@Param("branch_code") String branch_code);

	@Select("select distinct rac.id ,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname  from rtgs_awb_core rac,rtgs_matched rm, users u,file_user_date fud  where  u.id =fud.user_id and rac.file_id=fud.file_id and rac.amount <= #{max_amount} and rm.match_date between #{min_match_date} and #{max_match_date} and rac.additional_information like '%'+ #{reference} + '%' and rac.value_date like '%'+#{value_date}+ '%' and rac.branch_code like '%'+#{branch_code}+'%'  and rm.match_id = rac.match_id and rac.availability = 1 and rac.status=1 union"
			...
			+ " select distinct sac.id,sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,sos_matched sm, users u,file_user_date fud  where  u.id =fud.user_id and sac.file_id=fud.file_id and sac.amount <= #{max_amount} and sm.match_date between #{min_match_date} and #{max_match_date} and sac.additional_information like '%'+ #{reference} + '%' and sac.value_date like '%'+#{value_date}+ '%' and sac.branch_code like '%'+#{branch_code}+'%'  and sm.match_id = sac.match_id and sac.availability = 1 and sac.status=1 union"
			+ " select distinct cr.id,cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.upload_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr, users u,file_user_date fud  where  u.id =fud.user_id and cr.file_id=fud.file_id and cr.amount <= #{max_amount} and cr.match_date between #{min_match_date} and #{max_match_date} and cr.additional_information like '%'+ #{reference} + '%' and cr.value_date like '%'+#{value_date}+ '%' and cr.branch_code like '%'+#{branch_code}+'%' and cr.availability = 1 and cr.status=1 union"
			+ " select distinct cpm.id,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,partially_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.amount <= #{max_amount} and pm.match_date between #{min_match_date} and #{max_match_date} and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+#{value_date}+ '%' and cpm.branch_code like '%'+#{branch_code}+'%'  and pm.match_id = cpm.match_id and cpm.availability = 1 and cpm.status=1")
	public List<Transactionhistory> searchWithMaxAmountAndMatchDateCore(@Param("max_amount") float max_amount,
			@Param("min_match_date") int min_match_date, @Param("max_match_date") int max_match_date,
			@Param("reference") String reference, @Param("value_date") String value_date,
			@Param("branch_code") String branch_code);

	@Select("select dna.id,dna.additional_information,dna.amount,dna.dr_cr,dna.match_status,dna.REFERENCE,dna.sender,dna.receiver,dna.upload_date,dna.value_date_type,u.firstname,u.lastname  from data_nbe_ats dna,users u,file_user_date fud  where  u.id =fud.user_id and dna.file_id=fud.file_id  and dna.REFERENCE like '%'+ #{reference}+ '%'  and dna.status = 1 and dna.availability = 1  union"
			+ " select rna.id,rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname  from rtgs_nbe_ats rna,users u,file_user_date fud, rtgs_matched rm  where  u.id =fud.user_id and rna.file_id=fud.file_id and rna.REFERENCE like '%'+ #{reference}+ '%' and rm.rtgs_nbe_ats_id = rna.id and rna.availability =1 and rna.status =1  union"
			+ " select bna.id,bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna,users u,file_user_date fud, b2b_matched bm  where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.REFERENCE like '%'+ #{reference}+ '%' and bm.b2b_nbe_ats_id = bna.id and bna.availability =1 and bna.status =1  union"
			+ " select ena.id, ena.additional_information,ena.amount,ena.dr_cr,ena.match_status,ena.REFERENCE,ena.sender,ena.receiver,ena.upload_date,ena.value_date_type,u.firstname,u.lastname  from erca_nbe_ats ena,users u,file_user_date fud, erca_matched em  where  u.id =fud.user_id and ena.file_id=fud.file_id and ena.REFERENCE like '%'+ #{reference}+ '%' and em.erca_nbe_ats_id = ena.id and ena.availability =1 and ena.status =1 union"
			+ " select sna.id, sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna,users u,file_user_date fud, sos_matched sm  where  u.id =fud.user_id and sna.file_id=fud.file_id  and sna.REFERENCE like '%'+ #{reference}+ '%' and sm.sos_nbe_ats_id = sna.id and sna.availability =1 and sna.status =1 union"
	...
	@Select("select dna.id,dna.additional_information,dna.amount,dna.dr_cr,dna.match_status,dna.REFERENCE,dna.sender,dna.receiver,dna.upload_date,dna.value_date_type, u.firstname,u.lastname  from data_nbe_ats dna,users u,file_user_date fud  where  u.id =fud.user_id and dna.file_id=fud.file_id  and dna.value_date_type like '%'+ #{value_date}+ '%' and  dna.status = 1 and dna.availability = 1  union"
			+ " select rna.id,rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type, u.firstname,u.lastname  from rtgs_nbe_ats rna,users u,file_user_date fud, rtgs_matched rm  where  u.id =fud.user_id and rna.file_id=fud.file_id and rna.value_date_type like '%'+ #{value_date}+ '%' and rm.rtgs_nbe_ats_id = rna.id and rna.availability =1 and rna.status =1  union"
			+ " select bna.id,bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type, u.firstname,u.lastname  from b2b_nbe_ats bna,users u,file_user_date fud, b2b_matched bm  where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.value_date_type like '%'+ #{value_date}+ '%' and bm.b2b_nbe_ats_id = bna.id and bna.availability =1 and bna.status =1  union"
			+ " select ena.id, ena.additional_information,ena.amount,ena.dr_cr,ena.match_status,ena.REFERENCE,ena.sender,ena.receiver,ena.upload_date,ena.value_date_type, u.firstname,u.lastname  from erca_nbe_ats ena,users u,file_user_date fud, erca_matched em  where  u.id =fud.user_id and ena.file_id=fud.file_id and ena.value_date_type like '%'+ #{value_date}+ '%' and em.erca_nbe_ats_id = ena.id and ena.availability =1 and ena.status =1 union"
			+ " select sna.id, sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type, u.firstname,u.lastname  from sos_nbe_ats sna,users u,file_user_date fud, sos_matched sm  where  u.id =fud.user_id and sna.file_id=fud.file_id  and sna.value_date_type like '%'+ #{value_date}+ '%' and sm.sos_nbe_ats_id = sna.id and sna.availability =1 and sna.status =1 union"
	...
	@Select("select dna.id,dna.additional_information,dna.amount,dna.dr_cr,dna.match_status,dna.REFERENCE,dna.sender,dna.receiver,dna.upload_date,dna.value_date_type,u.firstname,u.lastname  from data_nbe_ats dna,users u,file_user_date fud  where  u.id =fud.user_id and dna.file_id=fud.file_id  and dna.REFERENCE like '%'+ #{reference}+ '%' and dna.value_date_type like '%'+ #{value_date}+ '%' and  dna.status = 1 and dna.availability = 1  union"
			+ " select rna.id,rna.additional_information,rna.amount,rna.dr_cr,rna.match_status,rna.REFERENCE,rna.sender,rna.receiver,rna.upload_date,rna.value_date_type,u.firstname,u.lastname  from rtgs_nbe_ats rna,users u,file_user_date fud, rtgs_matched rm  where  u.id =fud.user_id and rna.file_id=fud.file_id and rna.REFERENCE like '%'+ #{reference}+ '%' and rna.value_date_type like '%'+ #{value_date}+ '%' and rm.rtgs_nbe_ats_id = rna.id and rna.availability =1 and rna.status =1  union"
			+ " select bna.id,bna.additional_information,bna.amount,bna.dr_cr,bna.match_status,bna.REFERENCE,bna.sender,bna.receiver,bna.upload_date,bna.value_date_type,u.firstname,u.lastname  from b2b_nbe_ats bna,users u,file_user_date fud, b2b_matched bm  where  u.id =fud.user_id and bna.file_id=fud.file_id and bna.REFERENCE like '%'+ #{reference}+ '%' and bna.value_date_type like '%'+ #{value_date}+ '%' and bm.b2b_nbe_ats_id = bna.id and bna.availability =1 and bna.status =1  union"
			+ " select ena.id, ena.additional_information,ena.amount,ena.dr_cr,ena.match_status,ena.REFERENCE,ena.sender,ena.receiver,ena.upload_date,ena.value_date_type,u.firstname,u.lastname  from erca_nbe_ats ena,users u,file_user_date fud, erca_matched em  where  u.id =fud.user_id and ena.file_id=fud.file_id and ena.REFERENCE like '%'+ #{reference}+ '%' and ena.value_date_type like '%'+ #{value_date}+ '%' and em.erca_nbe_ats_id = ena.id and ena.availability =1 and ena.status =1 union"
			+ " select sna.id, sna.additional_information,sna.amount,sna.dr_cr,sna.match_status,sna.REFERENCE,sna.sender,sna.receiver,sna.upload_date,sna.value_date_type,u.firstname,u.lastname  from sos_nbe_ats sna,users u,file_user_date fud, sos_matched sm  where  u.id =fud.user_id and sna.file_id=fud.file_id  and sna.REFERENCE like '%'+ #{reference}+ '%' and sna.value_date_type like '%'+ #{value_date}+ '%' and sm.sos_nbe_ats_id = sna.id and sna.availability =1 and sna.status =1 union"
	...
	@Select("select distinct dac.id,dac.additional_information,dac.amount,dac.dr_cr,dac.match_status,dac.transaction_reference,dac.branch_code,dac.value_date,dac.upload_date,dac.posting_date,u.firstname,u.lastname  from data_awb_core dac,users u,file_user_date fud  where  u.id =fud.user_id and dac.file_id=fud.file_id and dac.additional_information like '%'+ #{reference} + '%' and  dac.status = 1 and dac.availability = 1 union"
			+ " select distinct rac.id,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname from rtgs_awb_core rac,users u,file_user_date fud,rtgs_matched rm where  u.id =fud.user_id and rac.file_id=fud.file_id  and rac.additional_information like '%'+ #{reference} + '%'  and rm.match_id = rac.match_id and rac.availability=1 and rac.status=1 union"
			+ " select distinct bac.id, bac.additional_information,bac.amount,bac.dr_cr,bac.match_status,bac.transaction_reference,bac.branch_code,bac.value_date,bac.upload_date,bac.posting_date,u.firstname,u.lastname  from b2b_awb_core bac,users u,file_user_date fud,b2b_matched bm  where  u.id =fud.user_id and bac.file_id=fud.file_id  and bac.additional_information like '%'+ #{reference} + '%'  and bm.match_id = bac.match_id and  bac.availability=1 and bac.status=1 union"
			...
			+ " select distinct sac.id, sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,users u,file_user_date fud,sos_matched sm  where  u.id =fud.user_id and sac.file_id=fud.file_id  and sac.additional_information like '%'+ #{reference} + '%'  and sm.match_id = sac.match_id and  sac.availability=1 and sac.status=1 union"
			+ " select distinct cr.id, cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.upload_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr,users u,file_user_date fud  where  u.id =fud.user_id and cr.file_id=fud.file_id  and cr.additional_information like '%'+ #{reference} + '%' and  cr.availability=1 and cr.status=1 union"
			+ " select distinct cpm.id,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,users u,file_user_date fud,partially_matched pm  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.additional_information like '%'+ #{reference} + '%' and pm.match_id = cpm.match_id and  cpm.availability=1 and cpm.status=1")
	List<Transactionhistory> searchWithReferenceCore(String reference);

	@Select("select distinct dac.id,dac.additional_information,dac.amount,dac.dr_cr,dac.match_status,dac.transaction_reference,dac.branch_code,dac.value_date,dac.upload_date,dac.posting_date,u.firstname,u.lastname  from data_awb_core dac,users u,file_user_date fud  where  u.id =fud.user_id and dac.file_id=fud.file_id and dac.value_date like '%'+ #{value_date} + '%' and  dac.status = 1 and dac.availability = 1 union"
			+ " select distinct rac.id,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname from rtgs_awb_core rac,users u,file_user_date fud ,rtgs_matched rm where  u.id =fud.user_id and rac.file_id=fud.file_id  and rac.value_date like '%'+ #{value_date} + '%' and rm.match_id = rac.match_id and rac.availability=1 and rac.status=1 union"
			+ " select distinct bac.id, bac.additional_information,bac.amount,bac.dr_cr,bac.match_status,bac.transaction_reference,bac.branch_code,bac.value_date,bac.upload_date,bac.posting_date,u.firstname,u.lastname  from b2b_awb_core bac,users u,file_user_date fud ,b2b_matched bm where  u.id =fud.user_id and bac.file_id=fud.file_id  and bac.value_date like '%'+ #{value_date} + '%' and bm.match_id = bac.match_id and  bac.availability=1 and bac.status=1 union"
			...
			+ " select distinct sac.id, sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,users u,file_user_date fud,sos_matched sm  where  u.id =fud.user_id and sac.file_id=fud.file_id  and sac.value_date like '%'+ #{value_date} + '%' and sm.match_id = sac.match_id and  sac.availability=1 and sac.status=1 union"
			+ " select distinct cr.id, cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.upload_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr,users u,file_user_date fud where  u.id =fud.user_id and cr.file_id=fud.file_id  and cr.posting_date like '%'+ #{value_date} + '%' and  cr.availability=1 and cr.status=1 union"
			+ " select distinct cpm.id,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,users u,file_user_date fud,partially_matched pm  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.value_date like '%'+ #{value_date} + '%' and pm.match_id = cpm.match_id and  cpm.availability=1 and cpm.status=1")
	List<Transactionhistory> searchWithValueDateCore(String value_date);

	@Select("select distinct dac.id,dac.additional_information,dac.amount,dac.dr_cr,dac.match_status,dac.transaction_reference,dac.branch_code,dac.value_date,dac.upload_date,dac.posting_date,u.firstname,u.lastname  from data_awb_core dac,users u,file_user_date fud  where  u.id =fud.user_id and dac.file_id=fud.file_id and dac.branch_code like '%'+ #{branch_code} + '%'  and dac.status = 1 and dac.availability = 1 union"
			+ " select distinct rac.id,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname from rtgs_awb_core rac,users u,file_user_date fud,rtgs_matched rm where  u.id =fud.user_id and rac.file_id=fud.file_id  and rac.branch_code like '%'+ #{branch_code} + '%' and rm.match_id = rac.match_id and rac.availability=1 and rac.status=1 union"
			+ " select distinct bac.id, bac.additional_information,bac.amount,bac.dr_cr,bac.match_status,bac.transaction_reference,bac.branch_code,bac.value_date,bac.upload_date,bac.posting_date,u.firstname,u.lastname  from b2b_awb_core bac,users u,file_user_date fud,b2b_matched bm  where  u.id =fud.user_id and bac.file_id=fud.file_id  and bac.branch_code like '%'+ #{branch_code} + '%' and bm.match_id = bac.match_id and  bac.availability=1 and bac.status=1 union"
			...
			+ " select distinct sac.id, sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,users u,file_user_date fud,sos_matched sm  where  u.id =fud.user_id and sac.file_id=fud.file_id  and sac.branch_code like '%'+ #{branch_code} + '%' and sm.match_id = sac.match_id and  sac.availability=1 and sac.status=1 union"
			+ " select distinct cpm.id,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,users u,file_user_date fud,partially_matched pm  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.branch_code like '%'+ #{branch_code} + '%' and pm.match_id = cpm.match_id and  cpm.availability=1 and cpm.status=1")
	List<Transactionhistory> searchWithBranchCodeCore(String branch_code);

	@Select("select distinct dac.id,dac.additional_information,dac.amount,dac.dr_cr,dac.match_status,dac.transaction_reference,dac.branch_code,dac.value_date,dac.upload_date,dac.posting_date,u.firstname,u.lastname  from data_awb_core dac,users u,file_user_date fud  where  u.id =fud.user_id and dac.file_id=fud.file_id and dac.branch_code like '%'+ #{branch_code} + '%'  and dac.additional_information like '%'+ #{reference} + '%'  and dac.value_date like '%'+ #{value_date} + '%' and  dac.status = 1 and dac.availability = 1 union"
			+ " select distinct rac.id,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname from rtgs_awb_core rac,users u,file_user_date fud ,rtgs_matched rm where  u.id =fud.user_id and rac.file_id=fud.file_id  and rac.branch_code like '%'+ #{branch_code} + '%' and rac.additional_information like '%'+ #{reference} + '%'  and rac.value_date like '%'+ #{value_date} + '%' and rm.match_id = rac.match_id and rac.availability=1 and rac.status=1 union"
			+ " select distinct bac.id, bac.additional_information,bac.amount,bac.dr_cr,bac.match_status,bac.transaction_reference,bac.branch_code,bac.value_date,bac.upload_date,bac.posting_date,u.firstname,u.lastname  from b2b_awb_core bac,users u,file_user_date fud ,b2b_matched bm where  u.id =fud.user_id and bac.file_id=fud.file_id  and bac.branch_code like '%'+ #{branch_code} + '%' and bac.additional_information like '%'+ #{reference} + '%'  and bac.value_date like '%'+ #{value_date} + '%'  and bm.match_id = bac.match_id and  bac.availability=1 and bac.status=1 union"
			...
			+ " select distinct sac.id, sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,users u,file_user_date fud,sos_matched sm  where  u.id =fud.user_id and sac.file_id=fud.file_id  and sac.branch_code like '%'+ #{branch_code} + '%' and sac.additional_information like '%'+ #{reference} + '%'  and sac.value_date like '%'+ #{value_date} + '%'  and sm.match_id = sac.match_id and  sac.availability=1 and sac.status=1 union"
			+ " select distinct cpm.id,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,users u,file_user_date fud,partially_matched pm  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.branch_code like '%'+ #{branch_code} + '%' and cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+ #{value_date} + '%' and pm.match_id = cpm.match_id and  cpm.availability=1 and cpm.status=1")
	List<Transactionhistory> searchWithRefValueDateBranchCode(@Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	@Select("select distinct dac.id,dac.additional_information,dac.amount,dac.dr_cr,dac.match_status,dac.transaction_reference,dac.branch_code,dac.value_date,dac.upload_date,dac.posting_date,u.firstname,u.lastname  from data_awb_core dac,users u,file_user_date fud  where  u.id =fud.user_id and dac.file_id=fud.file_id  and dac.additional_information like '%'+ #{reference} + '%'  and dac.value_date like '%'+ #{value_date} + '%' and  dac.status = 1 and dac.availability = 1 union"
			+ " select distinct rac.id,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname from rtgs_awb_core rac,users u,file_user_date fud,rtgs_matched rm where  u.id =fud.user_id and rac.file_id=fud.file_id  and rac.additional_information like '%'+ #{reference} + '%'  and rac.value_date like '%'+ #{value_date} + '%' and rm.match_id = rac.match_id and rac.availability=1 and rac.status=1 union"
			+ " select distinct bac.id, bac.additional_information,bac.amount,bac.dr_cr,bac.match_status,bac.transaction_reference,bac.branch_code,bac.value_date,bac.upload_date,bac.posting_date,u.firstname,u.lastname  from b2b_awb_core bac,users u,file_user_date fud,b2b_matched bm  where  u.id =fud.user_id and bac.file_id=fud.file_id  and bac.additional_information like '%'+ #{reference} + '%'  and bac.value_date like '%'+ #{value_date} + '%' and bm.match_id = bac.match_id and  bac.availability=1 and bac.status=1 union"
			...
			+ " select distinct sac.id, sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,users u,file_user_date fud,sos_matched sm  where  u.id =fud.user_id and sac.file_id=fud.file_id   and sac.additional_information like '%'+ #{reference} + '%'  and sac.value_date like '%'+ #{value_date} + '%' and sm.match_id = sac.match_id and  sac.availability=1 and sac.status=1 union"
			+ " select distinct cpm.id,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,users u,file_user_date fud,partially_matched pm  where  u.id =fud.user_id and cpm.file_id=fud.file_id and  cpm.additional_information like '%'+ #{reference} + '%' and cpm.value_date like '%'+ #{value_date} + '%' and pm.match_id = cpm.match_id and  cpm.availability=1 and cpm.status=1")
	List<Transactionhistory> searchWithValueDateAndReferenceCore(@Param("value_date") String value_date,
			@Param("reference") String reference);

	@Select("select distinct dac.id,dac.additional_information,dac.amount,dac.dr_cr,dac.match_status,dac.transaction_reference,dac.branch_code,dac.value_date,dac.upload_date,dac.posting_date,u.firstname,u.lastname  from data_awb_core dac,users u,file_user_date fud  where  u.id =fud.user_id and dac.file_id=fud.file_id and dac.branch_code like '%'+ #{branch_code} + '%'  and dac.value_date like '%'+ #{value_date} + '%' and  dac.status = 1 and dac.availability = 1 union"
			+ " select distinct rac.id,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname from rtgs_awb_core rac,users u,file_user_date fud,rtgs_matched rm where  u.id =fud.user_id and rac.file_id=fud.file_id  and rac.branch_code like '%'+ #{branch_code} + '%'  and rac.value_date like '%'+ #{value_date} + '%' and rm.match_id = rac.match_id and rac.availability=1 and rac.status=1 union"
			+ " select distinct bac.id, bac.additional_information,bac.amount,bac.dr_cr,bac.match_status,bac.transaction_reference,bac.branch_code,bac.value_date,bac.upload_date,bac.posting_date,u.firstname,u.lastname  from b2b_awb_core bac,users u,file_user_date fud,b2b_matched bm  where  u.id =fud.user_id and bac.file_id=fud.file_id  and bac.branch_code like '%'+ #{branch_code} + '%'  and bac.value_date like '%'+ #{value_date} + '%' and bm.match_id = bac.match_id and  bac.availability=1 and bac.status=1 union"
			...
			+ " select distinct sac.id, sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,users u,file_user_date fud,sos_matched sm  where  u.id =fud.user_id and sac.file_id=fud.file_id  and sac.branch_code like '%'+ #{branch_code} + '%'  and sac.value_date like '%'+ #{value_date} + '%' and sm.match_id = sac.match_id and  sac.availability=1 and sac.status=1 union"
			+ " select distinct cpm.id,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,users u,file_user_date fud,partially_matched pm  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.branch_code like '%'+ #{branch_code} + '%' and cpm.value_date like '%'+ #{value_date} + '%' and pm.match_id = cpm.match_id and  cpm.availability=1 and cpm.status=1")
	List<Transactionhistory> searchWithValueDateAndBranchCodeCore(@Param("value_date") String value_date,
			@Param("branch_code") String branch_code);

	@Select("select distinct dac.id,dac.additional_information,dac.amount,dac.dr_cr,dac.match_status,dac.transaction_reference,dac.branch_code,dac.value_date,dac.upload_date,dac.posting_date,u.firstname,u.lastname  from data_awb_core dac,users u,file_user_date fud  where  u.id =fud.user_id and dac.file_id=fud.file_id and dac.branch_code like '%'+ #{branch_code} + '%'  and dac.additional_information like '%'+ #{reference} + '%' and dac.status = 1 and dac.availability = 1  union"
			+ " select distinct rac.id,rac.additional_information,rac.amount,rac.dr_cr,rac.match_status,rac.transaction_reference,rac.branch_code,rac.value_date,rac.upload_date,rac.posting_date,u.firstname,u.lastname from rtgs_awb_core rac,users u,file_user_date fud,rtgs_matched rm where  u.id =fud.user_id and rac.file_id=fud.file_id  and rac.branch_code like '%'+ #{branch_code} + '%' and rac.additional_information like '%'+ #{reference} + '%' and rm.match_id = rac.match_id and rac.availability=1 and rac.status=1 union"
			+ " select distinct bac.id, bac.additional_information,bac.amount,bac.dr_cr,bac.match_status,bac.transaction_reference,bac.branch_code,bac.value_date,bac.upload_date,bac.posting_date,u.firstname,u.lastname  from b2b_awb_core bac,users u,file_user_date fud,b2b_matched bm  where  u.id =fud.user_id and bac.file_id=fud.file_id  and bac.branch_code like '%'+ #{branch_code} + '%' and bac.additional_information like '%'+ #{reference} + '%' and bm.match_id = bac.match_id  and  bac.availability=1 and bac.status=1 union"
			...
			+ " select distinct sac.id, sac.additional_information,sac.amount,sac.dr_cr,sac.match_status,sac.transaction_reference,sac.branch_code,sac.value_date,sac.upload_date,sac.posting_date,u.firstname,u.lastname  from sos_awb_core sac,users u,file_user_date fud ,sos_matched sm where  u.id =fud.user_id and sac.file_id=fud.file_id  and sac.branch_code like '%'+ #{branch_code} + '%' and sac.additional_information like '%'+ #{reference} + '%' and sm.match_id = sac.match_id   and  sac.availability=1 and sac.status=1 union"
			+ " select distinct cr.id, cr.additional_information,cr.amount,cr.dr_cr,cr.match_status,cr.transaction_reference,cr.branch_code,cr.value_date,cr.upload_date,cr.posting_date,u.firstname,u.lastname  from pas_awb_core_reversal cr,users u,file_user_date fud where  u.id =fud.user_id and cr.file_id=fud.file_id  and cr.branch_code like '%'+ #{branch_code} + '%' and cr.additional_information like '%'+ #{reference} + '%'  and  cr.availability=1 and cr.status=1 union"
			+ " select distinct cpm.id,cpm.additional_information,cpm.amount,cpm.dr_cr,cpm.match_status,cpm.transaction_reference,cpm.branch_code,cpm.value_date,cpm.upload_date,cpm.posting_date,u.firstname,u.lastname  from core_partially_matched cpm,users u,file_user_date fud,partially_matched pm  where  u.id =fud.user_id and cpm.file_id=fud.file_id and cpm.branch_code like '%'+ #{branch_code} + '%' and cpm.additional_information like '%'+ #{reference} + '%' and pm.match_id = cpm.match_id and  cpm.availability=1 and cpm.status=1")
	List<Transactionhistory> searchWithReferenceAndBranchCodeCore(@Param("reference") String reference,
			@Param("branch_code") String branch_code);
	// ====================================== receivable
	// ===================================
	...
	@Select("select distinct dp.id,dp.additional_information,dp.amount,dp.dr_cr,dp.match_status,dp.branch as branch_code,dp.value_date,dp.upload_date,dp.posting_date,u.firstname,u.lastname  from data_issue_core dp,users u,file_user_date fud where  u.id =fud.user_id and dp.file_id=fud.file_id and dp.amount  between #{min_amount} and #{max_amount}  and dp.additional_information like '%'+ #{reference} + '%' and dp.value_date like '%'+#{value_date}+ '%' and dp.branch like '%'+#{branch_code}+'%' and dp.availability=1 union "
			+ "select distinct pd.id, pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,users u,file_user_date fud, issue_matched pm  where  u.id =fud.user_id and pd.file_id=fud.file_id and pd.amount  between #{min_amount} and #{max_amount} and pd.additional_information like '%'+ #{reference} + '%' and pd.value_date like '%'+#{value_date}+ '%' and pd.branch like '%'+#{branch_code}+'%'  and pm.issue_core_id = pd.id and pd.availability = 1 and pd.status=1 ")
	public List<Transactionhistory> searchWithAmountIssueCore(@Param("min_amount") float min_amount,
			@Param("max_amount") float max_amount, @Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	@Select("select distinct dp.id,dp.additional_information,dp.amount,dp.dr_cr,dp.match_status,dp.branch as branch_code,dp.value_date,dp.upload_date,dp.posting_date,u.firstname,u.lastname  from data_issue_core dp,users u,file_user_date fud where  u.id =fud.user_id and dp.file_id=fud.file_id and dp.amount  >= #{min_amount} and dp.additional_information like '%'+ #{reference} + '%' and dp.value_date like '%'+#{value_date}+ '%' and dp.branch like '%'+#{branch_code}+'%' and dp.availability=1 union "
			+ "select distinct pd.id, pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,users u,file_user_date fud, issue_matched pm  where  u.id =fud.user_id and pd.file_id=fud.file_id and pd.amount  >= #{min_amount} and pd.additional_information like '%'+ #{reference} + '%' and pd.value_date like '%'+#{value_date}+ '%' and pd.branch like '%'+#{branch_code}+'%'  and pm.issue_core_id = pd.id and pd.availability = 1 and pd.status=1 ")
	public List<Transactionhistory> searchWithMinAmountIssueCore(@Param("min_amount") float min_amount,
			@Param("reference") String reference, @Param("value_date") String value_date,
			@Param("branch_code") String branch_code);

	@Select("select distinct dp.id,dp.additional_information,dp.amount,dp.dr_cr,dp.match_status,dp.branch as branch_code,dp.value_date,dp.upload_date,dp.posting_date,u.firstname,u.lastname  from data_issue_core dp,users u,file_user_date fud where  u.id =fud.user_id and dp.file_id=fud.file_id and dp.amount  <= #{max_amount} and dp.additional_information like '%'+ #{reference} + '%' and dp.value_date like '%'+#{value_date}+ '%' and dp.branch like '%'+#{branch_code}+'%' and dp.availability=1 union "
			+ "select distinct pd.id, pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,users u,file_user_date fud, issue_matched pm  where  u.id =fud.user_id and pd.file_id=fud.file_id and pd.amount  <= #{max_amount} and pd.additional_information like '%'+ #{reference} + '%' and pd.value_date like '%'+#{value_date}+ '%' and pd.branch like '%'+#{branch_code}+'%'  and pm.issue_core_id = pd.id and pd.availability = 1 and pd.status=1 ")
	public List<Transactionhistory> searchWithMaxAmountIssueCore(@Param("max_amount") float max_amount,
			@Param("reference") String reference, @Param("value_date") String value_date,
			@Param("branch_code") String branch_code);

	@Select("select distinct dp.id,dp.additional_information,dp.amount,dp.dr_cr,dp.match_status,dp.branch as branch_code,dp.value_date,dp.upload_date,dp.posting_date,u.firstname,u.lastname  from data_issue_core dp,users u,file_user_date fud  where  u.id =fud.user_id and dp.file_id=fud.file_id and dp.additional_information like '%'+ #{reference} + '%' and dp.availability=1 union "
			+ "select distinct pd.id, pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,users u,file_user_date fud,issue_matched pm  where  u.id =fud.user_id and pd.file_id=fud.file_id  and pd.additional_information like '%'+ #{reference} + '%'  and pm.issue_core_id = pd.id and  pd.availability=1 and pd.status=1 ")
	List<Transactionhistory> searchWithReferenceIssueCore(String reference);

	@Select("select distinct dp.id,dp.additional_information,dp.amount,dp.dr_cr,dp.match_status,dp.branch as branch_code,dp.value_date,dp.upload_date,dp.posting_date,u.firstname,u.lastname  from data_issue_core dp,users u,file_user_date fud  where  u.id =fud.user_id and dp.file_id=fud.file_id and dp.value_date like '%'+ #{value_date} + '%'  and dp.availability=1 union "
			+ "select distinct pd.id, pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,users u,file_user_date fud ,issue_matched pm where  u.id =fud.user_id and pd.file_id=fud.file_id  and pd.value_date like '%'+ #{value_date} + '%' and pm.issue_core_id = pd.id and  pd.availability=1 and pd.status=1 ")
	List<Transactionhistory> searchWithValueDateIssueCore(String value_date);

	@Select("select distinct dp.id,dp.additional_information,dp.amount,dp.dr_cr,dp.match_status,dp.branch as branch_code,dp.value_date,dp.upload_date,dp.posting_date,u.firstname,u.lastname  from data_issue_core dp,users u,file_user_date fud  where  u.id =fud.user_id and dp.file_id=fud.file_id and dp.branch like '%'+ #{branch_code} + '%' and dp.availability=1 union "
			+ "select distinct pd.id, pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,users u,file_user_date fud,issue_matched pm  where  u.id =fud.user_id and pd.file_id=fud.file_id  and pd.branch like '%'+ #{branch_code} + '%' and pm.issue_core_id = pd.id and  pd.availability=1 and pd.status=1 ")
	List<Transactionhistory> searchWithBranchCodeIssueCore(String branch_code);

	@Select("select distinct dp.id,dp.additional_information,dp.amount,dp.dr_cr,dp.match_status,dp.branch as branch_code,dp.value_date,dp.upload_date,dp.posting_date,u.firstname,u.lastname  from data_issue_core dp,users u,file_user_date fud  where  u.id =fud.user_id and dp.file_id=fud.file_id  and dp.additional_information like '%'+ #{reference} + '%'  and dp.value_date like '%'+ #{value_date} + '%' and dp.availability=1  union "
			+ "select distinct pd.id, pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,users u,file_user_date fud,issue_matched pm  where  u.id =fud.user_id and pd.file_id=fud.file_id  and pd.additional_information like '%'+ #{reference} + '%'  and pd.value_date like '%'+ #{value_date} + '%' and pm.issue_core_id = pd.id and  pd.availability=1 and pd.status=1 ")
	List<Transactionhistory> searchWithValueDateAndReferenceIssueCore(@Param("value_date") String value_date,
			@Param("reference") String reference);

	@Select("select distinct dp.id,dp.additional_information,dp.amount,dp.dr_cr,dp.match_status,dp.branch as branch_code,dp.value_date,dp.upload_date,dp.posting_date,u.firstname,u.lastname  from data_issue_core dp,users u,file_user_date fud  where  u.id =fud.user_id and dp.file_id=fud.file_id and dp.branch like '%'+ #{branch_code} + '%'  and dp.value_date like '%'+ #{value_date} + '%' and dp.availability=1  union "
			+ "select distinct pd.id, pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,users u,file_user_date fud,issue_matched pm  where  u.id =fud.user_id and pd.file_id=fud.file_id  and pd.branch like '%'+ #{branch_code} + '%'  and pd.value_date like '%'+ #{value_date} + '%' and pm.issue_core_id = pd.id and  pd.availability=1 and pd.status=1 ")
	List<Transactionhistory> searchWithValueDateAndBranchCodeIssueCore(@Param("value_date") String value_date,
			@Param("branch_code") String branch_code);

	@Select("select distinct dp.id,dp.additional_information,dp.amount,dp.dr_cr,dp.match_status,dp.branch as branch_code,dp.value_date,dp.upload_date,dp.posting_date,u.firstname,u.lastname  from data_issue_core dp,users u,file_user_date fud  where  u.id =fud.user_id and dp.file_id=fud.file_id and dp.branch like '%'+ #{branch_code} + '%'  and dp.additional_information like '%'+ #{reference} + '%'  and dp.value_date like '%'+ #{value_date} + '%'  and dp.availability=1 union "
			+ "select distinct pd.id, pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,users u,file_user_date fud ,issue_matched pm where  u.id =fud.user_id and pd.file_id=fud.file_id  and pd.branch like '%'+ #{branch_code} + '%' and pd.additional_information like '%'+ #{reference} + '%'  and pd.value_date like '%'+ #{value_date} + '%'  and pm.issue_core_id = pd.id and  pd.availability=1 and pd.status=1 ")
	List<Transactionhistory> searchWithRefValueDateBranchCodeIssueCore(@Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	@Select("select distinct dp.id,dp.additional_information,dp.amount,dp.dr_cr,dp.match_status,dp.branch as branch_code,dp.value_date,dp.upload_date,dp.posting_date,u.firstname,u.lastname  from data_issue_core dp,users u,file_user_date fud  where  u.id =fud.user_id and dp.file_id=fud.file_id and dp.branch like '%'+ #{branch_code} + '%'  and dp.additional_information like '%'+ #{reference} + '%' and dp.availability=1  union "
			+ "select distinct pd.id, pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,users u,file_user_date fud,issue_matched pm  where  u.id =fud.user_id and pd.file_id=fud.file_id  and pd.branch like '%'+ #{branch_code} + '%' and pd.additional_information like '%'+ #{reference} + '%' and pm.issue_core_id = pd.id  and  pd.availability=1 and pd.status=1 ")
	List<Transactionhistory> searchWithReferenceAndBranchCodeIssueCore(@Param("reference") String reference,
			@Param("branch_code") String branch_code);

	@Select("select dp.id,dp.additional_information,dp.amount,dp.dr_cr,dp.match_status,dp.branch as branch_code,dp.value_date,dp.upload_date,dp.posting_date,u.firstname,u.lastname  from data_issue_core dp,users u,file_user_date fud  where  u.id =fud.user_id and dp.file_id=fud.file_id  and dp.upload_date between #{min_upload_date} and #{max_upload_date} and dp.additional_information like '%'+ #{reference} + '%' and dp.value_date like '%'+#{value_date}+ '%' and dp.branch like '%'+#{branch_code}+'%' and dp.availability=1 union "
			+ "select pd.id,pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,users u,file_user_date fud, issue_matched pm  where  u.id =fud.user_id and pd.file_id=fud.file_id and pd.upload_date between #{min_upload_date} and #{max_upload_date} and pd.additional_information like '%'+ #{reference} + '%' and pd.value_date like '%'+#{value_date}+ '%' and pd.branch like '%'+#{branch_code}+'%'  and pm.issue_core_id = pd.id and pd.availability = 1 and pd.status=1 ")
	public List<Transactionhistory> searchWithUploadDateIssueCore(@Param("min_upload_date") int min_upload_date,
			@Param("max_upload_date") int max_upload_date, @Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	@Select("select pd.id,pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,issue_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and pd.file_id=fud.file_id and  pm.match_date between #{min_match_date} and #{max_match_date} and pd.additional_information like '%'+ #{reference} + '%' and pd.value_date like '%'+#{value_date}+ '%' and pd.branch like '%'+#{branch_code}+'%'  and pm.issue_core_id = pd.id and pd.availability = 1 and pd.status=1 ")
	public List<Transactionhistory> searchWithMatchDateIssueCore(@Param("min_match_date") int min_match_date,
			@Param("max_match_date") int max_match_date, @Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	@Select("select distinct dp.id, dp.additional_information,dp.amount,dp.dr_cr,dp.match_status,dp.branch as branch_code,dp.value_date,dp.upload_date,dp.posting_date,u.firstname,u.lastname  from data_issue_core dp, users u,file_user_date fud  where  u.id =fud.user_id and dp.file_id=fud.file_id and dp.amount  between #{min_amount} and  #{max_amount}  and dp.upload_date between #{min_upload_date} and #{max_upload_date} and dp.additional_information like '%'+ #{reference} + '%' and dp.value_date like '%'+#{value_date}+ '%' and dp.branch like '%'+#{branch_code}+'%' and dp.availability=1  union "
			+ "select distinct pd.id,pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd, users u,file_user_date fud, issue_matched pm  where  u.id =fud.user_id and pd.file_id=fud.file_id and pd.amount  between #{min_amount} and #{max_amount}  and pd.upload_date between #{min_upload_date} and #{max_upload_date} and pd.additional_information like '%'+ #{reference} + '%' and pd.value_date like '%'+#{value_date}+ '%' and pd.branch like '%'+#{branch_code}+'%'  and pm.issue_core_id = pd.id and pd.availability = 1 and pd.status=1 ")
	public List<Transactionhistory> searchWithAmountAndUploadDateIssueCore(@Param("min_amount") float min_amount,
			@Param("max_amount") float max_amount, @Param("min_upload_date") int min_upload_date,
			@Param("max_upload_date") int max_upload_date, @Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	@Select("select distinct dp.id, dp.additional_information,dp.amount,dp.dr_cr,dp.match_status,dp.branch as branch_code,dp.value_date,dp.upload_date,dp.posting_date,u.firstname,u.lastname  from data_issue_core dp, users u,file_user_date fud  where  u.id =fud.user_id and dp.file_id=fud.file_id and dp.amount  >= #{min_amount} and dp.upload_date between #{min_upload_date} and #{max_upload_date} and dp.additional_information like '%'+ #{reference} + '%' and dp.value_date like '%'+#{value_date}+ '%' and dp.branch like '%'+#{branch_code}+'%' and dp.availability=1 union "
			+ "select distinct pd.id,pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd, users u,file_user_date fud, issue_matched pm  where  u.id =fud.user_id and pd.file_id=fud.file_id and pd.amount  >= #{min_amount}  and pd.upload_date between #{min_upload_date} and #{max_upload_date} and pd.additional_information like '%'+ #{reference} + '%' and pd.value_date like '%'+#{value_date}+ '%' and pd.branch like '%'+#{branch_code}+'%'  and pm.issue_core_id = pd.id and pd.availability = 1 and pd.status=1 ")
	public List<Transactionhistory> searchWithMinAmountAndUploadDateIssueCore(@Param("min_amount") float min_amount,
			@Param("min_upload_date") int min_upload_date, @Param("max_upload_date") int max_upload_date,
			@Param("reference") String reference, @Param("value_date") String value_date,
			@Param("branch_code") String branch_code);

	@Select("select distinct dp.id, dp.additional_information,dp.amount,dp.dr_cr,dp.match_status,dp.branch as branch_code,dp.value_date,dp.upload_date,dp.posting_date,u.firstname,u.lastname  from data_issue_core dp, users u,file_user_date fud  where  u.id =fud.user_id and dp.file_id=fud.file_id and dp.amount  <= #{max_amount} and dp.upload_date between #{min_upload_date} and #{max_upload_date} and dp.additional_information like '%'+ #{reference} + '%' and dp.value_date like '%'+#{value_date}+ '%' and dp.branch like '%'+#{branch_code}+'%' and dp.availability=1  union "
			+ "select distinct pd.id,pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd, users u,file_user_date fud, issue_matched pm  where  u.id =fud.user_id and pd.file_id=fud.file_id and pd.amount  <= #{max_amount}  and pd.upload_date between #{min_upload_date} and #{max_upload_date} and pd.additional_information like '%'+ #{reference} + '%' and pd.value_date like '%'+#{value_date}+ '%' and pd.branch like '%'+#{branch_code}+'%'  and pm.issue_core_id = pd.id and pd.availability = 1 and pd.status=1 ")
	public List<Transactionhistory> searchWithMaxAmountAndUploadDateIssueCore(@Param("max_amount") float max_amount,
			@Param("min_upload_date") int min_upload_date, @Param("max_upload_date") int max_upload_date,
			@Param("reference") String reference, @Param("value_date") String value_date,
			@Param("branch_code") String branch_code);

	@Select("select distinct pd.id,pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,issue_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and pd.file_id=fud.file_id and pd.amount between  #{min_amount} and #{max_amount}  and pd.upload_date between #{min_upload_date} and #{max_upload_date} and pm.match_date between #{min_match_date} and #{max_match_date} and pd.additional_information like '%'+ #{reference} + '%' and pd.value_date like '%'+#{value_date}+ '%' and pd.branch like '%'+#{branch_code}+'%'  and pm.issue_core_id = pd.id and pd.availability = 1 and pd.status=1 ")
	public List<Transactionhistory> searchWithAmountUploadDateAndMatchDateIssueCore(
			@Param("min_amount") float min_amount, @Param("max_amount") float max_amount,
			@Param("min_upload_date") int min_upload_date, @Param("max_upload_date") int max_upload_date,
			@Param("min_match_date") int min_match_date, @Param("max_match_date") int max_match_date,
			@Param("reference") String reference, @Param("value_date") String value_date,
	...
	@Select("select distinct pd.id,pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,issue_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and pd.file_id=fud.file_id and pd.amount >=  #{min_amount}  and pd.upload_date between #{min_upload_date} and #{max_upload_date} and pm.match_date between #{min_match_date} and #{max_match_date} and pd.additional_information like '%'+ #{reference} + '%' and pd.value_date like '%'+#{value_date}+ '%' and pd.branch like '%'+#{branch_code}+'%'  and pm.issue_core_id = pd.id and pd.availability = 1 and pd.status=1 ")
	public List<Transactionhistory> searchWithMinAmountUploadDateAndMatchDateIssueCore(
			@Param("min_amount") float min_amount, @Param("min_upload_date") int min_upload_date,
			@Param("max_upload_date") int max_upload_date, @Param("min_match_date") int min_match_date,
			@Param("max_match_date") int max_match_date, @Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	@Select("select distinct pd.id,pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,issue_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and pd.file_id=fud.file_id and pd.amount <=  #{max_amount}  and pd.upload_date between #{min_upload_date} and #{max_upload_date} and pm.match_date between #{min_match_date} and #{max_match_date} and pd.additional_information like '%'+ #{reference} + '%' and pd.value_date like '%'+#{value_date}+ '%' and pd.branch like '%'+#{branch_code}+'%'  and pm.issue_core_id = pd.id and pd.availability = 1 and pd.status=1")
	public List<Transactionhistory> searchWithMaxAmountUploadDateAndMatchDateIssueCore(
			@Param("max_amount") float max_amount, @Param("min_upload_date") int min_upload_date,
			@Param("max_upload_date") int max_upload_date, @Param("min_match_date") int min_match_date,
			@Param("max_match_date") int max_match_date, @Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	@Select("select distinct pd.id ,pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,issue_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and pd.file_id=fud.file_id and  pd.upload_date between #{min_upload_date} and #{max_upload_date} and pm.match_date between #{min_match_date} and #{max_match_date} and pd.additional_information like '%'+ #{reference} + '%' and pd.value_date like '%'+#{value_date}+ '%' and pd.branch like '%'+#{branch_code}+'%'  and pm.issue_core_id = pd.id and pd.availability = 1 and pd.status=1 ")
	public List<Transactionhistory> searchWithUploadDateAndMatchDateIssueCore(
			@Param("min_upload_date") int min_upload_date, @Param("max_upload_date") int max_upload_date,
			@Param("min_match_date") int min_match_date, @Param("max_match_date") int max_match_date,
			@Param("reference") String reference, @Param("value_date") String value_date,
			@Param("branch_code") String branch_code);

	@Select("select distinct pd.id ,pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,issue_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and pd.file_id=fud.file_id and pd.amount between #{min_amount} and #{max_amount} and pm.match_date between #{min_match_date} and #{max_match_date} and pd.additional_information like '%'+ #{reference} + '%' and pd.value_date like '%'+#{value_date}+ '%' and pd.branch like '%'+#{branch_code}+'%' and pm.issue_core_id = pd.id and pd.availability = 1 and pd.status=1 ")
	public List<Transactionhistory> searchWithAmountAndMatchDateIssueCore(@Param("min_amount") float min_amount,
			@Param("max_amount") float max_amount, @Param("min_match_date") int min_match_date,
			@Param("max_match_date") int max_match_date, @Param("reference") String reference,
			@Param("value_date") String value_date, @Param("branch_code") String branch_code);

	@Select("select distinct pd.id ,pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,issue_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and pd.file_id=fud.file_id and pd.amount >= #{min_amount} and pm.match_date between #{min_match_date} and #{max_match_date} and pd.additional_information like '%'+ #{reference} + '%' and pd.value_date like '%'+#{value_date}+ '%' and pd.branch like '%'+#{branch_code}+'%' and pm.issue_core_id = pd.id and pd.availability = 1 and pd.status=1 ")
	public List<Transactionhistory> searchWithMinAmountAndMatchDateIssueCore(@Param("min_amount") float min_amount,
			@Param("min_match_date") int min_match_date, @Param("max_match_date") int max_match_date,
			@Param("reference") String reference, @Param("value_date") String value_date,
			@Param("branch_code") String branch_code);

	@Select("select distinct pd.id ,pd.additional_information,pd.amount,pd.dr_cr,pd.match_status,pd.branch as branch_code,pd.value_date,pd.upload_date,pd.posting_date,u.firstname,u.lastname  from issue_core pd,issue_matched pm, users u,file_user_date fud  where  u.id =fud.user_id and pd.file_id=fud.file_id and pd.amount <= #{max_amount} and pm.match_date between #{min_match_date} and #{max_match_date} and pd.additional_information like '%'+ #{reference} + '%' and pd.value_date like '%'+#{value_date}+ '%' and pd.branch like '%'+#{branch_code}+'%' and pm.issue_core_id = pd.id and pd.availability = 1 and pd.status=1 ")
	public List<Transactionhistory> searchWithMaxAmountAndMatchDateIssueCore(@Param("max_amount") float max_amount,
			@Param("min_match_date") int min_match_date, @Param("max_match_date") int max_match_date,
			@Param("reference") String reference, @Param("value_date") String value_date,
			@Param("branch_code") String branch_code);
...
//			@Param("value_date") String value_date, @Param("branch_code") String branch_code);
	...
	@Select("select  rna.id,rm.match_date,rm.reconciliation_type,u.firstname,u.lastname,rac.amount,rac.additional_information,rac.branch_code  from rtgs_nbe_ats rna,rtgs_matched rm,users u,user_rgts_matched urm,rtgs_awb_core rac where u.id=urm.user_id and rm.id=urm.rtgs_matched_partial_id and rna.id=rm.rtgs_nbe_ats_id and rm.match_id=rac.match_id and  rna.id=#{id} and rna.value_date_type=#{value_date}  union "
			+ "select  bna.id,bm.match_date,bm.reconciliation_type,u.firstname,u.lastname,bac.amount,bac.additional_information,bac.branch_code  from b2b_nbe_ats bna,b2b_matched bm,users u,user_b2b_matched ubm,b2b_awb_core bac where u.id=ubm.user_id and bm.id=ubm.b2b_matched_partial_id and bna.id=bm.b2b_nbe_ats_id and bm.match_id=bac.match_id and  bna.id=#{id} and bna.value_date_type=#{value_date} union "
			+ "select  ena.id,em.match_date,em.reconciliation_type,u.firstname,u.lastname,eac.amount,eac.additional_information,eac.branch_code  from erca_nbe_ats ena,erca_matched em,users u,user_erca_matched uem,erca_awb_core eac where u.id=uem.user_id and em.id=uem.erca_matched_id and ena.id=em.erca_nbe_ats_id and em.match_id=eac.match_id and  ena.id=#{id} and ena.value_date_type=#{value_date} union "
			+ "select  sna.id,sm.match_date,sm.reconciliation_type,u.firstname,u.lastname,sac.amount,sac.additional_information,sac.branch_code  from sos_nbe_ats sna,sos_matched sm,users u,user_sos_matched usm,sos_awb_core sac where u.id=usm.user_id and sm.id=usm.sos_matched_id and sna.id=sm.sos_nbe_ats_id and sm.match_id=sac.match_id and  sna.id=#{id} and sna.value_date_type=#{value_date} union "
			+ "select  apm.id,pm.match_date,pm.reconciliation_type,u.firstname,u.lastname,cpm.amount,cpm.additional_information,cpm.branch_code  from ats_partially_matched apm,partially_matched pm,users u,user_parial_match upm,core_partially_matched cpm where u.id=upm.user_id and pm.id=upm.partial_matched_id and apm.id=pm.ats_id and pm.match_id=cpm.match_id and  apm.id=#{id} and apm.value_date_type=#{value_date} ")
	...
	@Select("select distinct dna.id, dna.reference, dna.amount, dna.value_date_type, dna.dr_cr, dna.upload_date, dna.additional_information, er.edit_delete from data_nbe_ats dna, edit_reason er where dna.id=er.current_id and er.type='data_nbe_ats'"
			+ "union select distinct rna.id, rna.reference, rna.amount, rna.value_date_type, rna.dr_cr, rna.upload_date, rna.additional_information,er.edit_delete from rtgs_nbe_ats rna, edit_reason er where rna.id=er.current_id and er.type='rtgs_nbe_ats' and er.edit_delete='1'"
			+ "union select distinct bna.id, bna.reference, bna.amount, bna.value_date_type, bna.dr_cr, bna.upload_date, bna.additional_information,er.edit_delete from b2b_nbe_ats bna, edit_reason er where bna.id=er.current_id and er.type='b2b_nbe_ats' and er.edit_delete='1' "
			+ "union select distinct ena.id, ena.reference, ena.amount, ena.value_date_type, ena.dr_cr, ena.upload_date, ena.additional_information,er.edit_delete from erca_nbe_ats ena, edit_reason er where ena.id=er.current_id and er.type='erca_nbe_ats' and er.edit_delete='1'"
			+ "union select distinct sna.id, sna.reference, sna.amount, sna.value_date_type, sna.dr_cr, sna.upload_date, sna.additional_information,er.edit_delete from sos_nbe_ats sna, edit_reason er where sna.id=er.current_id and er.type='sos_nbe_ats' and er.edit_delete='1'")
	List<File_rtgs_nbe_ats> get_edited_ats();

	@Select("select distinct dna.id, dna.amount, dna.value_date, dna.dr_cr, dna.upload_date, dna.additional_information , er.edit_delete, er.type from data_awb_core dna, edit_reason er where dna.id=er.current_id and er.type='data_awb_core'"
			...
			+ "union select distinct rna.id, rna.amount, rna.value_date, rna.dr_cr, rna.upload_date, rna.additional_information, er.edit_delete, er.type from rtgs_awb_core rna, edit_reason er where rna.id=er.current_id and er.type='rtgs_awb_core' and er.edit_delete='1'"
			+ "union select distinct bna.id, bna.amount, bna.value_date, bna.dr_cr, bna.upload_date, bna.additional_information, er.edit_delete, er.type from b2b_awb_core bna, edit_reason er where bna.id=er.current_id and er.type='b2b_awb_core' and er.edit_delete='1' "
			+ "union select distinct ena.id, ena.amount, ena.value_date, ena.dr_cr, ena.upload_date, ena.additional_information, er.edit_delete, er.type from erca_awb_core ena, edit_reason er where ena.id=er.current_id and er.type='erca_awb_core' and er.edit_delete='1'"
			+ "union select distinct sna.id, sna.amount, sna.value_date, sna.dr_cr, sna.upload_date, sna.additional_information, er.edit_delete, er.type from sos_awb_core sna, edit_reason er where sna.id=er.current_id and er.type='sos_awb_core' and er.edit_delete='1'"
	...
	List<File_rtgs_awb_core> get_edited_core();
	...
	@Select("select distinct dna.id, dna.amount, dna.value_date, dna.dr_cr, dna.upload_date, dna.additional_information ,er.edit_delete from data_payable dna, edit_reason er where dna.id=er.current_id and er.type='data_payable' "
			...
			+ "")
	List<File_rtgs_awb_core> get_edited_payable();
...
//			+ "")
	...
	List<File_rtgs_awb_core> get_edited_detail_payable(Long id);
	// ======================================================ending

	@Select("select distinct dna.id, dna.new_old, dna.reference, dna.amount, dna.value_date_type, dna.dr_cr, dna.upload_date, dna.additional_information,er.reason, u.firstname, u.lastname, er.DATE, er.edit_delete , dna.edit_reason_id from nbe_ats_edit_history dna, edit_reason er,users u where er.current_id=#{id} and er.id=dna.edit_reason_id and u.id=er.user_id")
	List<File_rtgs_nbe_ats> get_edited_detail_ats(Long id);

	@Select("select distinct dna.id, dna.new_old, dna.amount, dna.value_date, dna.dr_cr, dna.upload_date, dna.additional_information, er.reason, u.firstname, u.lastname, er.DATE, er.edit_delete , dna.edit_reason_id from awb_core_edit_history dna, edit_reason er, users u where er.current_id=#{id} and er.id=dna.edit_reason_id and u.id=er.user_id"
	...
	List<File_rtgs_awb_core> get_edited_detail_core(Long id);

	@Select("select  rac.id,rm.match_date,rm.reconciliation_type,u.firstname,u.lastname,rna.amount,rna.reference,rna.value_date_type  from rtgs_awb_core rac,rtgs_matched rm,users u,user_rgts_matched urm,rtgs_nbe_ats rna where u.id=urm.user_id and rm.id=urm.rtgs_matched_partial_id and rac.match_id=rm.match_id and rm.rtgs_nbe_ats_id=rna.id and  rac.id=#{id} and rac.additional_information=#{reference} union "
			+ "select top 1 bac.id,bm.match_date,bm.reconciliation_type,u.firstname,u.lastname,bna.amount,bna.reference,bna.value_date_type   from b2b_awb_core bac,b2b_matched bm,users u,user_b2b_matched ubm,b2b_nbe_ats bna where u.id=ubm.user_id and bm.id=ubm.b2b_matched_partial_id and bac.match_id=bm.match_id and bm.b2b_nbe_ats_id=bna.id and  bac.id=#{id} and bac.additional_information=#{reference} union "
			+ "select top 1 eac.id,em.match_date,em.reconciliation_type,u.firstname,u.lastname,ena.amount,ena.reference,ena.value_date_type   from erca_awb_core eac,erca_matched em,users u,user_erca_matched uem,erca_nbe_ats ena where u.id=uem.user_id and em.id=uem.erca_matched_id and eac.match_id=em.match_id and em.erca_nbe_ats_id=ena.id and  eac.id=#{id} and eac.additional_information=#{reference}  union "
			+ "select sac.id,sm.match_date,sm.reconciliation_type,u.firstname,u.lastname,sna.amount,sna.reference,sna.value_date_type   from sos_awb_core sac,sos_matched sm,users u,user_sos_matched usm,sos_nbe_ats sna where u.id=usm.user_id and sm.id=usm.sos_matched_id and sac.match_id=sm.match_id and sm.sos_nbe_ats_id=sna.id and  sac.id=#{id}  and sac.additional_information=#{reference} union  "
			+ "select  cpm.id,pm.match_date,pm.reconciliation_type,u.firstname,u.lastname,apm.amount,apm.reference,apm.value_date_type  from core_partially_matched cpm,partially_matched pm,users u,user_parial_match upm,ats_partially_matched apm where u.id=upm.user_id and pm.id=upm.partial_matched_id and apm.id=pm.ats_id and pm.match_id=cpm.match_id and cpm.id=#{id} and cpm.additional_information=#{reference}  ")
	public MatchDetailCore matchDetailCore(@Param("id") Long id, @Param("reference") String reference);
	...
	public MatchDetailCore matchDetailPayableCredit(Long id);
	...
	public MatchDetailCore matchDetailReceivableCredit(Long id);

	@Select("select top 1  rc.id,rm.match_date,rm.reconciliation_type,u.firstname,u.lastname,rd.amount,rd.additional_information,rd.value_date  from issue_core rc,issue_matched rm,users u,user_issue_matched urm,issue_qbs rd where u.id=urm.user_id and rm.id=urm.issue_matched_id and rc.id=rm.issue_core_id and rm.match_id=rd.match_id and  rc.id=#{id}")
	public MatchDetailCore matchDetailIssueCore(Long id);

	@Select("select top 1  rc.id,rm.match_date,rm.reconciliation_type,u.firstname,u.lastname,rd.amount,rd.additional_information,rd.value_date  from issue_qbs rc,issue_matched rm,users u,user_issue_matched urm,issue_core rd where u.id=urm.user_id and rm.id=urm.issue_matched_id and rc.match_id=rm.match_id and rm.issue_core_id=rd.id and  rc.id=#{id}")
	public MatchDetailCore matchDetailIssueQBS(Long id);
	...
	public MatchDetailCore matchDetailReceivableDebit(Long id);
	...
	public MatchDetailCore matchDetailPayableDebit(Long id);
	////////////////////////////////////////////////////////////////////////////
	...
	@Select("               select distinct rna.id, rna.value_date_type, rna.sender, rna.receiver, u.firstname,u.lastname,rm.match_date, "
			...
			+ "						         rna.availability, rna.reference,rna.file_id,rm.match_id,mr.reason from users u, match_reason mr, user_rgts_matched urm, rtgs_nbe_ats rna, file_account fa, rtgs_matched rm where rna.status = 1 and rna.availability = 1 and rna.match_status=1  "
			+ "			 	                 and rna.file_id = fa.file_id and fa.account_id =#{account_id} and  rm.match_date = #{date} and rm.rtgs_nbe_ats_id = rna.id "
			...
			+ "					             sna.availability, sna.reference,sna.file_id, sm.match_id, mr.reason from  users u, match_reason mr, user_sos_matched usm, sos_nbe_ats sna, sos_matched sm, file_account fa where sna.status = 1 and sna.availability = 1 and sna.match_status = 1 "
			+ "						 		and sna.file_id = fa.file_id and fa.account_id = #{account_id} and sm.match_date =#{date} and sm.sos_nbe_ats_id = sna.id  "
			...
			+ "						        bna.availability, bna.reference,bna.file_id, bm.match_id, mr.reason from users u, match_reason mr, user_b2b_matched ubm, b2b_nbe_ats bna, b2b_matched bm, file_account fa where bna.status = 1 and bna.availability = 1 and bna.match_status = 1  "
			+ "						 		and bna.file_id = fa.file_id and fa.account_id= #{account_id} and bm.match_date= #{date} and bm.b2b_nbe_ats_id = bna.id and bm.match_id=mr.match_id and bm.status=1 and  "
			...
			+ "				               ena.availability, ena.reference, ena.file_id, em.match_id, mr.reason  from  users u, match_reason mr ,user_erca_matched uem, erca_nbe_ats ena, file_account fa, erca_matched em where ena.status = 1 and ena.availability = 1 and ena.match_status = 1 "
			+ "					          and ena.file_id = fa.file_id and fa.account_id = #{account_id} and em.match_date= #{date} and em.erca_nbe_ats_id=ena.id and em.match_id=mr.match_id and em.status=1 "
			+ "					        and em.availability =1  and u.id=uem.user_id and uem.erca_matched_id=em.id and uem.availability=1 and uem.status=1")
	List<File_rtgs_nbe_ats> get_all_ats_matched_with_reason(@Param("date") String recon_date,
			@Param("account_id") Long account_id);
	...
	@Select("select distinct eac.id, eac.posting_date, eac.transaction_reference, eac.branch_code, "
			...
			+ "			 			eac.value_date, eac.additional_information, eac.file_id, eac.match_id, mr.reason from erca_awb_core eac,  match_reason mr, file_account fa,users u,user_erca_matched uem, erca_matched em where eac.status = 1 and eac.availability = 1 and eac.match_status = 1 "
			...
			+ "					bac.value_date, bac.additional_information, bac.file_id, bac.match_id, mr.reason from b2b_awb_core bac, match_reason mr, b2b_matched bm, file_account fa, "
			...
			+ "					sac.value_date, sac.additional_information, sac.file_id, sac.match_id, mr.reason from sos_awb_core sac, match_reason mr, sos_matched sm, file_account fa, "
			...
			+ "					rac.value_date, rac.additional_information, rac.file_id, rac.match_id, mr.reason from rtgs_awb_core rac, match_reason mr, file_account fa,  "
	...
	List<File_rtgs_awb_core> get_all_core_matched_with_reason(@Param("date") String recon_date,
			@Param("account_id") Long account_id);
	...
	@Select("		select  distinct rac.id, rac.posting_date, rac.transaction_reference, rac.branch_code, "
			...
			+ "					 and urm.user_id=u.id and urm.rtgs_matched_partial_id=rm.id and urm.availability=1 and rm.match_id=mr.match_id and urm.status=1")
	List<File_rtgs_awb_core> get_payable_creditmatched_with_reason();
	...
	@Select("		select  distinct rac.id, rac.posting_date, rac.transaction_reference, rac.branch_code, "
			...
			+ "					 and urm.user_id=u.id and urm.rtgs_matched_partial_id=rm.id and urm.availability=1 and rm.match_id=mr.match_id and urm.status=1")
	List<File_rtgs_awb_core> get_payable_debitmatched_with_reason();

	@Select("select amount from data_nbe_ats where id=#{long1}")
	Double getAtsAmount(Long long1);

	@Select("select amount from data_awb_core where id=#{long1}")
	Double getCoreAmount(String long1);

	@Select("select REFERENCE from data_nbe_ats where id =#{long1}")
...
//	List<File_rtgs_awb_core> get_credit_for_reconcile(@Param("account_id") Long account_id);

	@Select("select *" + " from data_payable dp where "
			+ " dp.dr_cr = 'cr' and dp.status = 1 and dp.availability = 1")
	List<File_rtgs_awb_core> get_credit_for_reconcile();
//
//	@Select("select *" + " from data_payable dp, file_account fa where "
...
//	List<File_rtgs_awb_core> get_debit_for_reconcile(@Param("account_id") Long account_id);

	@Select("select *" + " from data_payable dp where "
			+ " dp.dr_cr = 'dr' and dp.status = 1 and dp.availability = 1")
	List<File_rtgs_awb_core> get_debit_for_reconcile();
	...
	List<File_rtgs_awb_core> PayableRawData_unmatched(String replace);
	...
	List<File_rtgs_awb_core> PayableRawData_matched_cr(String replace);
	...
	List<File_rtgs_awb_core> PayableRawData_matched_dr(String replace);
...
//	List<File_rtgs_awb_core> RecievableRawData_unmatched(String replace);
//
...
//	List<File_rtgs_awb_core> RecievableRawData_matched_cr(String replace);
//
...
//			+ ", rd.balance, rd.ctr from "
	...
	List<File_rtgs_awb_core> RecievableRawData_matched_dr(String replace);
	...
	@Select(" SELECT CASE "
			...
			+ "WHEN EXISTS (SELECT 1 FROM data_stock_core dsc  WHERE cast(dsc.posting_date as date)<= #{date} and dsc.account like '%12100') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_mms sm  WHERE cast(sm.date as date)<= #{date} and sm.account_segment like '%121') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_core sc  WHERE cast(sc.posting_date as date) <= #{date} and sc.account like '%12100') THEN 'True' "
			+ "ELSE 'False' "
			+ "END AS record_exists; ")
	...
	@Select(" SELECT CASE "
			...
			+ "WHEN EXISTS (SELECT 1 FROM data_stock_core dsc  WHERE cast(dsc.posting_date as date)<= #{date} and dsc.account like '%11100') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_mms sm  WHERE cast(sm.date as date)<= #{date} and sm.account_segment like '%111') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_core sc  WHERE cast(sc.posting_date as date) <= #{date} and sc.account like '%11100') THEN 'True' "
			+ "ELSE 'False' "
			+ "END AS record_exists; ")
	...
	@Select(" SELECT CASE "
			...
			+ "WHEN EXISTS (SELECT 1 FROM data_stock_core dsc  WHERE cast(dsc.posting_date as date)<= #{date} and dsc.account like '%11300') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_mms sm  WHERE cast(sm.date as date)<= #{date} and sm.account_segment like '%113') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_core sc  WHERE cast(sc.posting_date as date) <= #{date} and sc.account like '%11300') THEN 'True' "
			+ "ELSE 'False' "
			+ "END AS record_exists; ")
	...
	@Select(" SELECT CASE "
			...
			+ "WHEN EXISTS (SELECT 1 FROM data_stock_core dsc  WHERE cast(dsc.posting_date as date)<= #{date} and dsc.account like '%10500') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_mms sm  WHERE cast(sm.date as date)<= #{date} and sm.account_segment like '%105') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_core sc  WHERE cast(sc.posting_date as date) <= #{date} and sc.account like '%10500') THEN 'True' "
			+ "ELSE 'False' "
			+ "END AS record_exists; ")
	...
	@Select(" SELECT CASE "
			...
			+ "WHEN EXISTS (SELECT 1 FROM data_stock_core dsc  WHERE cast(dsc.posting_date as date)<= #{date} and dsc.account like '%11900') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_mms sm  WHERE cast(sm.date as date)<= #{date} and sm.account_segment like '%119') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_core sc  WHERE cast(sc.posting_date as date) <= #{date} and sc.account like '%11900') THEN 'True' "
			+ "ELSE 'False' "
			+ "END AS record_exists; ")
	...
	@Select(" SELECT CASE "
			...
			+ "WHEN EXISTS (SELECT 1 FROM data_stock_core dsc  WHERE cast(dsc.posting_date as date)<= #{date} and dsc.account like '%12000') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_mms sm  WHERE cast(sm.date as date)<= #{date} and sm.account_segment like '%120') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_core sc  WHERE cast(sc.posting_date as date) <= #{date} and sc.account like '%12000') THEN 'True' "
			+ "ELSE 'False' "
			+ "END AS record_exists; ")
	...
	@Select(" SELECT CASE "
			...
			+ "WHEN EXISTS (SELECT 1 FROM data_stock_core dsc  WHERE cast(dsc.posting_date as date)<= #{date} and dsc.account like '%11200') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_mms sm  WHERE cast(sm.date as date)<= #{date} and sm.account_segment like '%112') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_core sc  WHERE cast(sc.posting_date as date) <= #{date} and sc.account like '%11200') THEN 'True' "
			+ "ELSE 'False' "
			+ "END AS record_exists; ")
	...
	@Select(" SELECT CASE "
			...
			+ "WHEN EXISTS (SELECT 1 FROM data_stock_core dsc  WHERE cast(dsc.posting_date as date)<= #{date} and dsc.account like '%10600') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_mms sm  WHERE cast(sm.date as date)<= #{date} and sm.account_segment like '%106') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_core sc  WHERE cast(sc.posting_date as date) <= #{date} and sc.account like '%10600') THEN 'True' "
			+ "ELSE 'False' "
			+ "END AS record_exists; ")
	...
	@Select(" SELECT CASE "
			...
			+ "WHEN EXISTS (SELECT 1 FROM data_stock_core dsc  WHERE cast(dsc.posting_date as date)<= #{date} and dsc.account like '%10700') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_mms sm  WHERE cast(sm.date as date)<= #{date} and sm.account_segment like '%107') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_core sc  WHERE cast(sc.posting_date as date) <= #{date} and sc.account like '%10700') THEN 'True' "
			+ "ELSE 'False' "
			+ "END AS record_exists; ")
	...
	@Select(" SELECT CASE "
			...
			+ "WHEN EXISTS (SELECT 1 FROM data_stock_core dsc  WHERE cast(dsc.posting_date as date)<= #{date} and dsc.account like '%10400') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_mms sm  WHERE cast(sm.date as date)<= #{date} and sm.account_segment like '%104') THEN 'True' "
			+ "WHEN EXISTS (SELECT 1 FROM stock_core sc  WHERE cast(sc.posting_date as date) <= #{date} and sc.account like '%10400') THEN 'True' "
			+ "ELSE 'False' "
			+ "END AS record_exists; ")
	boolean existsFile104_office_equipment(@Param("date") String string);
	
	@Select("IF EXISTS (SELECT * FROM data_awb_core WHERE id = #{id}) SELECT 'True' ELSE SELECT 'False'")
	boolean coreIdExist(Long id);
