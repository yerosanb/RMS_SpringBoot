...
public interface MapperFixedReport {
	...
	//Furniture
	@Select(" select distinct dic.id, dic.transaction_date, dic.account_description, dic.naration, dic.account_name, dic.debit as amount from data_fixed_ dic where dic.transaction_date <=#{date} and dic.credit=0 and dic.status = 1 and dic.availability = 1 and dic.account_number like '01195000054%' union \r\n"
			+ "	select distinct ic.id, ic.transaction_date, ic.account_description, ic.naration, ic.account_name, ic.debit as amount from fixed_ ic, fixed_matched im, fixed_mms iq \r\n"
	...
	List<Fixed__report> reportDebitFurn(String date);
	
	@Select(" select distinct dic.id, dic.transaction_date, dic.account_description, dic.naration, dic.account_name, dic.credit as amount from data_fixed_ dic where dic.transaction_date <= #{date} and dic.debit=0 and dic.status = 1 and dic.availability = 1 and dic.account_number like '01195000054%' union \r\n"
			+ "	 select distinct ic.id, ic.transaction_date, ic.account_description, ic.naration, ic.account_name, ic.debit as amount from fixed_ ic, fixed_matched im, fixed_mms iq \r\n"
			...
			+ "	  and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1 and ic.account_number like '01195000054%'")
	List<Fixed__report> reportCreditFurn(String date);
    ...
    @Select(" select distinct dic.id, cast(dic.created_date as date), dic.asset_description, dic.tag_number,dic.branch_name, dic.original_cost as amount from data_fixed_mms dic where cast(dic.created_date as date) <= #{date}  and status = 1 and availability = 1 and dic.main_pg='BF' union \r\n"
    		+ "	  select distinct iq.id, cast(iq.created_date as date), iq.asset_description, iq.tag_number,iq.branch_name, iq.original_cost as amount from fixed_mms iq, fixed_matched im, fixed_ ic\r\n"
...
//			+ "  select distinct rna.id, rna.transaction_date, rna.category_description, rna.reference,rna.additional_information, rna.debit as amount from fixed_mms rna, fixed_matched rm, fixed_ rac \r\n"
...
//	List<Fixed_mms_report> reportMMSDebitFurn(String date);
	...
	@Select(" select ((select ISNULL(sum(dic.debit + dic.credit), 0) from data_fixed_ dic where transaction_date <= #{date} and dic.account_number like '01195000054%' and dic.status = 1 and dic.availability = 1 )\r\n"
			+ "	   + (\r\n"
			+ "	  select ISNULL(sum(ic.credit + ic.debit ), 0) from fixed_ ic, fixed_matched im, fixed_mms iq\r\n"
			...
			+ "	 and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1 and ic.account_number like '01195000054%'))")
	Double getTotalFurn(String date);
	...
	@Select("select ((select ISNULL(sum(dna.original_cost), 0) from data_fixed_mms dna where cast(dna.created_date as date) <= #{date} and dna.main_pg = 'BF' and status = 1 and availability = 1)\r\n"
			+ "	+ (select ISNULL(sum(rna.original_cost), 0) from fixed_mms rna, fixed_matched rm, fixed_ rac \r\n"
	...
	//computer
	@Select(" select distinct dic.id, dic.transaction_date, dic.account_description, dic.naration, dic.account_name, dic.debit as amount from data_fixed_ dic where dic.transaction_date <=#{date} and dic.credit=0 and dic.status = 1 and dic.availability = 1 and dic.account_number like '01197000058%' union \r\n"
			+ "	select distinct ic.id, ic.transaction_date, ic.account_description, ic.naration, ic.account_name, ic.debit as amount from fixed_ ic, fixed_matched im, fixed_mms iq \r\n"
			+ "	 where ic.credit=0 and ic.transaction_date <= #{date}  and (ic.match_id = im.match_id and iq.id = im.fixed_mms_id and cast(iq.created_date as date) > #{date}) and ic.status = 1 and ic.availability = 1  \r\n"
			+ "	and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1  and ic.account_number like '01197000058%'")
	List<Fixed__report> reportDebitComp(String date);

	@Select(" select distinct dic.id, dic.transaction_date, dic.account_description, dic.naration, dic.account_name, dic.credit as amount from data_fixed_ dic where dic.transaction_date <= #{date} and dic.debit=0 and dic.status = 1 and dic.availability = 1 and dic.account_number like '01197000058%' union \r\n"
			+ "	 select distinct ic.id, ic.transaction_date, ic.account_description, ic.naration, ic.account_name, ic.debit as amount from fixed_ ic, fixed_matched im, fixed_mms iq \r\n"
			+ "	 where ic.debit=0 and ic.transaction_date <= #{date} and (ic.match_id = im.match_id and iq.id = im.fixed_mms_id and cast(iq.created_date as date) > #{date}) and ic.status = 1 and ic.availability = 1 \r\n"
			+ "	  and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1 and ic.account_number like '01197000058%'")
	List<Fixed__report> reportCreditComp(String date);
    ...
    @Select(" select distinct dic.id, cast(dic.created_date as date), dic.asset_description, dic.tag_number,dic.branch_name, dic.original_cost as amount from data_fixed_mms dic where cast(dic.created_date as date) <= #{date} and status = 1 and availability = 1 and dic.main_pg='AB' union \r\n"
    		+ "	  select iq.id, cast(iq.created_date as date), iq.asset_description, iq.tag_number,iq.branch_name, iq.original_cost as amount from fixed_mms iq, fixed_matched im, fixed_ ic\r\n"
...
//	List<Fixed_mms_report> reportMMSDebitComp(String date);
	...
	@Select(" select ((select ISNULL(sum(dic.debit + dic.credit), 0) from data_fixed_ dic where transaction_date <= #{date} and dic.account_number like '01197000058%' and dic.status = 1 and dic.availability = 1 )\r\n"
			+ "	   + (\r\n"
			+ "	  select ISNULL(sum(ic.credit + ic.debit ), 0) from fixed_ ic, fixed_matched im, fixed_mms iq\r\n"
			+ "	 where ic.transaction_date <= #{date} and (ic.match_id = im.match_id and iq.id = im.fixed_mms_id and cast(iq.created_date as date) > #{date}) and ic.status = 1 and ic.availability = 1  \r\n"
			+ "	 and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1 and account_number like '01197000058%'))")
	Double getTotalComp(String date);
	...
	//Motor Vehicle
	@Select(" select distinct dic.id, dic.transaction_date, dic.account_description, dic.naration, dic.account_name, dic.debit as amount from data_fixed_ dic where dic.transaction_date <=#{date} and dic.credit=0 and dic.status = 1 and dic.availability = 1 and dic.account_number like '01198000059%' union \r\n"
			+ "	select distinct ic.id, ic.transaction_date, ic.account_description, ic.naration, ic.account_name, ic.debit as amount from fixed_ ic, fixed_matched im, fixed_mms iq \r\n"
			+ "	 where ic.credit=0 and ic.transaction_date <= #{date}  and (ic.match_id = im.match_id and iq.id = im.fixed_mms_id and cast(iq.created_date as date) > #{date}) and ic.status = 1 and ic.availability = 1  \r\n"
			+ "	and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1  and ic.account_number like '01198000059%'")
	List<Fixed__report> reportDebitMotor(String date);

	@Select(" select distinct dic.id, dic.transaction_date, dic.account_description, dic.naration, dic.account_name, dic.credit as amount from data_fixed_ dic where dic.transaction_date <= #{date} and dic.debit=0 and dic.status = 1 and dic.availability = 1 and dic.account_number like '01198000059%' union \r\n"
			+ "	 select distinct ic.id, ic.transaction_date, ic.account_description, ic.naration, ic.account_name, ic.debit as amount from fixed_ ic, fixed_matched im, fixed_mms iq \r\n"
			+ "	 where ic.debit=0 and ic.transaction_date <= #{date} and (ic.match_id = im.match_id and iq.id = im.fixed_mms_id and cast(iq.created_date as date) > #{date}) and ic.status = 1 and ic.availability = 1 \r\n"
			+ "	  and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1 and ic.account_number like '01198000059%'")
	List<Fixed__report> reportCreditMotor(String date);
...
//	List<Fixed_mms_report> reportMMSDebitMotor(String date);
	...
	@Select(" select ((select ISNULL(sum(dic.debit + dic.credit), 0) from data_fixed_ dic where transaction_date <= #{date} and dic.account_number like '01198000059%' and dic.status = 1 and dic.availability = 1 )\r\n"
			+ "	   + (\r\n"
			+ "	  select ISNULL(sum(ic.credit + ic.debit ), 0) from fixed_ ic, fixed_matched im, fixed_mms iq\r\n"
			+ "	 where ic.transaction_date <= #{date} and (ic.match_id = im.match_id and iq.id = im.fixed_mms_id and cast(iq.created_date as date) > #{date}) and ic.status = 1 and ic.availability = 1  \r\n"
			+ "	 and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1 and account_number like '01198000059%'))")
	Double getTotalMotor(String date);
	...
	//Office equipment
	@Select(" select distinct dic.id, dic.transaction_date, dic.account_description, dic.naration, dic.account_name, dic.debit as amount from data_fixed_ dic where dic.transaction_date <=#{date} and dic.credit=0 and dic.status = 1 and dic.availability = 1 and dic.account_number like '01196000056%' union \r\n"
			+ "	select distinct ic.id, ic.transaction_date, ic.account_description, ic.naration, ic.account_name, ic.debit as amount from fixed_ ic, fixed_matched im, fixed_mms iq \r\n"
			+ "	 where ic.credit=0 and ic.transaction_date <= #{date}  and (ic.match_id = im.match_id and iq.id = im.fixed_mms_id and cast(iq.created_date as date) > #{date}) and ic.status = 1 and ic.availability = 1  \r\n"
			+ "	and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1  and ic.account_number like '01196000056%'")
	List<Fixed__report> reportDebitOfficeEqipment(String date);
    
	@Select(" select distinct dic.id, dic.transaction_date, dic.account_description, dic.naration, dic.account_name, dic.credit as amount from data_fixed_ dic where dic.transaction_date <= #{date} and dic.debit=0 and dic.status = 1 and dic.availability = 1 and dic.account_number like '01196000056%' union \r\n"
			+ "	 select distinct ic.id, ic.transaction_date, ic.account_description, ic.naration, ic.account_name, ic.debit as amount from fixed_ ic, fixed_matched im, fixed_mms iq \r\n"
			+ "	 where ic.debit=0 and ic.transaction_date <= #{date} and (ic.match_id = im.match_id and iq.id = im.fixed_mms_id and cast(iq.created_date as date) > #{date}) and ic.status = 1 and ic.availability = 1 \r\n"
			+ "	  and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1 and ic.account_number like '01196000056%'")
	List<Fixed__report> reportCreditOfficeEqipment(String date);
...
//	List<Fixed_mms_report> reportMMSDebitOfficeEqSipment(String date);
	...
	@Select(" select ((select ISNULL(sum(dic.debit + dic.credit), 0) from data_fixed_ dic where transaction_date <= #{date} and dic.account_number like '01196000056%' and dic.status = 1 and dic.availability = 1 )\r\n"
			+ "	   + (\r\n"
			+ "	  select ISNULL(sum(ic.credit + ic.debit ), 0) from fixed_ ic, fixed_matched im, fixed_mms iq\r\n"
			+ "	 where ic.transaction_date <= #{date} and (ic.match_id = im.match_id and iq.id = im.fixed_mms_id and cast(iq.created_date as date) > #{date}) and ic.status = 1 and ic.availability = 1  \r\n"
			+ "	 and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1 and account_number like '01196000056%'))")
	Double getTotalOfficeEqipment(String date);
