...
public interface MapperIssueAccount {
	...
	@Select("select * from data_issue_core where status = 1 and availability = 1 and match_status = 0")
	List<File_rtgs_awb_core> get_core_issue_for_recon();
	...
	@Select("select ic.posting_date,ic.additional_information,ic.amount,ic.branch,ic.file_id,ic.match_status,ic.value_date,u.firstname,u.middlename,"
			+ "	u.lastname,im.match_date,im.match_id,ic.id,ic.upload_date,ic.dr_cr from issue_core ic,users u,user_issue_matched uim, issue_matched im where ic.status = 1 and ic.availability = 1 and ic.match_status = 1"
			+ "	and im.issue_core_id= ic.id  and u.id=uim.user_id and uim.issue_matched_id=im.id and uim.availability=1 and uim.status=1 and im.match_date between #{minDate} and #{maxDate} ;")
	List<File_rtgs_awb_core> get_core_issue_for_view_matched(@Param("minDate") int minDate,
			@Param("maxDate") int maxDate);

	@Select("select * from issue_core_reversal where match_date between #{minDate} and #{maxDate} and availability=1;")
	List<File_rtgs_awb_core> get_core_issue_reversal_matched(@Param("minDate") int minDate,
			@Param("maxDate") int maxDate);

	@Select("select * from issue_qbs_reversal where match_date between #{minDate} and #{maxDate} and availability=1;")
	List<File_rtgs_awb_core> get_qbs_issue_reversal_matched(@Param("minDate") int minDate,
			@Param("maxDate") int maxDate);

	@Select("select * from data_issue_core where availability=1 and status=1 and match_status=0;")
	List<File_rtgs_awb_core> get_core_issue_for_view_unmatched();
	...
	@Select("SELECT distinct dic.id, dic.posting_date, dic.value_date, dic.additional_information, dic.branch, dic.amount, dic.dr_cr, dic.balance, \r\n"
			...
			+ "FROM data_issue_core dic JOIN data_issue_qbs diq ON dic.amount = diq.amount \r\n"
	...
	List<File_rtgs_awb_core> get_core_issue_for_recon_auto();
	...
	@Select("SELECT distinct diq.id, diq.posting_date, diq.value_date, diq.additional_information, diq.branch, diq.amount, diq.dr_cr, diq.balance, \r\n"
			...
			+ "FROM data_issue_qbs diq JOIN data_issue_core dic ON diq.amount = dic.amount \r\n"
	...
	List<File_rtgs_awb_core> get_qbs_issue_for_recon_auto();

	@Select("select * from data_issue_qbs where status = 1 and availability = 1 and match_status = 0")
	List<File_rtgs_awb_core> get_qbs_issue_for_recon();
	...
	List<File_rtgs_awb_core> get_qbs_issue_for_view_matched(@Param("minDate") int minDate,
			@Param("maxDate") int maxDate);

	@Select("select * from data_issue_qbs where availability=1 and status=1 and match_status=0;")
	List<File_rtgs_awb_core> get_qbs_issue_for_view_unmatched();

	@Select("INSERT INTO issue_core(posting_date, value_date, additional_information, branch, amount, dr_cr, balance, upload_date, match_status, status, availability, file_id) "
			...
			+ "FROM data_issue_core where id = #{id}; delete from data_issue_core where id = #{id};")
	Long moveIssueCoreData(@Param("id") Long id);

	@Select("INSERT INTO issue_core_reversal(posting_date, value_date, additional_information, branch, amount, dr_cr, balance, upload_date, match_status,match_id,match_date, status, availability, file_id) "
			...
			+ "FROM data_issue_core where id = #{id}; delete from data_issue_core where id = #{id};")
	Long moveIssueCoreReversalData(@Param("id") Long id, @Param("match_id") String match_id,
	...
	@Select("INSERT INTO dbo.issue_matched(issue_core_id, match_id, match_date, reconciliation_type, many_to_many, status, availability) OUTPUT Inserted.Id "
	...
	@Update("update issue_matched set availability=0 where issue_core_id=#{id}")
	public void deleteIssueMatched(@Param("id") Long long1);

	@Select("INSERT INTO data_issue_core(posting_date, balance, branch, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, value_date, additional_information, file_id )OUTPUT Inserted.id "
			+ "SELECT posting_date, balance, branch, amount, dr_cr, upload_date, '0', status, "
			+ "availability, value_date, additional_information, file_id "
			+ "FROM issue_core where id = #{id}; update issue_core set availability = 0 where id = #{id};")
	Long moveIssueCoreMatched(@Param("id") Long long1);

	@Select("INSERT INTO data_issue_core(posting_date, balance, branch, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, value_date, additional_information, file_id )OUTPUT Inserted.id "
			+ "SELECT posting_date, balance, branch, amount, dr_cr, upload_date, '0', status, "
			+ "availability, value_date, additional_information, file_id "
			+ "FROM issue_core_reversal where id = #{id}; update issue_core_reversal set availability = 0 where id = #{id};")
	Long moveIssueCoreMatchedReveersal(@Param("id") Long long1);
	...
	@Select("select id from issue_matched where issue_core_id = #{id}")
	...
	@Select("select  distinct rac.id, rac.posting_date,rac.branch as branch_code,\r\n"
			...
			+ "	and urm.user_id=u.id and urm.issue_matched_id=rm.id and urm.availability=1 and rm.match_id=mr.match_id and urm.status=1")
	List<File_rtgs_awb_core> get_issue_qbs_matched_with_reason();

	@Select("select distinct dna.id, dna.amount, dna.value_date, dna.dr_cr, dna.upload_date, dna.additional_information , er.edit_delete from data_issue_core dna, edit_reason er where dna.id=er.current_id and er.type='data_issue_core'\r\n"
			+ "union select distinct rna.id, rna.amount, rna.value_date, rna.dr_cr, rna.upload_date, rna.additional_information, er.edit_delete from issue_core rna, edit_reason er where rna.id=er.current_id and er.type='issue_core' and er.edit_delete='1' ")
	List<File_rtgs_awb_core> get_edited_issue_core();
	...
	List<File_rtgs_awb_core> get_edited_detail_issue_QBS(Long id);
	...
	Boolean updateTransactionQBS(File_rtgs_nbe_ats edit_data);
	...
	@Select("INSERT INTO issue_core_history(posting_date, branch, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, value_date, additional_information, file_id,edit_reason_id,new_old) "
			+ "SELECT posting_date, branch, amount, dr_cr, upload_date, '1', status, "
			+ "availability, value_date, additional_information, file_id, #{edit_reason_id},'old' FROM data_issue_core where id = #{id}")
	void moveOldIssueCoreData(@Param("id") Long id, @Param("edit_reason_id") Long edit_reason_id);

	@Update("update data_issue_core set amount =#{amount}, value_date = #{value_date_type}, "
			+ "dr_cr = #{dr_cr}, upload_date = #{upload_date}, additional_information = #{additional_information} where id = #{id}")
	...
	@Select("INSERT INTO issue_core_history(posting_date,branch, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, value_date, additional_information, file_id,edit_reason_id,new_old) "
			+ "SELECT posting_date, branch, amount, dr_cr, upload_date, '1', status, "
			+ "availability, value_date, additional_information, file_id, #{edit_reason_id},'new' FROM data_issue_core where id = #{id}")
	void moveEditedIssueCoreeData(@Param("id") Long id, @Param("edit_reason_id") Long edit_reason_id);
	...
	@Select("INSERT INTO issue_core_history(posting_date,  branch, amount, dr_cr, upload_date, "
			+ "match_status, status, availability, value_date, additional_information, file_id,edit_reason_id,new_old) "
			+ "SELECT posting_date, branch, amount, dr_cr, upload_date, '1', status, "
			+ "availability, value_date, additional_information, file_id, #{reason_id},'delete' FROM data_issue_core where id = #{id}")
	void moveDeletedIssueCoreData(@Param("id") Long id, @Param("reason_id") Long reason_id);

	@Update("update data_issue_core set availability = '0' where id = #{id}")
	void deleteTransactionissueore(@Param("id") Long long1);

	@Select("select distinct dic.id, dic.amount, dic.branch as branch_code, dic.additional_information, SUBSTRING(dic.value_date, 1, 8) as value_date from data_issue_core dic where upload_date <= #{date} and dr_cr = 'DR' and dic.status = 1 and dic.availability = 1 union \r\n"
			+ " 			 select distinct ic.id, ic.amount, ic.branch, ic.additional_information, ic.value_date from issue_core ic, issue_matched im, issue_qbs iq \r\n"
			+ "			 where ic.dr_cr = 'DR' and ic.upload_date <= #{date}  and (iq.match_id = im.match_id and ic.id = im.issue_core_id and iq.upload_date > #{date} ) and ic.status = 1 and ic.availability = 1  \r\n"
	...
	@Select("select distinct dic.id, dic.amount, dic.branch as branch_code, dic.additional_information,  SUBSTRING(dic.value_date, 1, 8) as value_date from data_issue_core dic where upload_date <= #{date} and dr_cr = 'CR' and dic.status = 1 and dic.availability = 1 union \r\n"
			+ " 			  select distinct ic.id, ic.amount, ic.branch, ic.additional_information, ic.value_date from issue_core ic, issue_matched im, issue_qbs iq \r\n"
			+ "			  where ic.dr_cr = 'CR' and ic.upload_date <= #{date} and (iq.match_id = im.match_id and ic.id = im.issue_core_id and iq.upload_date > #{date}) and ic.status = 1 and ic.availability = 1  \r\n"
			+ "			 and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1")
	...
	@Select("select distinct diq.id, diq.value_date as value_date_type, diq.branch as sender, diq.additional_information as reference, diq.amount from data_issue_qbs diq where upload_date <= #{date} and dr_cr = 'CR' and status = 1 and availability = 1 union \r\n"
			+ " 			select distinct iq.id, iq.value_date as value_date_type, iq.branch as sender, iq.additional_information as reference, iq.amount from issue_qbs iq, issue_matched im, issue_core ic\r\n"
			+ "			 where iq.dr_cr = 'CR' and iq.upload_date <= #{date} and iq.match_id = im.match_id and im.issue_core_id = ic.id and ic.upload_date > #{date} \r\n"
			+ "			 and iq.status = 1 and iq.availability = 1 and im.status = 1 and im.availability = 1  and ic.status = 1 and ic.availability = 1  \r\n"
			+ "			 ")
	List<File_rtgs_nbe_ats> reportQbsCredit(@Param("date") String string);
	...
	@Select("Select distinct dna.id, dna.value_date as value_date_type, dna.branch as sender, dna.additional_information as reference, dna.amount from data_issue_qbs dna where upload_date <= #{date} and dr_cr = 'DR' and status = 1 and availability = 1 union \r\n"
			+ " 			 select distinct rna.id, rna.value_date as value_date_type, rna.branch as sender, rna.additional_information as reference, rna.amount from issue_qbs rna, issue_matched rm, issue_core rac \r\n"
			+ "			 where rna.dr_cr = 'DR' and rna.upload_date <= #{date} and rna.match_id = rm.match_id and rm.issue_core_id = rac.id and rac.upload_date > #{date} \r\n"
			+ "			 and rna.status = 1 and rna.availability = 1 and rm.status = 1 and rm.availability = 1  and rac.status = 1 and rac.availability = 1")
	List<File_rtgs_nbe_ats> reportQbsDebit(@Param("date") String string);

	@Select("\r\n"
			+ "			 select ((select ISNULL(sum(amount), 0) from data_issue_core dic where upload_date <= #{date} and dr_cr = 'DR' and dic.status = 1 and dic.availability = 1 )\r\n"
			+ "			 + (\r\n"
			+ "			 select ISNULL(sum(ic.amount), 0) from issue_core ic, issue_matched im, issue_qbs iq \r\n"
			+ "			 where ic.dr_cr = 'DR' and ic.upload_date <= #{date} and (iq.match_id = im.match_id and ic.id = im.issue_core_id and iq.upload_date > #{date}) and ic.status = 1 and ic.availability = 1  \r\n"
			+ "			 and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1))\r\n"
			+ "")
	Double getCoreTotalDebit(@Param("date") String string);

	@Select(" select ((select ISNULL(sum(amount), 0) from data_issue_core dic where upload_date <= #{date} and dr_cr = 'CR' and dic.status = 1 and dic.availability = 1 )\r\n"
			+ "			 + (\r\n"
			+ "			 select ISNULL(sum(ic.amount), 0) from issue_core ic, issue_matched im, issue_qbs iq \r\n"
			+ "			 where ic.dr_cr = 'DR' and ic.upload_date <= #{date} and (iq.match_id = im.match_id and ic.id = im.issue_core_id and iq.upload_date > #{date}) and ic.status = 1 and ic.availability = 1  \r\n"
			+ "			 and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1))")
...
//
	@Select("select ISNULL(sum(ending_balance_ifb), 0) from files where upload_date like #{date}+'%' and type = 'ISSUE_CORE' and status = 1 and availability = 1")
	Double getIfbEndingBalance(@Param("date") String string);

	@Select("select ending_balance_ifb FROM files where upload_date = #{date} and type = 'ISSUE_CORE' and status = 1 and availability = 1")
	...
	@Select("select ((select ISNULL(sum(dna.amount), 0) from data_issue_qbs dna where upload_date <= #{date} and dr_cr = 'CR' and status = 1 and availability = 1)  \r\n"
			+ " 		+ (select ISNULL(sum(rna.amount), 0) from issue_qbs rna, issue_matched rm, issue_core rac \r\n"
			+ "			where rna.dr_cr = 'CR' and rna.upload_date <= #{date} and rna.match_id = rm.match_id and rm.issue_core_id = rac.id and rac.upload_date > #{date} \r\n"
			+ "			and rna.status = 1 and rna.availability = 1 and rm.status = 1 and rm.availability = 1  and rac.status = 1 and rac.availability = 1))")
	...
	@Select("select distinct ic.id, ic.amount, ic.branch, ic.additional_information, SUBSTRING(ic.value_date, 1, 8) as value_date,iq.value_date as setteled_date from issue_core ic, issue_matched im, issue_qbs iq \r\n"
			+ "where ic.dr_cr = 'DR' and ic.upload_date <= #{date} and ic.upload_date   >= STUFF(#{date}, LEN(#{date})-1, 2, '01')  and  (iq.match_id = im.match_id and ic.id = im.issue_core_id ) and ic.status = 1 and ic.availability = 1 \r\n"
	...
	List<File_rtgs_awb_core> reportCoreDebitSetteled(String date);

	@Select("select ISNULL(sum(ic.amount), 0) from issue_core ic, issue_matched im, issue_qbs iq \r\n"
			+ "where ic.dr_cr = 'DR' and ic.upload_date   <= #{date} and ic.upload_date   >= STUFF(#{date}, LEN(#{date})-1, 2, '01')  and (iq.match_id = im.match_id and ic.id = im.issue_core_id ) and ic.status = 1 and ic.availability = 1 \r\n"
			+ "and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1 and  cast(iq.value_date as date) > cast(SUBSTRING(ic.value_date, 1, 8) as date)")
	Double getCoreTotalDebitSetteled(String date);

	@Select("select distinct ic.id, ic.amount, ic.branch, ic.additional_information, SUBSTRING(ic.value_date, 1, 8) as value_date,iq.value_date as setteled_date from issue_core ic, issue_matched im, issue_qbs iq \r\n"
			+ "where ic.dr_cr = 'CR' and ic.upload_date <= #{date} and ic.upload_date   >= STUFF(#{date}, LEN(#{date})-1, 2, '01')  and  (iq.match_id = im.match_id and ic.id = im.issue_core_id ) and ic.status = 1 and ic.availability = 1 \r\n"
			+ "and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1 and  cast(iq.value_date as date) > cast(SUBSTRING(ic.value_date, 1, 8) as date)")
	List<File_rtgs_awb_core> reportCoreCreditSetteled(String replace);

	@Select("select ISNULL(sum(ic.amount), 0) from issue_core ic, issue_matched im, issue_qbs iq \r\n"
			+ "where ic.dr_cr = 'CR' and ic.upload_date   <= #{date} and ic.upload_date   >= STUFF(#{date}, LEN(#{date})-1, 2, '01')  and (iq.match_id = im.match_id and ic.id = im.issue_core_id ) and ic.status = 1 and ic.availability = 1 \r\n"
			+ "and im.status = 1 and im.availability = 1  and iq.status = 1 and iq.availability = 1 and  cast(iq.value_date as date) > cast(SUBSTRING(ic.value_date, 1, 8) as date)")
	Double getCoreTotalCreditSetteled(String date);

	@Select(" select distinct iq.id, iq.value_date as value_date_type, iq.branch as sender, iq.additional_information as reference, iq.amount, SUBSTRING(ic.value_date, 1, 8) as setteled_date from issue_qbs iq, issue_matched im, issue_core ic\r\n"
			+ " where iq.dr_cr = 'CR' and iq.upload_date <= #{date} and  iq.upload_date   >= STUFF(#{date}, LEN(#{date})-1, 2, '01') and iq.match_id = im.match_id and im.issue_core_id = ic.id\r\n"
	...
	List<File_rtgs_nbe_ats> reportQbsCreditSetteled(String date);

	@Select("select ISNULL(sum(iq.amount), 0) from issue_qbs iq, issue_matched im, issue_core ic\r\n"
			+ " where iq.dr_cr = 'CR' and iq.upload_date <= #{date} and iq.upload_date   >= STUFF(#{date}, LEN(#{date})-1, 2, '01') and iq.match_id = im.match_id and im.issue_core_id = ic.id\r\n"
			+ "and iq.status = 1 and iq.availability = 1 and im.status = 1 and im.availability = 1  and ic.status = 1 and ic.availability = 1 and cast(SUBSTRING(ic.value_date, 1, 8) as date)> cast(iq.value_date as date)")
	Double getQbsTotalCreditSetteled(String date);

	@Select(" select distinct iq.id, iq.value_date as value_date_type, iq.branch as sender, iq.additional_information as reference, iq.amount, SUBSTRING(ic.value_date, 1, 8) as setteled_date from issue_qbs iq, issue_matched im, issue_core ic\r\n"
			+ " where iq.dr_cr = 'DR' and iq.upload_date <= #{date} and  iq.upload_date   >= STUFF(#{date}, LEN(#{date})-1, 2, '01') and iq.match_id = im.match_id and im.issue_core_id = ic.id\r\n"
			+ "and iq.status = 1 and iq.availability = 1 and im.status = 1 and im.availability = 1  and ic.status = 1 and ic.availability = 1 and cast(SUBSTRING(ic.value_date, 1, 8) as date)> cast(iq.value_date as date) ")
	List<File_rtgs_nbe_ats> reportQbsDebitSetteled(String date);
